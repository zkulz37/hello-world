...
<%
    ...
    = "SELECT G.ORG_NM,v.code " +
        ",sum(case when  MONTHS_BETWEEN (last_day(to_date('" + p_dt + "','yyyymm')),TO_DATE(E.JOIN_DT,'YYYYMMDD') ) " +
        " > NVL(V.NUM_1,0) AND MONTHS_BETWEEN (last_day(to_date('" + p_dt + "','yyyymm')),TO_DATE(E.JOIN_DT,'YYYYMMDD') ) <=  NVL(V.NUM_2,MONTHS_BETWEEN (last_day(to_date('" + p_dt + "','yyyymm')),TO_DATE(E.JOIN_DT,'YYYYMMDD')) )  " +
        ...
        "  and nvl(left_dt,'30000101')>to_char(last_day(to_date('" + p_dt + "','yyyymm')),'yyyymmdd') " +
         ...
         "                   OR '" + p_org_pk + "' = 'ALL') " +
        "  GROUP BY nvl(g.seq,1000),G.PK,G.ORG_NM,V.CODE " +
        "  order by nvl(g.seq,1000),G.ORG_NM,G.PK,V.CODE ";
