...
<%
    ...
    string SQL
     = "select (SELECT   max(T.org_nm)    " +
         ...
         "from tco_org g,  " +
         "    (select a.tco_org_pk tco_org_pk " +
         "    ,sum( case when substr(a.left_dt,1,6)='" + p_year + "'||'01'    then 1 else 0 end ) wt_01 " +
         "    ,sum( case when substr(a.left_dt,1,6)='" + p_year + "'||'02'    then 1 else 0 end ) wt_02 " +
         "    ,sum( case when substr(a.left_dt,1,6)='" + p_year + "'||'03'    then 1 else 0 end ) wt_03 " +
         "    ,sum( case when substr(a.left_dt,1,6)='" + p_year + "'||'04'    then 1 else 0 end ) wt_04 " +
         "    ,sum( case when substr(a.left_dt,1,6)='" + p_year + "'||'05'    then 1 else 0 end ) wt_05 " +
         "    ,sum( case when substr(a.left_dt,1,6)='" + p_year + "'||'06'    then 1 else 0 end ) wt_06 " +
         "    ,sum( case when substr(a.left_dt,1,6)='" + p_year + "'||'07'    then 1 else 0 end ) wt_07 " +
         "    ,sum( case when substr(a.left_dt,1,6)='" + p_year + "'||'08'    then 1 else 0 end ) wt_08 " +
         "    ,sum( case when substr(a.left_dt,1,6)='" + p_year + "'||'09'    then 1 else 0 end ) wt_09 " +
         "    ,sum( case when substr(a.left_dt,1,6)='" + p_year + "'||'10'    then 1 else 0 end ) wt_10 " +
         "    ,sum( case when substr(a.left_dt,1,6)='" + p_year + "'||'11'    then 1 else 0 end ) wt_11 " +
         "    ,sum( case when substr(a.left_dt,1,6)='" + p_year + "'||'12'    then 1 else 0 end ) wt_12 " +
         "from thr_employee a " +
         "where a.del_if=0 and a.left_dt is not null " +
         "and a.left_dt like '" + p_year + "%' " +
...
<table border=0 cellpadding=0 cellspacing=0 width=2101 style='border-collapse:
 ...
 <tr height=43 style='height:32.25pt'>
  ...
  mso-ignore:colspan;width:96pt'>Yearly Resign Employees</td>
