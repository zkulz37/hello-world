...
<%   string kind, p_nation, p_org_pk;
     string p_status;
     p_status = Request["p_status"].ToString();  
     kind = Request["p_1"].ToString();
    ...
    if (kind == "5")//position ( job title)
     {
         hr = "HR0008";
         field = "e.pos_type";
     }

   if (kind == "6")//employee type 
     {
         hr = "HR0017";
         field = "e.employee_type";
     }
   if (kind == "7")//contract type 
   {
       hr = "HR0001";
       field = "e.contract_type";
   }
     string SQL
        = "select G.ORG_NM,g.code,g.num from ( " +
            "SELECT g.pk,G.ORG_NM,v.code code  " +
            ",sum(case when  nvl(" + field + ",' ')=v.code   " +
            " then 1 else 0 end )as num   " +
            " FROM THR_EMPLOYEE E, VHR_HR_CODE V,TCO_ORG G     " +
            "  WHERE  V.ID='"+hr+"' AND E.DEL_IF =0 AND G.DEL_IF=0 AND G.PK=E.TCO_ORG_PK  " +
            "  and e.JOIN_DT <=to_char(sysdate,'yyyymmdd')  " +
            "  and nvl(left_dt,'30000101')>to_char(sysdate,'yyyymmdd')  " +
            ...
            "union all   " +//null
            "SELECT g.pk,G.ORG_NM,'ZZZZZZ' code  " +
            ",sum(case when  nvl(" + field + ",' ')=' '   " +
            " then 1 else 0 end )as num   " +
            " FROM THR_EMPLOYEE E, TCO_ORG G     " +
            "  WHERE   E.DEL_IF =0 AND G.DEL_IF=0 AND G.PK=E.TCO_ORG_PK  " +
            "  and e.JOIN_DT <=to_char(sysdate,'yyyymmdd')  " +
            "  and nvl(left_dt,'30000101')>to_char(sysdate,'yyyymmdd')  " +
            " and decode('" + p_nation + "','ALL','ALL',e.nation)='" + p_nation + "' " +
             " AND (   e.tco_org_pk IN ( " +
             ...
             "                         CONNECT BY PRIOR g.pk = g.p_pk) " +
             "                   OR '" + p_org_pk + "' = 'ALL') " +
            "  GROUP BY G.PK,G.ORG_NM  " +
            ")g " +
            "  order by G.ORG_NM,G.PK,g.CODE   " +
            "    ";
    ...
    string SQL2
    = "select d.CODE_NM,upper(s.CODE_NM)||' STATISTICS'  from thr_code_detail d,thr_code_master s where s.del_if=0 and d.del_if=0 and s.use_yn='Y'  " +
        "and d.use_yn='Y' and s.id='" + hr + "' and s.pk=d.THR_CODE_MASTER_PK order by d.seq ";
    ...
    Double total=0;
    Double[] dept = new Double[dt_Label.Rows.Count+2];
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=581 style='border-collapse:
 ...
 width:75pt'>
 <col class=xl24 width=105 span=<%=dt_Label.Rows.Count+1%> style='mso-width-source:userset;mso-width-alt:3840;
 width:79pt'>
 ...
 <tr height=21 style='height:15.75pt'>
  ...
  <td colspan=6 rowspan=2 class=xl32 width=277 style='width:208pt'><%=dt_Label.Rows[0][1].ToString() %></td>
 </tr>
 <tr height=21 style='height:15.75pt'>
 ...
 <tr height=25 style='height:18.75pt'>
  ...
  <%} %>
  <td class=xl29 style='border-left:none'>Other </td>
 ...
 <tr height=21 style='height:15.75pt'>
  ...
  <% for(int j=0 ;j<= dt_Label.Rows.Count; j++) {
   %>
    <td class=xl33 style='border-top:none;border-left:none' x:num><%=dept[j] %></td>
  <%} %>
