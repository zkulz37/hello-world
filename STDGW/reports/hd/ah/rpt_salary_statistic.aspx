...
<%
    string p_work_mon, p_nation, p_org_pk;   
    string p_status;
    ...
    string SQL
    = "select e.ORG_NM,v.CODE, " +
        "  sum(case when nvl(s.SALARY_LEVEL2,0) between nvl(v.NUM_1,0) and nvl(v.NUM_2,1000000000)-1 then 1 else 0 end)as num " +
        "  from thr_month_salary s, vhr_hr_code v, tco_org e,thr_employee v " +
        "  where v.id= 'HR0133' and s.DEL_IF=0 and e.DEL_IF=0 and v.del_if=0 and s.thr_emp_pk=v.pk and e.pk= s.TCO_DEPT_PK and s.WORK_MON='" + p_work_mon + "' " +
         " and decode('" + p_nation + "','ALL','ALL',s.nation)='" + p_nation + "' " +
         " and decode('" + p_status + "','ALL','ALL',v.status)='" + p_status + "' " +
         " AND (   e.pk IN ( " +
         "                         SELECT     g.pk " +
         "                               FROM tco_org g " +
         "                              WHERE g.del_if = 0 " +
         ...
         "                   OR '" + p_org_pk + "' = 'ALL') " +
        "  group by e.P_PK, e.ORG_NM, v.CODE " +
        "  order by e.ORG_NM, v.code ";
  ...
  //  Response.Write(SQL);
   // Response.End();
    ...
    string SQL2
        = "select v.CHAR_1  from vhr_hr_code v where v.id='HR0133' order by code ";
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=581 style='border-collapse:
 ...
 <td height=21 class=xl25 width=304 style='height:15.75pt;width:228pt'></td>
  <td height=21 class=xl25 width=304 style='height:15.75pt;width:228pt'>Report
  by: <%=Session["USER_NAME"] %></td>
  <td colspan=6 rowspan=2 class=xl32 width=277 style='width:208pt'>SALARY
  STATISTIC <%= p_work_mon.Substring(4,2)+"-"+ p_work_mon.Substring(0,4) %></td>
 </tr>
 <tr height=21 style='height:15.75pt'>
 ...
 <tr height=25 style='height:18.75pt'>
  <td class=xl29 style='border-left:none'>STT</td>
 ...
 <% 
     ...
     for (int i = 0; i < dt_Emp.Rows.Count; i=i+0)
     {
        ...
        for (int m = 0; m < dt_Label.Rows.Count; m++)
        {
         ...
         %>
 <tr height=21 style='height:15.75pt'>
  <td class=xl28 style='border-top:none;border-left:none'><b><%= k%></b></td>
  <td height=21 class=xl26 style='height:15.75pt;border-top:none;text-align:left'><b><%=dt_Emp.Rows[i][0].ToString()%></b></td>
 ...
 <td class=xl31 style='border-top:none;border-left:none'></td>
  <td height=21 class=xl30 style='height:15.75pt;border-top:none'>Total Employee </td>
 ...
 <td class=xl31 style='border-top:none;border-left:none'></td>
  ...
  <td class=xl31 style='border-top:none;border-left:none' x:num></td>
  <% for(int j=0 ;j< dt_Label.Rows.Count; j++) {
   %>
