...
DataTable dt = ESysLib.TableReadOpen(SQL);
    ...
    SQL = "SELECT SF_A_GET_ACCD(T.PAC_PK," + p_company_pk + ") UAC_CD, LPAD(T.AC_CD, 6 + (ac_level - 1)*6, CHR(32) || CHR(32)), T.AC_NM, T.AC_LNM, T.AC_KNM,  " + 
      ...
      "DECODE(T.BUDGET_YN,'Y','Yes','N','No'), DECODE(T.BANK_YN,'Y','Yes','N','No'), DECODE(T.EMP_YN,'Y','Yes','N','No'), null CONTROL_ITEM " +
        "       FROM TAC_ABACCTCODE T " +
        "      WHERE T.DEL_IF = 0 AND T.TCO_COMPANY_PK =  " + p_company_pk+
        "      AND T.AC_LEVEL > 0  " +
        "      ORDER BY T.AC_CD " ;
    //Response.Write(SQL);
    //Response.End();
...
%>
 <tr height=21 style='height:15.75pt'>
  ...
  <td class=xl42 <%= strColor %> style='border-top:none;border-left:none;text-align:left;' x:str><span> <%= strBegin %> <%= dt.Rows[i][1].ToString() %><%= strEnd %></span></td>
