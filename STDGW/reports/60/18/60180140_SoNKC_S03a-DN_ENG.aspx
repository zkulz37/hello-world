...
SQL = SQL + " FROM DUAL ";
	...
	/*
	//'This SQL Can't use in voucher status = 1
	SQL = " SELECT H.PK, TO_CHAR(H.TR_DATE,'DD/MM/YYYY') TR_DATE, H.VOUCHERNO, "
	SQL = SQL & "    SF_A_GET_ACCD(D.TAC_ABACCTCODE_PK,"+l_company+") DRAC_PK,SF_A_GET_ACCD(R.TAC_ABACCTCODE_PK_CR,"+l_company+") DRAC_PK, "
	SQL = SQL & "    TO_CHAR(SUM(R.TR_BOOKAMT), '" & l_bookformat & "') "
	SQL = SQL & "    , MAX(D.REMARK)  "
	SQL = SQL & "    , MAX(D.REMARK2)   "
	SQL = SQL & "FROM TAC_HGTRH H, TAC_HGTRD D, TAC_HGTRD_REF R "
	SQL = SQL & "WHERE H.DEL_IF = 0  "
	SQL = SQL & "    AND D.DEL_IF = 0  "
	SQL = SQL & "    AND R.DEL_IF = 0 "
	SQL = SQL & "    AND H.PK = D.TAC_HGTRH_PK  "
	SQL = SQL & "    AND D.PK = R.TAC_HGTRD_PK "
	...
	SQL = SQL & "    AND D.DRCR_TYPE = 'D' "
	SQL = SQL & "    AND (('" & l_voucher &"' IS NULL) OR (UPPER (h.voucherno) LIKE'%' || UPPER ('" & l_voucher &"')|| '%')) "
	SQL = SQL & "    AND (('" & l_Seq &"'  IS NULL) OR (h.pk = '" & l_Seq &"' )) "
	SQL = SQL & "  GROUP BY H.PK, H.TR_DATE, H.VOUCHERNO, D.TAC_ABACCTCODE_PK, R.TAC_ABACCTCODE_PK_CR  "
	SQL = SQL & "  ORDER BY H.TR_DATE ASC,H.VOUCHERNO "
	
SQL = "  SELECT H.PK, TO_CHAR(H.TR_DATE,ac_sf_get_format ('ACAB0300')) TR_DATE, H.VOUCHERNO,  ";
SQL = SQL & "    SF_A_GET_ACCD(D.TAC_ABACCTCODE_PK,"+l_company+") DRAC_PK, ";
SQL = SQL & "    SF_A_GET_ACCD(R.TAC_ABACCTCODE_PK,"+l_company+") CRAC_PK,  ";
SQL = SQL & "    SUM(r.TR_BOOKAMT) ";
SQL = SQL & "    , MAX(D.REMARK)   ";
SQL = SQL & "    , MAX(D.REMARK2)    ";
SQL = SQL & "FROM TAC_HGTRH H, TAC_HGTRD D, TAC_HGTRD R  ";
SQL = SQL & "WHERE H.DEL_IF = 0   ";
SQL = SQL & "    AND D.DEL_IF = 0   ";
SQL = SQL & "    AND R.DEL_IF = 0  ";
SQL = SQL & "    AND H.PK = D.TAC_HGTRH_PK   ";
SQL = SQL & "    AND D.TAC_HGTRH_PK  = R.TAC_HGTRH_PK  ";
SQL = SQL & "    AND H.TCO_COMPANY_PK =  " & l_company & " ";
SQL = SQL & "    AND TO_CHAR(H.TR_DATE,'YYYYMMDD') BETWEEN '"& l_date_from &"' AND '"& l_date_to &"'   ";
SQL = SQL & "    AND H.TR_STATUS IN (DECODE ("& l_Status &" , 2, 2, "& l_Status &"),DECODE ("& l_Status &", 2, 0, "& l_Status &"), DECODE ("& l_Status &", 2, 4, "& l_Status &")) ";
SQL = SQL & "    AND D.DRCR_TYPE = 'D'  ";
...
SQL = SQL & "    AND (('" & l_voucher &"' IS NULL) OR (UPPER (h.voucherno) LIKE'%' || UPPER ('" & l_voucher &"')|| '%'))  ";
SQL = SQL & "    AND (('" & l_Seq &"'  IS NULL) OR (h.pk = '" & l_Seq &"' ))  ";
SQL = SQL & "  GROUP BY H.PK, H.TR_DATE, H.VOUCHERNO, D.TAC_ABACCTCODE_PK, R.TAC_ABACCTCODE_PK  ";
SQL = SQL & "  ORDER BY H.TR_DATE ASC,H.VOUCHERNO ";
*/
/*
SQL = " SELECT H.PK, TO_CHAR(H.TR_DATE,ac_sf_get_format ('ACAB0300')) TR_DATE, H.VOUCHERNO, ";
...
*/
SQL = " SELECT H.PK, TO_CHAR(H.TR_DATE,ac_sf_get_format ('ACAB0300')) TR_DATE, H.VOUCHERNO, ";
SQL = SQL + "    SF_A_GET_ACCD_LOCAL1(R.TAC_ABACCTCODE_PK_DR," + l_company + "),  SF_A_GET_ACCD_LOCAL1(R.TAC_ABACCTCODE_PK_CR," + l_company + "), ";
SQL = SQL + "    R.TR_BOOKAMT, R.REMARK , R.REMARK2  ";
SQL = SQL + "    FROM TAC_HGTRH H,  TAC_HGTRD_REF R ";
SQL = SQL + "    WHERE H.DEL_IF = 0  ";
SQL = SQL + "    AND R.DEL_IF = 0 ";
SQL = SQL + "    AND H.PK = R.TAC_HGTRH_PK ";
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1180 style='border-collapse:
 ...
 <tr height=30 style='height:22.5pt'>
  <td colspan=11 height=30 class=xl55 style='height:22.5pt'>GENERAL JOURNAL</td>
 </tr>
