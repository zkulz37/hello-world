...
<%
    ...
    SQL9 = SQL9 + "ac_ac_sf_get_format('ACBG0040'), TO_CHAR(LAST_DAY(TO_DATE('" + l_date_to + "','YYYYMMDD')),'DD')  RP_DD,";
    ...
    SQL9 = SQL9 + " ac_sf_get_format ('ACRP0010') ";
    SQL9 = SQL9 + " FROM DUAL ";
...
SQL += "                                 START WITH pk = " + p_acc_pk_arr + "))) ";
    ...
    SQL += " UNION ALL  SELECT TO_CHAR(h.tr_date, 'DD/MM/YYYY') tr_date_fmt, h.voucherno " +
        ...
        "     , ac_sf_a_get_accd1(tac_abacctcode_pk, '" + l_company_pk + "') acc_code, decode(DRCR_TYPE,'D',r2.tr_bookamt,0)  debit_amt " +
        ...
        "       AND io_yn = 'Y' " +
        "   AND R2.tac_abacctcode_pk IN (  select pk from tac_abacctcode where del_if =0 and  USE_YN ='Y' and TAC_LOCALCODE_PK in     (SELECT hg.TAC_localCODE_PK  FROM TAC_HGDDBAL_local hg WHERE hg.DEL_IF =0 AND hg.tco_company_pk =  '"+ l_company_pk +"'  AND hg.tac_localcode_pk in ( SELECT pk "+
 "                                       FROM tac_localcode "+
 "                                      WHERE del_if = 0 AND leaf_yn = 'Y' "+
 "                                 CONNECT BY pac_pk = PRIOR pk "+
 "                                 START WITH pk = " + p_acc_pk_arr + "))) "+
