...
<%
    SQL = "SELECT T.PK, T.AC_CD, T.AC_NM,  " +
        " TO_CHAR(TO_NUMBER(DECODE(T.DRCR_TYPE,'D','1','-1'))* (AC_GET_DRCR_OPENBAL(T.TCO_COMPANY_PK,T.PK,'" + dtAccLst.Rows[k][3].ToString() + "','" + l_date_from + "','" + l_status + "','D','TRANS') - AC_GET_DRCR_OPENBAL(T.TCO_COMPANY_PK,T.PK,'" + dtAccLst.Rows[k][3].ToString() + "','" + l_date_from + "','" + l_status + "','C','TRANS')),'9,999,999,999,990')   OPEN_BAL  " +
        "FROM TAC_ABACCTCODE T  " +
        "WHERE T.DEL_IF =0  " +
        "AND T.LEAF_YN ='Y' " +
        "CONNECT BY pac_pk = PRIOR pk START WITH pk = (SELECT PK FROM TAC_ABACCTCODE  " +
        "WHERE DEL_IF =0 AND AC_CD = '" + dtAccLst.Rows[k][0].ToString() + "') ";
 ...
 <% 
    SQLDetail
        = "select * from  " +
            ...
            " 0  BAL_VND,  " +
            " D.TR_RATE, H.VOUCHERNO, D.CCY   " +
            "  FROM tac_hgtrh h, tac_hgtrd_ref r, TAC_HGTRD D  " +
            " WHERE h.del_if = 0  " +
...
<%
    ...
    {
        SQL = "SELECT PK, ACCOUNT_NO,BANK_ID, TO_CHAR(AC_GET_BANK_OPENBAL_MONTH('" + l_company_pk + "',PK,'" + l_date_from + "','','" + l_status + "','TRANS'),'9,999,999,999,990.99')  FROM TAC_ABDEPOMT WHERE DEL_IF =0 AND CCY IN ('" + dtAccLst.Rows[k][3].ToString() + "', 'EUR')  "; 
    }
    else
    {
        SQL = "SELECT PK, ACCOUNT_NO,BANK_ID, TO_CHAR(AC_GET_BANK_OPENBAL_MONTH('" + l_company_pk + "',PK,'" + l_date_from + "','','" + l_status + "','TRANS'),'9,999,999,999,990.99')  FROM TAC_ABDEPOMT WHERE DEL_IF =0 AND CCY ='" + dtAccLst.Rows[k][3].ToString() + "' ";
    }
...
<%    
    ...
    = " select * from ( " +
        ...
        "           and h.pk in ( " +
        "           		select H1.PK " +
        "                        FROM TAC_HGTRH H1,TAC_HGDEPODDBAL B  " +
	...
	" union all " +
        ...
        " and not exists (select 'X' from TAC_HGDEPODDBAL B1 where B1.tac_hgtrh_pk = H.pk and B1.del_if = 0 and NVL(B1.YMD_DRTRANS, 0) <> 0) " +
        "          ORDER BY  tr_date,voucherno,H_pK,B_PK " +
        ")	            " +        
        "	union all " +
        "        select  * from " +
        ...
        "                                AND B.STD_YMD BETWEEN '" + l_date_from + "' AND '" + l_date_to + "'             " +
        " AND NVL (B.YMD_CRTRANS, 0) <> 0 " +
        " and not exists (select 'X' from TAC_HGDEPODDBAL B1 where B1.tac_hgtrh_pk = H1.pk and B1.del_if = 0 and NVL(B1.YMD_DRTRANS, 0) <> 0) " +
        "                   )                                      " +
        "                  AND r.del_if = 0  " +
        "               AND r.drcr_type = 'C'  " +
