...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1019 style='border-collapse:
 ...
 <tr height=32 style='mso-height-source:userset;height:24.0pt'>
  ...
  height:24.0pt;width:98pt'>CASH IN HAND (VND)</td>
  <td class=xl39>&nbsp;</td>
...
<%
    SQL = "SELECT T.PK, T.AC_CD, T.AC_NM,  " +
        " TO_CHAR(TO_NUMBER(DECODE(T.DRCR_TYPE,'D','1','-1'))* (SF_GET_DRCR_OPENBAL(T.TCO_COMPANY_PK,T.PK,'VND','" + l_date_from + "','" + l_status + "','D','TRANS') - SF_GET_DRCR_OPENBAL(T.TCO_COMPANY_PK,T.PK,'VND','" + l_date_from + "','" + l_status + "','C','TRANS')),'9,999,999,999,990')   OPEN_BAL  " +
        "FROM TAC_ABACCTCODE T  " +
        "WHERE T.DEL_IF =0  " +
        "AND T.LEAF_YN ='Y' " +
        "CONNECT BY pac_pk = PRIOR pk START WITH pk = (SELECT PK FROM TAC_ABACCTCODE  " +
        "WHERE DEL_IF =0 AND TCO_COMPANY_PK = " + l_company_pk + " AND AC_CD = '1AA111000') ";
 ...
 <tr class=xl42 height=29 style='mso-height-source:userset;height:21.75pt'>
  ...
  height:21.75pt;width:98pt'><%= p_acnm%></td>
 ...
 <%
     SQLDetail = " SELECT 1 STT, INITCAP(TO_CHAR(H.TR_DATE,'DD-MON-YY')) TR_DATE, " +
 ...
 <tr height=25 style='mso-height-source:userset;height:18.75pt'>
  ...
  <td class=xl124 x:num><%= dt_detail.Rows[j][3].ToString()%></td>
  <td class=xl125  x:num><%= dt_detail.Rows[j][4].ToString()%></td>
  <td class=xl126  x:num><%= dt_detail.Rows[j][5].ToString()%></td>
  <td class=xl127  x:num><%= dt_detail.Rows[j][6].ToString()%></td>
  <td class=xl127  x:num><%= dt_detail.Rows[j][7].ToString()%></td>
  <td class=xl49 x:num><%= dt_detail.Rows[j][8].ToString()%></td>
...
<tr height=24 style='mso-height-source:userset;height:18.0pt'>
  ...
  <td class=xl135  style='border-top:.5pt solid black;'  x:num><%= dt_total1.Rows[0][4].ToString()%></td>
...
<%
 ...
 <tr class=xl56 height=38 style='mso-height-source:userset;height:28.5pt'>
  ...
  height:28.5pt;width:98pt'>CASH IN BANK (VND)</td>
...
<%
    SQL = " SELECT T.PK, T.AC_CD, T.AC_NM  " +
      ...
      " AND T.LEAF_YN ='Y' " +
      " CONNECT BY pac_pk = PRIOR pk START WITH pk = (SELECT PK FROM TAC_ABACCTCODE  " +
      " WHERE DEL_IF =0 AND TCO_COMPANY_PK = " + l_company_pk + " AND AC_CD = '1AA112100') ";
    ...
    SQL = "SELECT PK, ACCOUNT_NO,BANK_ID, TO_CHAR(SF_GET_BANK_OPENBAL_MONTH('" + l_company_pk + "',PK,'" + l_date_from + "','','" + l_status + "','TRANS'),'9,999,999,999,990')  FROM TAC_ABDEPOMT WHERE DEL_IF =0 AND CCY='VND' ";
...
<%
    SQLDetail = " SELECT  h.pk h_pk, h.voucherno,h.remark,h.remark2 , INITCAP(TO_CHAR(H.TR_DATE,'DD-MON-YY')) trans_date,B.CCY CCY, " +
        ...
        " DECODE(B.CCY,'VND',NVL(B.YMD_CRTRANS,0),0) out_amt_vnd, 0 BAL_USD_AMT, " +
        " (TO_NUMBER(TRIM('" + p_bankopenbal + "'),'9,999,999,999,990.99')+SUM(NVL(B.YMD_DRTRANS,0) - NVL(B.YMD_CRTRANS,0))OVER (ORDER BY  tr_date,h.voucherno,h.pk,B.PK ))BAL_AMT, " +
        "  h.tr_date,B.PK B_PK " +
        "        FROM TAC_HGTRH H,TAC_HGDEPODDBAL B " +
 ...
 <tr height=27 style='mso-height-source:userset;height:20.25pt'>
  ...
  <td class=xl152 x:num ><%= dt_detail.Rows[j][10].ToString()%></td>
  <td class=xl62 x:num ><%= dt_detail.Rows[j][11].ToString()%></td>
...
"         NVL(SUM(DECODE (b.ccy, 'VND', b.ymd_crtrans, 0)),0) out_amt_vnd, 0 bal_usd_amt, " +
"         (TO_NUMBER (TRIM ('" + p_bankopenbal + "'), '9,999,999,999,990.99') + NVL(SUM (b.ymd_drtrans - b.ymd_crtrans),0)) bal_amt " +
"    FROM tac_hgtrh h, tac_hgdepoddbal b " +
"   WHERE h.del_if = 0 " +
"     AND b.del_if = 0 " +
"     AND h.pk = b.tac_hgtrh_pk " +
...
<%
 ...
 %>
 <tr height=38 style='mso-height-source:userset;height:28.5pt'>
  <td colspan=3 height=38 class=xl110 width=131 style='border-right:.5pt solid black;
  height:28.5pt;width:98pt'>CASH IN BANK (USD)</td>
  <td class=xl58 width=302 style='width:227pt'></td>
  <td class=xl143>&nbsp;</td>
  <td class=xl161>&nbsp;</td>
  <td class=xl145>&nbsp;</td>
...
<%
    SQL = " SELECT T.PK, T.AC_CD, T.AC_NM  " +
      ...
      " AND T.LEAF_YN ='Y' " +
      " CONNECT BY pac_pk = PRIOR pk START WITH pk = (SELECT PK FROM TAC_ABACCTCODE  " +
      " WHERE DEL_IF =0 AND TCO_COMPANY_PK = " + l_company_pk + " AND AC_CD = '1AA112200') ";
    dt_accbank = ESysLib.TableReadOpen(SQL);
    p_strtmp =""; 
   p_accbank_pklist =""; 
    if (dt_accbank.Rows.Count > 0)
    {
    ...
    SQL = "SELECT PK, ACCOUNT_NO,BANK_ID, TO_CHAR(SF_GET_BANK_OPENBAL_MONTH('" + l_company_pk + "',PK,'" + l_date_from + "','','" + l_status + "','TRANS'),'9,999,999,999,990.99')  FROM TAC_ABDEPOMT WHERE DEL_IF =0 AND CCY<>'VND' ";
   
    dt_bank = ESysLib.TableReadOpen(SQL);
    if (dt_bank.Rows.Count > 0)
    {
        for (i = 0; i < dt_bank.Rows.Count; i++)
        {
 ...
 <tr height=35 style='mso-height-source:userset;height:26.25pt'>
  <td colspan=3 height=35 class=xl107 width=131 style='border-right:.5pt solid black;height:26.25pt;width:98pt'><%= p_bankid %></td>
  <td class=xl54 width=302 style='width:227pt'>A balance carried forward from
  the last account</td>
  <td class=xl129>&nbsp;</td>
  <td class=xl74>&nbsp;</td>
  <td class=xl130>&nbsp;</td>
  <td class=xl131>&nbsp;</td>
  <td class=xl163 x:num ><%= p_bankopenbal %></td>
 ...
 <%
    SQLDetail = " SELECT  h.pk h_pk, h.voucherno,h.remark,h.remark2 , INITCAP(TO_CHAR(H.TR_DATE,'DD-MON-YY')) trans_date,B.CCY CCY, " +
        ...
        " DECODE(B.CCY,'VND',NVL(B.YMD_DRTRANS,0),0) in_amt_vnd, " +
        " DECODE(B.CCY,'VND',0,NVL(B.YMD_CRTRANS,0)) out_amt_usd,  " +
        " DECODE(B.CCY,'VND',NVL(B.YMD_CRTRANS,0),0) out_amt_vnd, (TO_NUMBER(TRIM('" + p_bankopenbal + "'),'9,999,999,999,990.99')+SUM(NVL(B.YMD_DRTRANS,0) - NVL(B.YMD_CRTRANS,0))OVER (ORDER BY  tr_date,h.pk,B.PK,h.voucherno )) BAL_USD_AMT, " +
        " 0 BAL_AMT, " +
        "  h.tr_date,B.PK B_PK " +
        "        FROM TAC_HGTRH H,TAC_HGDEPODDBAL B " +
        "        WHERE H.DEL_IF =0  AND B.DEL_IF =0 " +
        ...
        "        AND H.TR_DATE BETWEEN TO_DATE('" + l_date_from + "','YYYYMMDD') AND TO_DATE('" + l_date_to + "' ,'YYYYMMDD') " +
        "        AND B.STD_YMD BETWEEN '" + l_date_from + "' AND '" + l_date_to + "' " +
        "  ORDER BY  tr_date,H_pK,B_PK,voucherno ";
 ...
 <tr height=27 style='mso-height-source:userset;height:20.25pt'>
  ...
  <td class=xl61 width=40 style='width:30pt'></td>
  <td class=xl48 ><%= dt_detail.Rows[j][2].ToString()%></td>
  <td class=xl124 x:num ><%= dt_detail.Rows[j][6].ToString()%></td>
  <td class=xl62 x:num ><%= dt_detail.Rows[j][7].ToString()%></td>
  <td class=xl126 x:num ><%= dt_detail.Rows[j][8].ToString()%></td>
  <td class=xl151 x:num ><%= dt_detail.Rows[j][9].ToString()%></td>
  <td class=xl152 x:num ><%= dt_detail.Rows[j][10].ToString()%></td>
  <td class=xl62 x:num ><%= dt_detail.Rows[j][11].ToString()%></td>
...
"         NVL(SUM(DECODE (b.ccy, 'VND', b.ymd_drtrans, 0)),0) in_amt_vnd, " +
"         NVL(SUM(DECODE (b.ccy, 'VND', 0, b.ymd_crtrans)),0) out_amt_usd, " +
"         NVL(SUM(DECODE (b.ccy, 'VND', b.ymd_crtrans, 0)),0) out_amt_vnd, (TO_NUMBER (TRIM ('" + p_bankopenbal + "'), '9,999,999,999,990.99') + NVL(SUM (b.ymd_drtrans - b.ymd_crtrans),0)) bal_usd_amt, " +
"         0 bal_amt " +
"    FROM tac_hgtrh h, tac_hgdepoddbal b " +
"   WHERE h.del_if = 0 " +
"     AND b.del_if = 0 " +
...
if (dt_total.Rows.Count > 0)
{
 ...
 <tr height=24 style='mso-height-source:userset;height:18.0pt'>
  <td colspan=4 height=24 class=xl89 width=433 style='border-right:.5pt solid black;
  height:18.0pt;width:325pt'>TOTAL</td>
  <td class=xl134  style='border-top:.5pt solid black;'  x:num><%= dt_total.Rows[0][0].ToString()%></td>
  <td class=xl141  style='border-top:.5pt solid black;'  x:num><%= dt_total.Rows[0][1].ToString()%></td>
  <td class=xl134  style='border-top:.5pt solid black;'   x:num><%= dt_total.Rows[0][2].ToString()%></td>
  <td class=xl134  style='border-top:.5pt solid black;'  x:num><%= dt_total.Rows[0][3].ToString()%></td>
  <td class=xl156  style='border-top:.5pt solid black;'  x:num><%= dt_total.Rows[0][4].ToString()%></td>
  <td class=xl44  style='border-top:.5pt solid black;'  x:num><%= dt_total.Rows[0][5].ToString()%></td>
  <td class=xl132  style='border-top:.5pt solid black;' >&nbsp;</td>
  <td class=xl172  style='border-top:.5pt solid black;' >&nbsp;</td>
 </tr>
 <%
