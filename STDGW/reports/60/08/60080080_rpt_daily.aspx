...
<%
    SQL = "SELECT T.PK, T.AC_CD, T.AC_NM,  " +
        " TO_CHAR(TO_NUMBER(DECODE(T.DRCR_TYPE,'D','1','-1'))* (ac_GET_DRCR_OPENBAL(T.TCO_COMPANY_PK,T.PK,'VND','" + l_date_from + "','" + l_status + "','D','TRANS') - ac_GET_DRCR_OPENBAL(T.TCO_COMPANY_PK,T.PK,'VND','" + l_date_from + "','" + l_status + "','C','TRANS')),'9,999,999,999,990')   OPEN_BAL  " +
        "FROM TAC_ABACCTCODE T  " +
        "WHERE T.DEL_IF =0  " +
        "AND T.LEAF_YN ='Y' " +
        "CONNECT BY pac_pk = PRIOR pk START WITH pk = (SELECT PK FROM TAC_ABACCTCODE  " +
        "WHERE DEL_IF =0 AND TCO_COMPANY_PK = " + l_company_pk + " AND AC_CD = '111100') ";
...
<%
    SQL = " SELECT T.PK, T.AC_CD, T.AC_NM  " +
      ...
      " AND T.LEAF_YN ='Y' " +
      " CONNECT BY pac_pk = PRIOR pk START WITH pk = (SELECT PK FROM TAC_ABACCTCODE  " +
      " WHERE DEL_IF =0 AND AC_CD = '112100') ";
    ...
    SQL = "SELECT PK, ACCOUNT_NO,BANK_ID, TO_CHAR(AC_GET_BANK_OPENBAL_MONTH('" + l_company_pk + "',PK,'" + l_date_from + "','','" + l_status + "','TRANS'),'9,999,999,999,990')  FROM TAC_ABDEPOMT WHERE DEL_IF =0 AND CCY='VND' ";
...
<%
   ...
   p_accbank_pklist =""; 
    ...
    SQL = "SELECT PK, ACCOUNT_NO,BANK_ID, TO_CHAR(AC_GET_BANK_OPENBAL_MONTH('" + l_company_pk + "',PK,'" + l_date_from + "','','" + l_status + "','TRANS'),'9,999,999,999,990.99')  FROM TAC_ABDEPOMT WHERE DEL_IF =0 AND CCY<>'VND' ";
