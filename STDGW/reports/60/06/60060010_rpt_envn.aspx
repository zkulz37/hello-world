  ...
  <x:ExcelWorksheets>
   <x:ExcelWorksheet>
	...
	<x:PageBreaks>
     <x:RowBreaks>
<%
	string SQL10 = "SELECT count(*) FROM TAC_HGTRD D WHERE D.DEL_IF = 0 AND D.TAC_HGTRH_PK = '"+p_seq+"' " ;
	DataTable dt_Page = ESysLib.TableReadOpen(SQL10);
	int i_row_break =46;
	for (int m=0; m < dt_Page.Rows.Count ;m++)
	{
		i_row_break += 52;
%>
	  <x:RowBreak>
...
<tr class=xl24 height=46 style='mso-height-source:userset;height:35.1pt'>
 ...
 <%
     SQL = "SELECT CASE    WHEN ac_sf_get_accdlocal_by_acpk(D.TAC_ABACCTCODE_PK, " + p_company_pk + ") <> " +
       "                                    ac_GET_ACCD(D.TAC_ABACCTCODE_PK, " + p_company_pk + ") " +
       "            THEN " +
       "             ac_sf_get_accdlocal_by_acpk(D.TAC_ABACCTCODE_PK, " + p_company_pk + ") || '/' " +
       "             || SF_A_GET_ACCD (D.TAC_ABACCTCODE_PK, " + p_company_pk + ") " +
       "          ELSE " +
       "             ac_GET_ACCD(D.TAC_ABACCTCODE_PK, " + p_company_pk + ") " +
       "       END   ACCD, " +
        " ac_GET_ACNM(D.TAC_ABACCTCODE_PK," + p_company_pk + ", 'ENG'),REMARK,REMARK2 " +
        " ,AC_SF_get_agtritem1(D.PK), " +
...
//Response.End();	
 ...
 <tr height=33 style='mso-height-source:userset;height:24.95pt'>
  <td colspan=3 class=xl111 width=139 style='border-top:.5pt solid black;border-right:.5pt solid black;
  width:104pt'>Korean Description</td>
  <td colspan=13 class=xl111 width=199 style='border-top:.5pt solid black;border-right:.5pt solid black;
  border-left:none;width:149pt'></td>
 </tr>
 
 <tr height=33 style='mso-height-source:userset;height:24.95pt'>
  <td colspan=3 class=xl111 width=139 style='border-right:.5pt solid black;
  width:104pt'>General Director /<br />opinion</td>
  <td colspan=13 class=xl113 width=199 style='border-right:.5pt solid black;
  border-left:none;width:149pt'></td>
 </tr>
