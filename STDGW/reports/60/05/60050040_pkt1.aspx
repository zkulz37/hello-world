...
<%
    ...
    string SQL5 = "SELECT h.pk, h.voucherno, TO_CHAR (h.tr_date, 'DD') ngay, " +
        ...
        "        c.partner_name,  h.tr_enclose, ( select upper(N.CODE_LNM) from TAC_COMMCODE_MASTER M, TAC_COMMCODE_DETAIL N where N.CODE = h.VOUCHER_TYPE and N.DEL_IF = 0 and N.TAC_COMMCODE_MASTER_PK = M.PK and M.del_if = 0 and m.id = 'ACBG0130') voucher_type     " +
        "  FROM tac_hgtrh h, " +
        "       tco_company c, " +
        "       tco_org d, " +
        "       tac_abtrtype a, " +
...
<link rel=File-List href="bbbbbbbbbbbbbbbbbb_files/filelist.xml">
<link rel=Edit-Time-Data href="bbbbbbbbbbbbbbbbbb_files/editdata.mso">
<link rel=OLE-Object-Data href="bbbbbbbbbbbbbbbbbb_files/oledata.mso">
<!--[if gte mso 9]><xml>
 <o:DocumentProperties>
  <o:Author>Phan Thi Thanh Tuyen</o:Author>
  <o:LastAuthor>Mr Long</o:LastAuthor>
  <o:LastPrinted>2010-04-15T02:26:54Z</o:LastPrinted>
  <o:Created>2010-04-09T03:21:05Z</o:Created>
  <o:LastSaved>2010-04-15T02:27:12Z</o:LastSaved>
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=735 style='border-collapse:
 ...
 <tr height=21 style='height:15.75pt'>
  ...
  <td colspan=3 class=xl41 width=345 style='width:260pt'>STT:
  <%=stt%></td>
 </tr>
 <tr height=21 style='height:15.75pt'>
  <td height=21 colspan=2 class=xl24 style='height:15.75pt;mso-ignore:colspan'><%=l_address%></td>
  <td colspan=3 class=xl36>Mẫu số: 002a-DN</td>
 </tr>
 <tr height=21 style='height:15.75pt'>
  <td height=21 colspan=2 class=xl24 style='height:15.75pt;mso-ignore:colspan'></td>
  <td colspan=3 class=xl36>(Ban hành theo QĐ 15/2006/QĐ-BTC</td>
 </tr>
 <tr height=21 style='height:15.75pt'>
  <td height=21 colspan=2 class=xl24 style='height:15.75pt;mso-ignore:colspan'></td>
  <td colspan=3 class=xl36>Ngày 20/03/2006 của Bộ trưởng BTC)</td>
 </tr>
 <tr height=34 style='height:25.5pt'>
  <td colspan=5 height=34 class=xl37 style='height:25.5pt'><%=title%></td>
 </tr>
 <tr height=21 style='height:15.75pt'>
  <td colspan=5 height=21 class=xl38 style='height:15.75pt'>Số phiếu: <%=Sophieu%></td>
 </tr>
 <tr height=21 style='height:15.75pt'>
  <td colspan=5 height=21 class=xl38 style='height:15.75pt'>Ngày <%=ngay%> tháng <%=thang%>
  năm <%=nam%></td>
 </tr>
 <tr height=21 style='height:15.75pt'>
 ...
 <tr class=xl25 height=29 style='mso-height-source:userset;height:21.75pt'>
  ...
  <td class=xl27>Diễn giải</td>
  <td class=xl27>Tài khoản</td>
  <td class=xl27>PS Nợ</td>
  <td class=xl27>PS Có</td>
 </tr>
  <%
     decimal Sum_Dr = 0;
     decimal Sum_Cr = 0;
     string Inword = ""; 
     string SQL7 = "SELECT   ac_get_accd (d.tac_abacctcode_pk, 2) accd, " +
         "         ac_get_acnm (d.tac_abacctcode_pk, 2, 'ENG'), d.remark, d.remark2, " +
         "         +DECODE (drcr_type, " +
         ...
         "          UNION ALL " +
         "          SELECT ac_get_buspartner_id(a.tco_buspartner_pk) buspartner_id, a.buspartner_lnm, a.pk tac_hgtrd_pk " +
         "            FROM tac_hgtrd a " +
         "           WHERE a.del_if = 0 " +
         ...
         "                                                                          d.pk " +
         "ORDER BY drcr_type DESC, drcr_ord ASC ";
     //Response.Write(SQL7);
     //Response.End();
     ...
     for (int i = 0; i < dt_detail.Rows.Count; i++)
     {
         ...
         {
             Sum_Dr += decimal.Parse(dt_detail.Rows[i][4].ToString());
         }
         else
         {
             Sum_Dr += 0;
         ...
         {
             Sum_Cr += decimal.Parse(dt_detail.Rows[i][5].ToString());
         }
         else
         {
             Sum_Cr += 0;
         }
         Inword = CommondLib.Num2VNText(Sum_Cr.ToString(), dt_detail.Rows[i][11].ToString());
 ...
 <tr height=21 style='height:15.75pt'>
  <td height=21 class=xl28 width=145 style='height:15.75pt;width:109pt'><%=dt_detail.Rows[i][9]%>&nbsp;<%=dt_detail.Rows[i][10]%></td>
  <td class=xl29 width=245 style='width:184pt'><%=dt_detail.Rows[i][3]%></td>
  <td class=xl30><%=dt_detail.Rows[i][0]%></td>
  <td class=xl31 style="mso-number-format:'<%= p_xls_ccy_format %>';text-align:right" x:num><span
  style='mso-spacerun:yes'> </span><%=dt_detail.Rows[i][4]%></td>
  <td class=xl31 style="mso-number-format:'<%= p_xls_ccy_format %>';text-align:right" x:num><%=dt_detail.Rows[i][5]%></td>
 </tr>
 <%} %>
  <tr height=21 style='height:15.75pt'>
  <td height=21 class=xl32 style='height:15.75pt'>&nbsp;</td>
  <td class=xl33 style='border-top:none'>&nbsp;</td>
  <td class=xl34 style='border-top:none'>Cộng</td>
  <td class=xl35 style="mso-number-format:'<%= p_xls_ccy_format %>';text-align:right" x:num><span
  style='mso-spacerun:yes'> </span><%=Sum_Dr%></td>
  <td class=xl35 style="mso-number-format:'<%= p_xls_ccy_format %>';text-align:right" x:num><span
  style='mso-spacerun:yes'> </span><%=Sum_Cr%></td>
 </tr>
 <tr height=21 style='height:15.75pt'>
  <td colspan=5 height=21 class=xl39 style='height:15.75pt'>Bằng chữ: <%=Inword%></td>
 </tr>
 <tr height=21 style='height:15.75pt'>
  <td height=21 class=xl24 style='height:15.75pt'>Kèm theo: HĐ số <%=kt%></td>
 ...
 <tr class=xl25 height=21 style='height:15.75pt'>
  ...
  <td colspan=2 class=xl40>QL kế toán tổng hợp</td>
  <td colspan=2 class=xl40>Giám đốc</td>
 </tr>
 <tr height=21 style='height:15.75pt'>
  <td height=21 class=xl24 style='height:15.75pt'>(Ký, họ tên)</td>
  <td colspan=2 class=xl38>(Ký, họ tên)</td>
  <td colspan=2 class=xl38>(Ký, họ tên)</td>
 </tr>
 <tr height=21 style='height:15.75pt'>
  <td height=21 colspan=5 class=xl24 style='height:15.75pt;mso-ignore:colspan'></td>
 ...
 <tr height=21 style='height:15.75pt'>
  <td height=21 class=xl24 style='height:15.75pt'><%= n1 %></td>
  <td colspan=2 class=xl38><%= n2 %></td>
  <td colspan=2 class=xl38><%= n3 %></td>
 </tr>
