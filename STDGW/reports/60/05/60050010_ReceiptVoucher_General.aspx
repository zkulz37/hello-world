...
SQL += " , SUM(d.TR_BOOKAMT) amt_drcr, ac_get_accd (d.tac_abacctcode_pk,'"+p_company_pk+"')|| ' : ' accd_tmp " ;
SQL += "    FROM tac_hgtrd d ";
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=751 style='border-collapse:
 ...
 <tr height=21 style='mso-height-source:userset;height:15.95pt'>
  ...
  width:365pt'>RECEIPT VOUCHER/ PHIẾU THU TIỀN MẶT<br>
    <font class="font9"><br>
    </font><font class="font5">Date/Ngày : <%= p_ngay %>/<%= p_thang%>/<%= p_nam%></font></td>
    <%
		decimal l_drcr_amt = 0;
	...
	{		
		...
		{
			l_drcr_amt = decimal.Parse(dt3.Rows[i][2].ToString());
			if(l_drcr_amt < 1)
				tmp2 = dt3.Rows[i][3].ToString() + l_drcr_amt.ToString();
			else
				tmp2 = dt3.Rows[i][1].ToString();
		}	
		if (tmp1=="D")
