...
<%
    ...
    string SQL9 = " SELECT (select full_name from TAC_EMPLOYEE_ADV where del_if = 0 and pk = a.TAC_EMPLOYEE_ADV_pk) person, " +
          "        a.voucherno, " +
          "        a.tr_enclose, " +
          "        a.voucher_type, " +
          "        a.pk, " +
          ...
          "                             (SELECT present_addr " +
          "                                FROM thr_employee " +
          "                               WHERE del_if = 0 AND pk = adv.thr_employee_pk)) " +
          "                     FROM tac_employee_adv adv " +
          "                    WHERE del_if = 0 AND pk = a.tac_employee_adv_pk)), " +
          "           (SELECT addr2 " +
          "              FROM tco_buspartner " +
          "             WHERE pk = a.tco_buspartner_pk AND del_if = 0)) " +
          "           addr1, " +
          "        DECODE ( " +
          "           a.tco_buspartner_pk, " +
          "           '', DECODE ( " +
...
//Response.End();
	...
	/* SQL = " SELECT a.item " +
          ...
          "  WHERE     a.del_if = 0 " +
          "        AND b.del_if = 0 " +
          "        AND a.TAC_ABACCTITEM_pk = b.pk " +
          "        AND b.TAC_ABITEM_ALIAS = 'CUSTOMER  NAME' " +
          "        AND a.tac_hgtrd_pk IN (SELECT pk " +
          "                                 FROM tac_hgtrd " +
          "                                WHERE drcr_type = 'C' and tac_hgtrh_pk = '"+l_seq+"') " ;
	DataTable dt_cus = ESysLib.TableReadOpen(SQL);
	if (dt_cus.Rows.Count >0)
	{
		p_person = dt_cus.Rows[0][0].ToString();
	}
	*/
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=731 style='border-collapse:
 ...
 1974;width:41pt'>
 <tr height=33 style='mso-height-source:userset;height:14.25pt'>
  <td colspan=6 height=33 class=xl43 width=458 style='height:14.25pt;
  width:345pt'></td>
  <td colspan=4 class=xl44 width=219 style='width:165pt;font-weight:500'>Print Date: <%=p_Print_Time%></td>
 ...
 <tr height=21 style='mso-height-source:userset;height:15.95pt'>
  ...
  width:345pt'>PAYMENT VOUCHER/PHIẾU CHI<br>
    <font class="font9"><br>
    </font><font class="font5">Date/Ngày : <%= p_ngay %>/<%= p_thang%>/<%= p_nam%></font>
...
%>
 ...
 <%
    SQL = "select   " +
        ...
        " AND M.ID='EACAB005' and m.tco_company_pk = '"+p_company_pk+"' " +
        " order by D.ORD ";
