...
//-----------------------------------------------------------------------------------------------------
       ...
       string SQL6 = "select d.tac_hgtrh_pk, COUNT (d.tac_hgtrh_pk),h.TR_TABLE_PK " +
            " from tac_hgtrh h, tac_hgtrd d, TFN_EPPAYTRH f   " +
            " where h.del_if =0 " +
            " and d.del_if =0 " +
            " and h.pk = d.tac_hgtrh_pk " +
            " and h.tr_status<>3 " +
            " and h.TR_TABLENM = 'TFN_EPPAYTRH'  " +
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=999 style='border-collapse:
 ...
 <col width=64 style='width:48pt'>
        ...
        for (int w = 0; w < dt_hgtrh_pk.Rows.Count; w++)
   {
	...
	string SQL
	= "SELECT a.*, c.partner_id, c.partner_name, b.branch, d.item_deposit,   " + 
        "       d.item_nm_deposit, d.trandate, d.voucherno, d.company,d.REMARK2, d.REMARK   " +
        "  FROM (SELECT a.item, a.item_nm, '' table_nm,   " +
        "               a.item_table_pk table_pk  " +
        "          FROM tfn_eppaytrditem a,   " +
        "               tac_abacctitem b,   " +
        "               tac_abitem c,   " +
        "               tfn_eppaytrd d,   " +
        "               tfn_eppaytrh e,   " +
        ...
        "           AND f.del_if = 0   " +
        "           and g.del_if(+) = 0  " +
        "           AND a.tfn_eppaytrd_pk = d.pk  " +
        "           AND d.tfn_eppaytrh_pk = e.pk  " +
        "           AND e.tco_company_pk = f.pk  " +
        "           and e.pk = g.TR_TABLE_PK(+)  " +
        "          and g.TR_TABLENM(+) = 'TFN_EPPAYTRH'  " +
...
string SQL1
	= "SELECT t.pk, sf_a_get_accd (tac_abacctcode_pk, h.tco_company_pk) accd,  " + 
        "       sf_a_get_acnm(tac_abacctcode_pk, h.tco_company_pk, 'ENG') acnm, t.ccy,  " +
        "       t.tr_rate, t.pay_famt, t.pay_amt, t.drcr_type, t.type_ref,  " +
        "       hh.trans_amt, hh.accd  " +
        "  FROM acnt.tfn_eppaytrd t,  " +
        "       tfn_eppaytrh h,  " +
        "       (SELECT t.pk, sf_a_get_accd (tac_abacctcode_pk, h.tco_company_pk) accd,  " +
        "               sf_a_get_acnm (tac_abacctcode_pk, h.tco_company_pk,  " +
        "                              'ENG') acnm, t.ccy, t.tr_rate, t.pay_famt trans_amt, t.pay_amt books_amt,  " +
        "               t.drcr_type, t.type_ref  " +
        "          FROM acnt.tfn_eppaytrd t, tfn_eppaytrh h  " +
        "         WHERE t.del_if = 0  " +
        "           AND h.del_if = 0  " +
        "           AND t.tfn_eppaytrh_pk = h.pk  " +
        "           AND t.tfn_eppaytrh_pk =  " +
        "                  (SELECT tr_table_pk  " +
        "                     FROM tac_hgtrh  " +
        "                    WHERE del_if = 0 AND tr_tablenm = 'TFN_EPPAYTRH'  " +
        ...
        "               (SELECT tr_table_pk  " +
        "                  FROM tac_hgtrh  " +
        "                 WHERE del_if = 0 AND tr_tablenm = 'TFN_EPPAYTRH' AND pk = '" + dt_hgtrh_pk.Rows[w][0].ToString() + "')  " +
        "   AND t.drcr_type = 'C'  " ;
      // Response.Write(SQL1);
      // Response.End();
 ...
 <tr height=25 style='mso-height-source:userset;height:18.75pt'>
  ...
  <td class=xl35 style="border-left:none;mso-number-format:'<%= p_xls_ccy_format %>'" x:num><span
