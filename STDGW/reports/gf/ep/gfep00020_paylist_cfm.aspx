...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1164 class=xl2418464
 ...
 <%
    ...
    string l_bank_acc_no1 = "";
    string l_customer_pk = "";
    if (dt3.Rows.Count > 0)
    {
        l_bank_acc_no = "Sub Total - " + dt3.Rows[0][18].ToString();
		l_bank_acc_no1 = dt3.Rows[0][18].ToString(); // bank id
		l_customer_pk = dt3.Rows[0][17].ToString();
    }    
	for( i = 0 ; i < dt3.Rows.Count ; i++)
	{
        
		if(l_bank_acc_no1 != dt3.Rows[i][18].ToString().Trim() || l_customer_pk != dt3.Rows[i][17].ToString())
		{
...
<%		 
		...
		l_bank_acc_no1 =dt3.Rows[i][18].ToString();
		l_customer_pk = dt3.Rows[i][17].ToString(); 
		}	
		else
		{	
 %>
