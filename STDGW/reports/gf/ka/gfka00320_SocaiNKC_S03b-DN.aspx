...
<%
    ...
    string l_rpt_Option = "FIFO";


    string seq = "" + Request["seq"];
    string l_company_pk = "" + Request["company_pk"];
    string l_account_pk = "2044";
    string l_date_from = "" + Request["dtbFrom"];
    string l_date_to = "" + Request["dtbTo"];
    string l_ccy = "VND";
    string l_status = "2";
    ...
    string SQL = " SELECT PARTNER_NAME,ADDR1, TAX_CODE, TO_CHAR(TO_DATE(TO_CHAR('" + l_date_from + "')||'01','YYYYMMDD'),'YYYYMMDD'), TO_CHAR(LAST_DAY(TO_DATE(TO_CHAR('" + l_date_to + "')||'01','YYYYMMDD')),'YYYYMMDD') FROM TCO_COMPANY WHERE PK = '" + l_company_pk + "' ";
     ...
     <x:RowBreaks>
       <% 
        ...
        for (int m=0; m < da.Rows.Count ;m++)
        {
            ...
            _SQL += " FROM (  SELECT TO_CHAR (h.tr_date, 'DD/MM/YYYY') TR_DATE_FMT, ";
            _SQL += "       h.voucherno , r1.remark2 REMARK2, ";
            _SQL += "       r1.remark REMARK, ac_get_accd (tac_abacctcode_pk_cr," + l_company_pk + ") ACC_CODE, ";
            _SQL += "       r1.tr_bookamt debit_amt, 0 credit_amt, h.pk, h.tr_date, r1.pk r_pk  ";
            ...
            _SQL += " UNION ALL ";
            _SQL += "  SELECT TO_CHAR (h.tr_date, 'DD/MM/YYYY') TR_DATE_FMT, ";
            _SQL += "       h.voucherno, R2.remark2 REMARK2, ";
            _SQL += "       R2.remark REMARK , ac_get_accd (tac_abacctcode_pk_Dr," + l_company_pk + ") ACC_CODE, ";
            _SQL += "       0 debit_amt, R2.tr_bookamt credit_amt, h.pk, h.tr_date, r2.pk r_pk  ";
            ...
            _SQL += "     AND h.tr_date BETWEEN TO_DATE ('" + l_date_from + "', 'YYYYMMDD') AND TO_DATE ('" + l_date_to + "', 'YYYYMMDD') ";
            _SQL += "     AND h.tco_company_pk = '" + l_company_pk + "' ";
            _SQL += " UNION ALL  SELECT TO_CHAR(h.tr_date, 'DD/MM/YYYY') tr_date_fmt, h.voucherno " +
               "     , r2.remark2 remark2, r2.remark remark " +
               "     , ac_get_accd(tac_abacctcode_pk, '" + l_company_pk + "') acc_code, decode(DRCR_TYPE,'D',r2.tr_bookamt,0)  debit_amt " +
               ...
               "       AND io_yn = 'Y' " +
               "   AND R2.tac_abacctcode_pk IN (SELECT     pk       FROM tac_abacctcode     WHERE del_if = 0 AND leaf_yn = 'Y'    CONNECT BY pac_pk = PRIOR pk          START WITH pk = "+da.Rows[m][0].ToString()+" )  " +
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1217 style='border-collapse:
 ...
 <%
    ...
    for (int t = 0; t < da.Rows.Count; t++)
    {
            ...
            SQL = " SELECT AC_CD,AC_LNM|| '  (' || ac_nm ||')',UPPER(a.DRCR_TYPE),  to_char(to_date('" + l_date_from + "','YYYYMMDD'),'DD/MM/YYYY') frm_date, ";
        ...
        else if (l_rpt_Option == "FIFO")
        {
         ...
         <tr class=xl33 height=21 style='height:15.75pt'>
          <td height=21 class=xl37 style='height:15.75pt'>S&#7889;</td>
          <td class=xl38 width=86  style='width:65pt'>Ngày tháng</td>
          <td class=xl37 >N&#7907;</td>
