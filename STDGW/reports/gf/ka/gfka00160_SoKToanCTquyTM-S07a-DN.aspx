...
<%
	...
	/*
SQL = " SELECT   TO_CHAR (NVL (openingbalance, 0), ac_get_format ('ACBG0040')) AS openingbalance, ";
SQL += "               TO_CHAR (NVL (debitbookamount, 0), ac_get_format ('ACBG0040')) AS debitbookamount, ";
SQL += "               TO_CHAR (NVL (creditbookamount, 0), ac_get_format ('ACBG0040')) AS creditbookamount, ";
SQL += "               TO_CHAR (NVL (closingbalance, 0), ac_get_format ('ACBG0040')) AS closingbalance, ";
...
SQL += "                           AND std_ym = SUBSTR (ac_get_lastclosemm ('" + l_date_from +"','" + l_company_pk +"'),1,6)) a, ";
...
SQL += "                           AND tr_status = '" + l_status +"' ";
SQL += "                           AND std_ymd ";
SQL += "                                  BETWEEN TO_CHAR(ADD_MONTHS(TO_DATE (ac_get_lastclosemm('" + l_date_from +"', '" + l_company_pk +"' ),'YYYYMMDD'),+1),'YYYYMMDD') ";
...
*/
    ...
    SQL += "    SELECT TAC_ABACCTCODE_PK, SUM(openingbalance) openingbalance, SUM(debitbookamount) debitbookamount, SUM(creditbookamount) creditbookamount,  " +
        ...
        "       AND std_ym = SUBSTR (ac_get_lastclosemm('" + l_date_from + "',' " + l_company_pk + "'),1,6)  " +
        "    group by TAC_ABACCTCODE_PK " +
        "    UNION ALL " +
...
//TO_CHAR((TO_NUMBER (" + p_open_bal +") + SUM (  NVL (debit_amt, 0)) - SUM(NVL (credit_amt, 0))) OVER (ORDER BY tr_date, pk), ac_get_format ('ACBG0040')) AS closingbalance, 
...
SQL += "       r1.remark REMARK, ac_get_accd (tac_abacctcode_pk_cr,"+l_company_pk+") ACC_CODE, ";
...
SQL += "       R2.remark REMARK , ac_get_accd (tac_abacctcode_pk_Dr,"+l_company_pk+") ACC_CODE, ";
...
*/
SQL = " SELECT   tr_date_fmt, voucherno_debit, voucherno_credit, remark2, remark, " + 
        "         acc_code, debit_amt, credit_amt, (  TO_NUMBER("+p_open_bal+") "+
        "          + SUM (NVL (debit_amt, 0) - NVL (credit_amt, 0)) OVER (ORDER BY  tr_date,order_cond, voucherno , pk,acc_code) ) AS closingbalance, " +
        "         pk, tr_date, voucherno " +
        "    FROM (SELECT TO_CHAR (h.tr_date, 'DD/MM/YYYY') tr_date_fmt, " +
        "                 h.voucherno voucherno_debit, '' voucherno_credit, " +
        "                 MAX(r1.remark2) remark2, MAX(r1.remark) remark, " +
        "                 ac_get_accd (tac_abacctcode_pk_cr,"+l_company_pk+") acc_code, " +
        "                 sum(r1.tr_bookamt) debit_amt, 0 credit_amt, h.pk, h.tr_date " +
        "                 ,h.voucherno voucherno, DECODE('"+p_acc_drcr+ "','D',1) order_cond "  +
        "            FROM tac_hgtrh h, tac_hgtrd_ref r1 " +
        "           WHERE h.del_if = 0 " +
        "             AND r1.del_if = 0 " +
        "             AND h.pk = r1.tac_hgtrh_pk " +
        "             AND r1.drcr_type = 'D' " +
        "             AND r1.tac_abacctcode_pk_dr IN ("+ p_acc_pk_arr +") " +
        ...
        "             GROUP BY h.tr_date,h.voucherno,h.pk,tac_abacctcode_pk_cr " +
        "          UNION ALL " +
        "          SELECT TO_CHAR (h.tr_date, 'DD/MM/YYYY') tr_date_fmt, " +
        "                 '' voucherno_debit, h.voucherno voucherno_credit, " +
        "                 MAX(r2.remark2) remark2, MAX(r2.remark) remark, " +
        "                 ac_get_accd (tac_abacctcode_pk_dr,"+l_company_pk+") acc_code, 0 debit_amt, " +
        "                 sum(r2.tr_bookamt) credit_amt, h.pk, h.tr_date " +
        "                 ,h.voucherno voucherno,DECODE('"+p_acc_drcr+ "','D',2) order_cond "  +
        "            FROM tac_hgtrh h, tac_hgtrd_ref r2 " +
        "           WHERE h.del_if = 0 " +
        "             AND r2.del_if = 0 " +
        "             AND h.pk = r2.tac_hgtrh_pk " +
        "             AND r2.drcr_type = 'D' " +
        "             AND r2.tac_abacctcode_pk_cr IN ("+ p_acc_pk_arr +") " +
       "     AND h.tr_status IN (" + l_status + ",DECODE(" + l_status + ",2,0," + l_status + "),DECODE(" + l_status + ",2,4," + l_status + ")) " +
        "     AND h.tr_date BETWEEN TO_DATE ('" + l_date_from +"', 'YYYYMMDD') AND TO_DATE ('" + l_date_to +"', 'YYYYMMDD') " +
        "             AND h.tco_company_pk like  DECODE ('" + l_company_pk + "', 0, '%%', '" + l_company_pk + "')" +
        "             GROUP BY h.tr_date,h.voucherno,h.pk,tac_abacctcode_pk_dr " +
        "             ) " +
        " ORDER BY tr_date,order_cond, voucherno , pk,acc_code ";
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1290 style='border-collapse:
 ...
 <tr height=21 style='height:15.75pt'>
  ...
  <td class=xl76 colspan=4 >20 tháng 03 n&#259;m 2006
  c&#7911;a B&#7897; tr&#432;&#7901;ng B&#7897; Tài chính)<span style='mso-spacerun:yes'>  </span></td>
  <td class=xl24></td>
 </tr>
 ...
 %>
 <tr class=xl27 >
  <td class=xl55 x:str='<%= strDate_Dsp%>' ><%= strDate_Dsp %></td>
  <td class=xl55 x:str='<%= dt_detail.Rows[i][0].ToString() %>'  style='border-left:none;white-space:nowrap;mso-text-control:shrinktofit;'><%=dt_detail.Rows[i][0].ToString()%></td>
 ...
 %>
 <tr class=xl27 height=21 style='height:15.75pt'>
  <td height=21  colspan=4 class=xl45 style='height:15.75pt;border-top:.5pt solid windowtext;border-right:none;'>&nbsp;</td>
  <td class=xl61 width=250 style='border-top:.5pt solid windowtext;border-left:none;width:188pt'>T&#7893;ng
  c&#7897;ng s&#7889; phát sinh</td>
  <td class=xl61 width=250 style='border-top:.5pt solid windowtext;border-left:none;width:188pt'>&nbsp;</td>
  <td class=xl56 style='border-top:.5pt solid windowtext;border-left:none'>&nbsp;</td>
  <td class=xl62 style='border-top:.5pt solid windowtext;border-left:none' style="mso-number-format:'<%= p_xls_ccy_format %>';" x:num><%= p_debit_xls %></td>
  <td class=xl62 style='border-top:.5pt solid windowtext;border-left:none' style="mso-number-format:'<%= p_xls_ccy_format %>';" x:num><%= p_credit_xls %></td>
  <td class=xl62 style='border-top:.5pt solid windowtext;border-left:none'
  x:str="                    "><span
  style='mso-spacerun:yes'>                      </span></td>
  <td class=xl63 style='border-top:.5pt solid windowtext;border-left:none'>&nbsp;</td>
 </tr>
 <tr class=xl27 height=21 style='height:15.75pt'>
  <td height=21 colspan=4 class=xl64 style='height:15.75pt;border-top:.5pt solid windowtext;border-right:none;'>&nbsp;</td>
  <td class=xl66 width=250 style='border-top:.5pt solid windowtext;border-left:none;width:188pt'>S&#7888;
  D&#431; CU&#7888;I K&#7922;</td>
  <td class=xl66 width=250 style='border-top:.5pt solid windowtext;border-left:none;width:188pt'>&nbsp;</td>
  <td class=xl67 style='border-top:.5pt solid windowtext;border-left:none'>&nbsp;</td>
  <td class=xl68 style='border-top:.5pt solid windowtext;border-left:none' x:str="           "><span
  style='mso-spacerun:yes'>             </span></td>
  <td class=xl69 style='border-top:.5pt solid windowtext;border-left:none' x:str="           "><span
  style='mso-spacerun:yes'>             </span></td>
  <td class=xl69 align=right style='border-top:.5pt solid windowtext;border-left:none'
  style="mso-number-format:'<%= p_xls_ccy_format %>';" x:num><%= p_close_bal_xls %></td>
  <td class=xl70 style='border-top:.5pt solid windowtext;border-left:none'>&nbsp;</td>
 </tr>
 <tr class=xl27 height=21 style='height:15.75pt'>
 ...
 <tr class=xl27 height=21 style='height:15.75pt'>
  ...
  <td class=xl27></td>
  <td colspan=3 class=xl46>Ngày …. Tháng .. N&#259;m</td>
  <td class=xl47></td>
 </tr>
<%
    SQL = "SELECT   " +
 ...
 <tr class=xl27 height=23 style='height:17.25pt'>
  <td colspan=11 height=23 class=xl79 style='height:17.25pt'></td>
 </tr> 
 <tr class=xl27 height=23 style='height:17.25pt'>
  <td colspan=11 height=23 class=xl79 style='height:17.25pt'></td>
 </tr> 
