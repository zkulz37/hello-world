   ...
   if (dt.Rows.Count >0)
    ...
    l_acc_pk = p_acc_grp;
        ...
        SQL = "SELECT   TO_CHAR (NVL (openingtransbalance, 0), '9,999,999,999,990.99') AS open_trans, " +
            "               TO_CHAR (NVL (openingbalance, 0), ac_SF_GET_FORMAT ('ACBG0040')) AS open_books, " +
            "               a.ccy, " +
            ...
            "                " +
            "               TO_CHAR (NVL (BookDeposit, 0), ac_SF_GET_FORMAT ('ACBG0040')) AS debit_book_amt,  " +
            "               TO_CHAR (NVL (BookWithdrawal, 0), ac_SF_GET_FORMAT ('ACBG0040')) AS credit_book_amt, " +
            "               TO_CHAR (NVL (closingbalance, 0), ac_SF_GET_FORMAT ('ACBG0040')) AS bal_book_amt " +
            ...
            "                           AND TAC_ABDEPOMT_PK like decode('" + l_bank_pk + "',0,'%','" + l_bank_pk + "')" +
             "           AND TAC_ABACCTCODE_PK IN (" + l_acc_pk + ") " +
            "                           AND tr_status = " + l_status +
            "                           AND std_ym = SUBSTR (sf_get_lastclosemm ('"+ l_date_from +"' , " + l_company_pk +"),1,6)) a, " +
            "                       (SELECT  " +
...
/*
        SQL = "SELECT TO_CHAR (tr_date, 'DD/MM/YYYY') trans_date, " +
            " h.voucherno, h.remark2,h.remark, SF_A_GET_ACCD(DECODE(D.DRCR_TYPE,'D',DECODE(R.DRCR_TYPE,'C',R.TAC_ABACCTCODE_PK_DR,R.TAC_ABACCTCODE_PK_CR),DECODE(R.DRCR_TYPE,'D',R.TAC_ABACCTCODE_PK_DR,R.TAC_ABACCTCODE_PK_CR)),'" + l_company_pk + "') ACCT_CODE, " +
...
*/
        SQL = "SELECT TO_CHAR (tr_date, 'DD/MM/YYYY') trans_date,  " +
                ...
                "         WHERE H.DEL_IF =0 AND D.DEL_IF =0 AND I.DEL_IF =0 AND R.DEL_IF = 0 " +
                "         AND R.TAC_HGTRD_PK = D.PK     and i.TABLE_NM='TAC_ABDEPOMT'" +
                "        AND H.PK = D.TAC_HGTRH_PK " +
                "        AND D.PK = I.TAC_HGTRD_PK " +
                "        AND D.TAC_ABACCTCODE_PK =  " + l_acc_pk +
                "        AND H.PK IN (SELECT DISTINCT(B.TAC_HGTRH_PK) FROM TAC_HGDEPODDBAL B " +
                "        WHERE B.DEL_IF =0 AND B.TCO_COMPANY_PK = " + l_company_pk +
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1287 style='border-collapse:
 ...
 <tr height=34 style='height:25.5pt'>
  <td colspan=10 height=34 class=xl57 style='height:25.5pt'>S&#7892; TI&#7872;N
  G&#7916;I NGÂN HÀNG</td>
 </tr>
 <tr height=21 style='height:15.75pt'>
  <td colspan=10 height=21 class=xl26 style='height:15.75pt'>T&#7915;
