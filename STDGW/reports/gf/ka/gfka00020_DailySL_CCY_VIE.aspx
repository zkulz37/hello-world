...
<%
	...
	if (l_status == "2")
    {
		p_status = "Đã xác nhận";
		p_status_bal = "Số Dư Cuối Kỳ (ghi sổ)";
        p_status_bal_trans = "Số Dư Cuối Kỳ (giao dịch)";
    }
	else if (l_status == "0") 
    {
		p_status = "Đã duyệt";
    ...
    SQL = "SELECT CCY,TO_CHAR (SUM(OPEN_BAL),AC_GET_FORMAT ('ACBG0040')) openingbalance,   " +
        "            TO_CHAR (SUM(DR_TRANS),'9,999,999,999,999,999,990.99') AS debittransamount, " +
        "            TO_CHAR (SUM(DR_BOOK), AC_GET_FORMAT ('ACBG0040')) AS debitbookamount,  " +
        "            TO_CHAR (SUM(CR_TRANS), '9,999,999,999,999,999,990.99') AS credittransamount, " +
        "            TO_CHAR (SUM(CR_BOOK), AC_GET_FORMAT ('ACBG0040')) AS creditbookamount, " +
        "            TO_CHAR ((SUM(OPEN_BAL) +(SUM(DR_BOOK) - SUM(CR_BOOK))*TO_NUMBER('" + p_acc_type + "')),AC_GET_FORMAT ('ACBG0040')) AS closingbalance, " +
...
SQL += "     ORDER BY h.tr_date,DECODE(d.drcr_type,'" + p_drcr + "',1,2), h.VOUCHERNO, h.PK,d.pk";
    ...
    else if (l_rpt_Option == "FIFO")
    {
        ...
        SQL += "                                 CONNECT BY pac_pk = PRIOR pk ";
        SQL += "                                 START WITH pk = " + l_account_pk + ") ";
        SQL += "     AND upper(tco_company_pk) like  DECODE ('" + l_company_pk + "', 0, '%%', '" + l_company_pk + "')";
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1967 style='border-collapse:
 ...
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  <td height=20 class=xl26 colspan=3 style='height:15.0pt;mso-ignore:colspan'><%= p_dt_today %></td>
 ...
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  <td class=xl27 colspan=6 style='mso-ignore:colspan'>Mã TK:<font
 ...
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  <td height=20 class=xl26 style='height:15.0pt'></td>
  <td class=xl29 colspan=3 style='mso-ignore:colspan'>Ngày:<font class="font9">
  <%= p_dt_from %> ~ <%= p_dt_to %></font></td>
  <td colspan=3 class=xl29 style='mso-ignore:colspan'></td>
  <td class=xl26></td>
  <td class=xl28 colspan=2 style='mso-ignore:colspan'>Trạng thái:<font class="font7">
  <%=p_status%></font></td>
  <td class=xl28></td>
  <td colspan=8 class=xl24 style='mso-ignore:colspan'></td>
 </tr>
 ...
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  width:51pt'>Số chứng từ</td>
  <td rowspan=2 class=xl48 width=68 style='border-bottom:.5pt solid black;
  width:51pt'>Ngày giao dịch</td>
  <td rowspan=2 class=xl48 width=75 style='border-bottom:.5pt solid black;
  width:56pt'>Người đề xuất</td>
  <td rowspan=2 class=xl46 style='border-bottom:.5pt solid black'>Tỷ giá</td>
  <td rowspan=2 class=xl48 width=89 style='border-bottom:.5pt solid black;
  width:67pt'>Số Dư Đầu Kỳ (giao dịch)</td>
  <td rowspan=2 class=xl48 width=89 style='border-bottom:.5pt solid black;
  ...
  <td rowspan=2 class=xl46 style='border-bottom:.5pt solid black'>Khách hàng</td>
  <td colspan=2 class=xl50 style='border-right:.5pt solid black;border-left:
  none'>Diễn giải</td>
  <td rowspan=2 class=xl46 style='border-bottom:.5pt solid black'>PL</td>
  <td rowspan=2 class=xl46 style='border-bottom:.5pt solid black'>Đối tượng</td>
  <td rowspan=2 class=xl46 style='border-bottom:.5pt solid black'>Kèm theo</td>
 </tr>
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  <td height=20 class=xl31 style='height:15.0pt'>Giao dịch</td>
  <td class=xl31>Ghi sổ</td>
  <td class=xl31>Giao dịch</td>
  <td class=xl31>Ghi sổ</td>
  <td class=xl31>Nước ngoài</td>
  <td class=xl31>Địa phương</td>
 </tr>
  <%		
	int i = 0;
...
%> 
 ...
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  border-bottom:.5pt solid black;height:30.0pt'>Tổng cộng</td>
  <td rowspan=2 class=xl46 style='border-bottom:.5pt solid black'>Tỷ giá</td>
  <td rowspan=2 class=xl48 width=89 style='border-bottom:.5pt solid black;
  width:67pt'>Số Dư Đầu Kỳ (giao dịch)</td>
  <td rowspan=2 class=xl48 width=89 style='border-bottom:.5pt solid black;
