...
<%
    ...
    SQL += "to_char(to_date('" + l_date_to + "','YYYYMMDD'),'DD/MM/YYYY') t_date,INITCAP(To_char (sysdate, 'day, month dd  yyyy')), DECODE (UPPER(drcr_type), 'D', 1, -1)  , drcr_type  ";
    ...
    SQL = "SELECT CCY,TO_CHAR (SUM(OPEN_BAL),ac_SF_GET_FORMAT ('ACBG0040')) openingbalance,   " +
        ...
        "          FROM " +
        "          ( " +
        "          SELECT M.TAC_ABACCTCODE_PK, M.CCY,   " +
        "            NVL((SUM(NVL(M.THIS_DRAMT,0)) - SUM(NVL(M.THIS_CRAMT,0))),0)*TO_NUMBER('" + p_acc_type + "') OPEN_BAL, NVL((SUM(NVL(M.THIS_DRFAMT,0)) - SUM(NVL(M.THIS_CRFAMT,0))),0)*TO_NUMBER('" + p_acc_type + "') OPEN_FBAL, " +
        ...
        "                           GROUP BY M.TAC_ABACCTCODE_PK,M.CCY " +
        "          UNION ALL " +
        "          SELECT D.TAC_ABACCTCODE_PK,D.CCY, " +
        "          NVL((SUM(NVL(D.YMD_DRBOOKS,0)) - SUM(NVL(D.YMD_CRBOOKS,0))),0)*TO_NUMBER('" + p_acc_type + "') OPEN_BAL,NVL((SUM(NVL(D.YMD_DRTRANS,0)) - SUM(NVL(D.YMD_CRTRANS,0))),0)*TO_NUMBER('" + p_acc_type + "') OPEN_FBAL, " +
        "           0 DR_TRANS, 0 CR_TRANS, 0 DR_BOOK, 0 CR_BOOK " +
        "                          FROM TAC_HGDDBAL D " +
        "                         WHERE del_if = 0 " +
        "                           AND tco_company_pk = " + l_company_pk +
        ...
        "                          GROUP BY D.TAC_ABACCTCODE_PK,D.CCY " +
        "          UNION ALL " +
        "          SELECT D.TAC_ABACCTCODE_PK, D.CCY, 0 OPEN_BAL,0 OPEN_FBAL, " +
	...
	{
		p_open_bal = dt_total.Rows[0][6].ToString();
	}
    ...
    if (l_rpt_Option == "DRCR")
    {
        ...
        SQL += ",D.REMARK ";
        SQL += ",D.REMARK2 ";
        SQL += ", DECODE(D.TAC_ABPLCENTER_PK,'','',AC_SF_GET_PLCENTER(D.TAC_ABPLCENTER_PK)) ";
        SQL += ",AC_SF_GET_AGTRITEM(D.PK),  h.TR_ENCLOSE  ";
        ...
        SQL += "                                      WHERE del_if = 0 AND upper(leaf_yn) = 'Y' ";
...
<!--[if gte mso 9]><xml>
 <o:DocumentProperties>
  ...
  <o:LastPrinted>2009-01-05T08:15:37Z</o:LastPrinted>
  <o:Created>2005-01-28T07:20:51Z</o:Created>
  <o:LastSaved>2009-01-05T08:33:47Z</o:LastSaved>
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1789 style='border-collapse:
 ...
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  <td rowspan=2 class=xl47 width=68 style='border-bottom:.5pt solid black;
  width:51pt'>Voucher<br>
    No</td>
  <td rowspan=2 class=xl47 width=68 style='border-bottom:.5pt solid black;
  width:51pt'>Trans Date</td>
  <td rowspan=2 class=xl47 width=75 style='border-bottom:.5pt solid black;
  width:56pt'>Proposed<br>
    By</td>
  <td rowspan=2 class=xl45 style='border-bottom:.5pt solid black'>Ccy</td>
  <td rowspan=2 class=xl47 width=89 style='border-bottom:.5pt solid black;
  width:67pt'>Openning <br>
    balance</td>
  ...
  none'>Credit</td>
  <td rowspan=2 class=xl45 style='border-bottom:.5pt solid black'><%= p_status_bal %></td>
  <td rowspan=2 class=xl45 style='border-bottom:.5pt solid black'>Customer</td>
  <td colspan=2 class=xl49 style='border-right:.5pt solid black;border-left:
  none'>Description</td>
  <td rowspan=2 class=xl45 style='border-bottom:.5pt solid black'>PL</td>
  <td rowspan=2 class=xl45 style='border-bottom:.5pt solid black'>Item Control</td>
  <td rowspan=2 class=xl45 style='border-bottom:.5pt solid black'>Enclose</td>
 </tr>
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  <td height=20 class=xl30 style='height:15.0pt'>Transaction</td>
  <td class=xl30>Book</td>
  <td class=xl30>Transaction</td>
  <td class=xl30>Book</td>
  <td class=xl30>Foreign</td>
  <td class=xl30>Local</td>
 </tr>
 <%	
 //  Response.Write(SQL);
//   Response.End();
 ...
 <tr height=26 style='mso-height-source:userset;height:20.1pt'>
  <td height=26 class=xl31 style='height:20.1pt' x:num><%= dt_detail.Rows[i][0].ToString()%></td>
  <td class=xl32><%= dt_detail.Rows[i][1].ToString()%></td>
  <td class=xl32><%= dt_detail.Rows[i][2].ToString()%></td>
  <td class=xl33><%= dt_detail.Rows[i][3].ToString()%></td>
  <td class=xl32><%= dt_detail.Rows[i][4].ToString()%></td>
  <% if( i ==0) { %>
  <td class=xl34 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';"><%=dt_detail.Rows[i][5].ToString() %></td>
  <% } else { %>
  <td class=xl34 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';"><%= dt_detail.Rows[i-1][10].ToString()%></td>
  <% } %>
  <% if (dt_detail.Rows[i][4].ToString() =="VND") { %>  
  <td class=xl34 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';"><%= dt_detail.Rows[i][6].ToString()%></td>
  <td class=xl34 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';"><%= dt_detail.Rows[i][7].ToString()%></td>
  <td class=xl34 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';"><%= dt_detail.Rows[i][8].ToString()%></td>
  <td class=xl34 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';"><%= dt_detail.Rows[i][9].ToString()%></td>
    <%
  }
  else
  {
  %>
  <td class=xl34 x:num style="mso-number-format:'<%= p_xls_ccy_format_usd %>';"><%= dt_detail.Rows[i][6].ToString()%></td>
  <td class=xl34 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';"><%= dt_detail.Rows[i][7].ToString()%></td>
  <td class=xl34 x:num style="mso-number-format:'<%= p_xls_ccy_format_usd %>';"><%= dt_detail.Rows[i][8].ToString()%></td>
  <td class=xl34 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';"><%= dt_detail.Rows[i][9].ToString()%></td>
   <%
  }
  %>
  <td class=xl34 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';"><%= dt_detail.Rows[i][10].ToString()%></td>
  <td class=xl35><%= dt_detail.Rows[i][11].ToString()%></td>
  <td class=xl33><%= dt_detail.Rows[i][12].ToString()%></td>
  <td class=xl33><%= dt_detail.Rows[i][13].ToString()%></td>
  <td class=xl33><%= dt_detail.Rows[i][14].ToString()%></td>
  <td class=xl33><%= dt_detail.Rows[i][15].ToString()%></td>
  <td class=xl36><%= dt_detail.Rows[i][16].ToString()%></td>
 </tr>
  <%
		}
	}
...
%> 
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  <td height=20 colspan=13 class=xl37 style='height:15.0pt;mso-ignore:colspan'></td>
 ...
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  none'>Credit</td>
  <td rowspan=2 class=xl45 style='border-bottom:.5pt solid black'><%= p_status_bal %></td>
  <td rowspan=2 class=xl60>&nbsp;</td>
  <td rowspan=2 class=xl61></td>
  <td colspan=4 class=xl24 style='mso-ignore:colspan'></td>
 </tr>
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  <td height=20 class=xl30 style='height:15.0pt'>Transaction</td>
...
%>  
 <tr height=26 style='mso-height-source:userset;height:20.1pt'>
  <td height=26 class=xl38 style='height:20.1pt'>&nbsp;</td>
  <td class=xl39>&nbsp;</td>
  <td class=xl39>&nbsp;</td>
  <td class=xl40>&nbsp;</td>
  <td class=xl41><%= dt_total.Rows[j][0].ToString() %></td>
  <td class=xl62 rowspan="<%= dt_total.Rows.Count %>" x:num style="mso-number-format:'<%= p_xls_ccy_format %>';border-bottom:.5pt solid windowtext;" ><%= dt_total.Rows[j][1].ToString()   %></td>
 
   <% if (  dt_total.Rows[j][0].ToString() =="VND") { %>
  <td class=xl42 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';"><%= dt_total.Rows[j][2].ToString()   %></td>
  <td class=xl42 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';" ><%= dt_total.Rows[j][3].ToString()   %></td>
  <td class=xl42 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';" ><%= dt_total.Rows[j][4].ToString()   %></td>
  <td class=xl42 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';" ><%= dt_total.Rows[j][5].ToString()   %></td>
   <% }else{ %>
  <td class=xl42 x:num style="mso-number-format:'<%= p_xls_ccy_format_usd %>';"><%= dt_total.Rows[j][2].ToString()   %></td>
  <td class=xl42 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';" ><%= dt_total.Rows[j][3].ToString()   %></td>
  <td class=xl42 x:num style="mso-number-format:'<%= p_xls_ccy_format_usd %>';" ><%= dt_total.Rows[j][4].ToString()   %></td>
  <td class=xl42 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';" ><%= dt_total.Rows[j][5].ToString()   %></td>
  <% } %>  
 
  <td  class=xl62 rowspan="<%= dt_total.Rows.Count %>" x:num style="mso-number-format:'<%= p_xls_ccy_format %>';border-bottom:.5pt solid windowtext;" ><%= dt_total.Rows[j][6].ToString()   %></td>
  <td class=xl43></td>
  <td class=xl44></td>
  <td colspan=4 class=xl24 style='mso-ignore:colspan'></td>
 </tr>
