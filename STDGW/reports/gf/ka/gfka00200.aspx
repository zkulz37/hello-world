...
<%
    //Report by Center
    string ls_tco_company_pk = "", ls_tr_date_from = "", ls_tr_date_to = "", ls_tac_abcenter_pk = "";
    string SQL = "";
    string ls_plc_cd = "", ls_tac_abpl_pk = "";
...
SQL
    = "select to_char(to_date(tbl.tr_date, 'yyyymmdd'),'dd/mm/yyyy') tr_date, pl.pl_cd, tbl.srn_banve, " +
    " tbl.invoice_no seri_begin, NULL seri_end, sum(tbl.qty_banve) qty_banve, " + 
        "        tbl.srn_trave, sum(tbl.qty_trave) qty_trave, sum(tbl.qty_banve) - sum(tbl.qty_trave) qty_thucban, " +
        " tbl.u_price, sum(tbl.tr_amt_trave) tr_amt_trave, " +
        "        sum(tbl.tr_amt_banve) tr_amt_banve, sum(tbl.tr_amt_hopdong) tr_amt_hopdong, sum(tbl.tr_amt_trave) + sum(tbl.tr_amt_banve) + sum(tbl.tr_amt_hopdong) tot_tr_amt      " +
        "    from (         " +
        "        select ar_mst.tr_date,              ch.pk tac_abcenter_pk,          xe_chn.tac_abpl_pk,  " +
        ...
        "        from tac_crcad ar, tac_crca ar_mst, tac_abcenter ch, tac_abplcenter xe_chn " +
        "            " +
        "        where ar.del_if     = 0  " +
        "        and   ar_mst.del_if = 0 " +
        "        and   xe_chn.del_if = 0 " +
        ...
        "        and xe_chn.tac_abcenter_pk   = ch.pk " +
		...
		"        and xe_chn.tac_abpl_pk = '" + ls_tac_abpl_pk + "'                " +		
        "        group by ar_mst.tr_date, ch.pk, xe_chn.tac_abpl_pk, ar.u_price, ar_mst.serial_no, ar_mst.invoice_no " +
        "   union all " +
        "        select ar_mst.tr_date, ch.pk tac_abcenter_pk, xe_chn.tac_abpl_pk,  " +
        "        NULL uprice, NULL srn_banve, ar_mst.serial_no srn_trave,  " +
        ...
        "        and xe_chn.tac_abcenter_pk = '" + ls_tac_abcenter_pk + "'             " +
	"        and xe_chn.tac_abpl_pk = '" + ls_tac_abpl_pk + "'                " +				
        "        group by ar_mst.tr_date, ch.pk, xe_chn.tac_abpl_pk, xe_chn.pk, ar_mst.invoice_no " +
        "    ) " +
        "    tbl, tac_abpl pl, tac_abcenter plc  " +
        "    where tbl.tac_abpl_pk = pl.pk(+) " +
        "    and tbl.tac_abcenter_pk = plc.pk(+) " +
        "    group by tbl.tr_date, pl.pl_cd, tbl.srn_banve, tbl.srn_trave, tbl.u_price, tbl.invoice_no " + 
...
*/    
 SQL
    = "        select   max (b.ac_lnm) cac_chi_tieu, b.ac_cd tai_khoan, e.pl_cd, e.pl_nm, " + 
        ...
        "         and g.tr_status in ('0', '2', '4')           " +
		"         and a.tr_status = '2'      " +
        "    group by b.ac_cd, e.pl_cd, e.pl_nm " ;
    DataTable dtExpense = new DataTable();        
    dtExpense = ESysLib.TableReadOpen(SQL) ;        
    if(ls_tac_abcenter_pk != "ALL")
    {
        SQL         = "select plc_cd || '-' || plc_nm from tac_abcenter where del_if = 0 and pk = '" + ls_tac_abcenter_pk + "' " ;
        DataTable dtPLC = new DataTable();        
        dtPLC    = ESysLib.TableReadOpen(SQL) ;
        ls_plc_cd    = dtPLC.Rows[0][0].ToString();
    }
    else
        ls_plc_cd = "Tất cả" ;                    
%>
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1431 style='border-collapse:
 ...
 <tr height=24 style='mso-height-source:userset;height:18.0pt'>
  <td colspan=14 height=24 class=xl83 style='height:18.0pt'><%=ls_plc_cd %></td>
 </tr>
 ...
 %>
  ...
  %>
  <% 
  if(ls_pl_cd != dtIncome.Rows[i][1].ToString().Trim())
  {
  %>      
  <td class=xl40><%=dtIncome.Rows[i][1] %></td>
  <% 
        ls_pl_cd = dtIncome.Rows[i][1].ToString() ;
  }
  else
  {
  %>
