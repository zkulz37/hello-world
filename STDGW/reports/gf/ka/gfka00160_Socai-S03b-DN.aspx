...
<%
    ...
    SQL = " SELECT AC_CD,AC_LNM,UPPER(DRCR_TYPE) DRCR_TYPE,  to_char(to_date('" + l_date_from + "','YYYYMMDD'),'DD/MM/YYYY') frm_date, ";
    ...
    SQL += "               NVL (openingbalance, 0) AS OB ,TO_CHAR(NVL(DEBIT_FAMT,0),ac_get_format ('ACBG0040')), TO_CHAR(NVL(CREDIT_FAMT,0),ac_get_format ('ACBG0040')),TO_CHAR(NVL(OPEN_TRANS,0),ac_get_format ('ACBG0040')),TO_CHAR(NVL(CLOSE_TRANS,0),ac_get_format ('ACBG0040')), NVL (OPEN_TRANS, 0) AS O_TRANS";
    SQL += "          FROM ( ";
    SQL += "    SELECT TAC_ABACCTCODE_PK,SUM(openingbalance) openingbalance, SUM(OPEN_TRANS) OPEN_TRANS, " +
        ...
        "    FROM " +
        "    ( " +
        "    SELECT TAC_ABACCTCODE_PK,CCY,0 openingbalance,0 OPEN_TRANS ,SUM (NVL (ymd_drbooks, 0)) debitbookamount, " +
        ...
        "    AND tac_abacctcode_pk = '" + l_account_pk + "' " +
        "    AND tr_status = '" + l_status + "'  " +
        "    AND std_ymd  BETWEEN TO_CHAR(ADD_MONTHS(TO_DATE (ac_get_lastclosemm('" + l_date_from + "', '" + l_company_pk + "' ),'YYYYMMDD'),+1),'YYYYMMDD') " +
...
SQL = "  SELECT  TR_DATE_FMT,voucherno,MAX(remark2) remark2, max(remark) remark, acc_code, SUM(debit_amt),SUM(credit_amt), pk, tr_date  " ;
...
SQL += "       r1.tr_bookamt debit_amt, 0 credit_amt, h.pk, h.tr_date ";
...
SQL += "       0 debit_amt, R2.tr_bookamt credit_amt, h.pk, h.tr_date ";
...
SQL += "     AND h.tr_date BETWEEN TO_DATE ('" + l_date_from +"', 'YYYYMMDD') AND TO_DATE ('" + l_date_to +"', 'YYYYMMDD') ";
SQL += "     AND h.tco_company_pk like  DECODE ('" + l_company_pk + "', 0, '%%', '" + l_company_pk + "'))";
SQL += "   group by tr_date, pk, voucherno,acc_code,tr_date_fmt ";
SQL += "     ORDER BY tr_date,voucherno, PK";
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1217 style='border-collapse:
 ...
 <tr height=19 style='mso-height-source:userset;height:14.25pt'>
  ...
  <td class=xl33 colspan=2 width=246 style='mso-ignore:colspan;width:184pt'><span
  style='mso-spacerun:yes'> </span>M&#7851;u s&#7889; S03b-DN</td>
  <td class=xl33 width=64 style='width:48pt'></td>
 </tr>
 <tr height=19 style='mso-height-source:userset;height:14.25pt'>
  <td colspan=4 height=19 class=xl69 width=552 style='height:14.25pt;
  width:415pt'><%= p_cmp_add %></td>
  <td class=xl78 colspan=4 x:str="(Ban hành theo Quy&#7871;t &#273;&#7883;nh s&#7889; 15/2006/Q&#272;-BTC ngày ">(Ban
 ...
 <tr height=21 style='height:15.75pt'>
  ...
  <td class=xl78 colspan=4 >20 tháng 03
 ...
 <tr class=xl31 height=26 style='mso-height-source:userset;height:19.5pt'>
  ...
  hi&#7879;u tài kho&#7843;n:</td>
  <td class=xl33 x:num><%= p_acc_code %></td>
 ...
 <tr class=xl33 height=21 style='height:15.75pt'>
  ...
  <td class=xl58 id="_x0000_s1032" style='border-right:.5pt solid windowtext;' x:autofilter="all">Có</td>
 ...
 <tr class=xl31 >
  ...
  <td class=xl44 x:num><%=dt_detail.Rows[i][4].ToString()%></td>
  <td class=xl45 style="mso-number-format:'<%= p_xls_ccy_format %>';" x:num><%= dt_detail.Rows[i][5].ToString() %></td>
...
{ %>
 <tr class=xl31>
  ...
  <td class=xl28 style='border-top:none;border-left:none' style='white-space:nowrap;mso-text-control:shrinktofit;'><%=dt_detail.Rows[i][0].ToString()%></td>
  ...
  <td class=xl44 x:num><%=dt_detail.Rows[i][4].ToString()%></td>
  <td class=xl45  style="mso-number-format:'<%= p_xls_ccy_format %>';" x:num><%= dt_detail.Rows[i][5].ToString() %></td>
  <td class=xl65 style='border-top:none;border-left:none'  style="mso-number-format:'<%= p_xls_ccy_format %>';" x:num><%= dt_detail.Rows[i][6].ToString() %></td>
 ...
 <tr class=xl31 height=21 style='height:15.75pt'>
  ...
  <td colspan=3 class=xl71>Ngày ........ tháng ........ n&#259;m ...........</td>
  <td class=xl31></td>
 </tr>
<%
    SQL = "SELECT   " +
