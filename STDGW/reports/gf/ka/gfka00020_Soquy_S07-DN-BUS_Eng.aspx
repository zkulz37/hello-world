...
SQL += "         DECODE (d.drcr_type,'D', TO_CHAR (d.tr_bookamt, ac_SF_GET_FORMAT ('ACBG0040')), 0) book_amt_debit, ";
SQL += "         DECODE (d.drcr_type,'C', TO_CHAR (d.tr_bookamt, ac_SF_GET_FORMAT ('ACBG0040')), 0) book_amt_credit, ";
SQL += "         TO_CHAR(TO_NUMBER (" + p_open_bal +") + SUM (  NVL (d.tr_bookamt, 0) * DECODE (drcr_type, 'D', 1, -1) * b.dc) OVER (ORDER BY h.tr_date, d.pk), ac_SF_GET_FORMAT ('ACBG0040')) AS closingbalance, ";
SQL += "         h.pk, h.tr_date ";
...
SQL = "select TO_CHAR (tr_date, 'DD/MM/YYYY'), " + 
        ...
        " remark2, remark, " +
        " TO_CHAR ( book_amt_debit, ac_SF_GET_FORMAT ('ACBG0040')) book_amt_debit, " +
        " TO_CHAR (book_amt_credit, ac_SF_GET_FORMAT ('ACBG0040')) book_amt_credit, " +
        " TO_CHAR(TO_NUMBER (" + p_open_bal + ") + SUM(book_amt_debit*dc - book_amt_credit*dc) OVER (ORDER BY tr_date,order_cond, voucherno ,pk),ac_SF_GET_FORMAT ('ACBG0040')) AS closingbalance, " +
        "         pk, tr_date, PARTNER_ID, PARTNER_NAME " +
        " FROM          " +
        " (     SELECT    " +
        ...
        "         max(b.dc) dc, " +
        "         h.pk, h.tr_date, " +
        "         DECODE(DECODE(max(b.dc),1,'D','C') ,d.drcr_type,1,2)  order_cond, max(c.PARTNER_ID) PARTNER_ID, max(c.PARTNER_NAME) PARTNER_NAME" +
        "    FROM (SELECT DECODE (drcr_type, 'D', 1, -1) dc " +
        "            FROM tac_abacctcode " +
        "           WHERE pk = '" + l_account_pk +"') b, " +
        "         tac_hgtrh h, " +
        "         tac_hgtrd d, TCO_BUSPARTNER c  " +
        "   WHERE h.del_if = 0 " +
        "     AND d.del_if = 0 and c.del_if(+) = 0 and h.TCO_BUSPARTNER_pk = c.pk(+) " +
...
else if (l_rpt_Option == "FIFO")
{
    SQL = "select TO_CHAR (tr_date, 'DD/MM/YYYY'), " +
        ...
        " TO_CHAR ( book_amt_debit, ac_SF_GET_FORMAT ('ACBG0040')) book_amt_debit, " +
        " TO_CHAR (book_amt_credit, ac_SF_GET_FORMAT ('ACBG0040')) book_amt_credit, " +
        " TO_CHAR(TO_NUMBER (" + p_open_bal + ") + SUM(book_amt_debit*dc - book_amt_credit*dc) OVER (ORDER BY tr_date ,pk,voucherno),ac_SF_GET_FORMAT ('ACBG0040')) AS closingbalance, " +
        "         pk, tr_date " +
        " FROM          " +
        " (     SELECT    " +
        "         d.drcr_type drcr_type, h.voucherno voucherno, " +
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1247 style='border-collapse: collapse;table-layout:fixed;width:937pt'>
 <col class=xl37 width=89 span=2 style='mso-width-source:userset;mso-width-alt: 3254;width:67pt'>
 <col class=xl37 width=103 span=2 style='mso-width-source:userset;mso-width-alt: 3766;width:77pt'>
 <col class=xl37 width=236 span=2 style='mso-width-source:userset;mso-width-alt: 8630;width:177pt'>
 <col class=xl38 width=110 span=3 style='mso-width-source:userset;mso-width-alt: 4022;width:83pt'>
 <col class=xl37 width=61 style='mso-width-source:userset;mso-width-alt:2230; width:46pt'>
 <col class=xl38 width=110 style='mso-width-source:userset;mso-width-alt: 4022;width:83pt'>
 <col class=xl37 width=236  style='mso-width-source:userset;mso-width-alt: 8630;width:177pt'>
 ...
 <tr height=30 style='height:22.5pt'>
  <td colspan=12 height=30 class=xl76 style='height:22.5pt'>CASH BOOK</td>
  <td class=xl39></td>
 </tr>
 <tr height=26 style='mso-height-source:userset;height:19.5pt'>
  <td colspan=12 height=26 class=xl77 style='height:19.5pt'>From date <%= p_dt_from %> to date <%= p_dt_to%></td>
  <td class=xl40></td>
 </tr>
 ...
 <tr class=xl45 height=21 style='height:15.75pt'>
  ...
  <td colspan=2 class=xl81 width=236 style='border-bottom:.5pt solid black;  width:177pt'>Customer</td>
 </tr>
 <tr class=xl45 height=21 style='height:15.75pt'>
  ...
  style='mso-spacerun:yes'> </span></td>
  <td class=xl49>Code</td>
  <td class=xl49>Name</td>
 </tr>
 <tr class=xl45 height=21 style='height:15.75pt'>
 ...
 %>
 <tr class=xl40 style='mso-height-source:auto'>
  ...
  <td class=xl53 style='border-left:none;white-space:normal' ><%=dt_detail.Rows[i][10].ToString()%></td>
  <td class=xl53 style='border-left:none;white-space:normal' ><%=dt_detail.Rows[i][11].ToString()%></td>
 </tr>
 <%
	}
		else
 ...
 <tr class=xl40 style='mso-height-source:auto>
  ...
  <td class=xl53 style='border-top:none;border-left:none;white-space:normal' ><%=dt_detail.Rows[i][10].ToString()%></td>
  <td class=xl53 style='border-top:none;border-left:none;white-space:normal' ><%=dt_detail.Rows[i][11].ToString()%></td>
 </tr>
 <%
	}
	}
