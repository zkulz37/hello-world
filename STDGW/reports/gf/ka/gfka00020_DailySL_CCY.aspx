...
<%
  ...
  DataTable dt_bookccy = ESysLib.TableReadOpen(SQL);
    ...
    SQL = "SELECT CCY,TO_CHAR (round(SUM(OPEN_BAL),2),AC_GET_FORMAT ('ACBG0040')) openingbalance,   " +
        "            TO_CHAR (round(SUM(DR_TRANS),2),'9,999,999,999,999,999,990.99') AS debittransamount, " +
        "            TO_CHAR (round(SUM(DR_BOOK),2), AC_GET_FORMAT ('ACBG0040')) AS debitbookamount,  " +
        "            TO_CHAR (round(SUM(CR_TRANS),2), '9,999,999,999,999,999,990.99') AS credittransamount, " +
        "            TO_CHAR (round(SUM(CR_BOOK),2), AC_GET_FORMAT ('ACBG0040')) AS creditbookamount, " +
        "            TO_CHAR (round((SUM(OPEN_BAL) +(SUM(DR_BOOK) - SUM(CR_BOOK))*TO_NUMBER('" + p_acc_type + "')),2),AC_GET_FORMAT ('ACBG0040')) AS closingbalance, " +
        "            round((SUM(OPEN_BAL) +(SUM(DR_BOOK) - SUM(CR_BOOK))*TO_NUMBER('" + p_acc_type + "')))  AS OB, " +
        "            TO_CHAR (round(SUM(OPEN_FBAL),2), '9,999,999,999,999,999,990.99') AS openingtransbalance, " +
        "            TO_CHAR (round((SUM(OPEN_FBAL) +(SUM(DR_TRANS) - SUM(CR_TRANS))*TO_NUMBER('" + p_acc_type + "')),2), '9,999,999,999,999,999,990.99') AS closingtransbalance" +
        "          FROM " +
        "          ( " +
...
SQL += ",DECODE(D.DRCR_TYPE,'D',round(D.TR_AMT),0)  ";
SQL += ",DECODE(D.DRCR_TYPE,'D',round(D.TR_BOOKAMT),0) ";
SQL += ",DECODE(D.DRCR_TYPE,'C',round(D.TR_AMT),0)  ";
SQL += ",DECODE(D.DRCR_TYPE,'C',round(D.TR_BOOKAMT),0) ";
SQL += ", TO_CHAR(TO_NUMBER ('" + p_open_bal + "','9,999,999,999,999,990.99') + SUM (  NVL (d.tr_bookamt, 0) * DECODE (d.drcr_type, 'D', 1, -1) *to_number('" + p_acc_type + "')) OVER (ORDER BY h.tr_date,DECODE(d.drcr_type,'" + p_drcr + "',1,2), h.VOUCHERNO, h.PK,d.pk), AC_GET_FORMAT ('ACBG0040')) AS closingbalance ";
SQL += ",D.BUSPARTNER_LNM ";
SQL += ",D.REMARK ";
SQL += ",D.REMARK2 ";
SQL += ", DECODE(D.TAC_ABPLCENTER_PK,'','',ac_get_PL_FROM_PLCENTER(D.TAC_ABPLCENTER_PK)) ";
SQL += ",ac_get_AGTRITEM(D.PK),  h.TR_ENCLOSE  ";
...
SQL += "     ORDER BY h.tr_date,DECODE(d.drcr_type,'" + p_drcr + "',1,2), h.VOUCHERNO, h.PK,d.pk";
    ...
    else if (l_rpt_Option == "FIFO")
    {
        ...
        SQL += ",DECODE(D.DRCR_TYPE,'C',round(D.TR_AMT),0)  ";
        SQL += ",DECODE(D.DRCR_TYPE,'C',round(D.TR_BOOKAMT),0) ";
        SQL += ", TO_CHAR(TO_NUMBER ('" + p_open_bal + "','9,999,999,999,999,990.99') + SUM (  NVL (d.tr_bookamt, 0) * DECODE (d.drcr_type, 'D', 1, -1) *to_number('" + p_acc_type + "')) OVER (ORDER BY h.tr_date,  h.PK,d.pk), AC_GET_FORMAT ('ACBG0040')) AS closingbalance ";
        SQL += ",D.BUSPARTNER_LNM ";
        SQL += ",D.REMARK ";
        SQL += ",D.REMARK2 ";
        SQL += ", DECODE(D.TAC_ABPLCENTER_PK,'','',ac_get_PL_FROM_PLCENTER(D.TAC_ABPLCENTER_PK)) ";
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1967 style='border-collapse:
 ...
 <tr height=26 style='mso-height-source:userset;height:20.1pt'>
  ...
  <td class=xl33><%= dt_detail.Rows[i][4].ToString()%></td>
   <% if( i ==0) { %>
  <td class=xl35  x:num  style="mso-number-format:'<%= p_xls_ccy_format%>';"><%=dt_detail.Rows[i][17].ToString() %></td>
  <td class=xl35  x:num  style="mso-number-format:'<%= p_xls_ccy_format %>';"><%=dt_detail.Rows[i][5].ToString() %></td>
  <% } else { %>
  <td class=xl35  x:num  style="mso-number-format:'<%= p_xls_ccy_format%>';"><%= dt_detail.Rows[i-1][18].ToString()%></td>
  ...
  <td class=xl35 x:num style="mso-number-format:'<%= p_xls_ccy_format%>';"><%= dt_detail.Rows[i][6].ToString()%></td>
  <td class=xl35 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';"><%= dt_detail.Rows[i][7].ToString()%></td>
  <td class=xl35 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';"><%= dt_detail.Rows[i][8].ToString()%></td>
  ...
  %>
  <td class=xl35 x:num style="mso-number-format:'<%= p_xls_ccy_format%>';"><%= dt_detail.Rows[i][18].ToString()%></td>
...
%>   
 <tr height=26 style='mso-height-source:userset;height:20.1pt'>
  ...
  <td class=xl35  x:num style="mso-number-format:'<%= p_xls_ccy_format%>';"><%= dt_total.Rows[j][8].ToString()   %></td>
  ...
  <td class=xl43 x:num style="mso-number-format:'<%= p_xls_ccy_format %>';" ><%= dt_total.Rows[j][5].ToString()   %></td>
   <% } %>  
  <td class=xl35  x:num style="mso-number-format:'<%= p_xls_ccy_format%>';"><%= dt_total.Rows[j][9].ToString()   %></td>
