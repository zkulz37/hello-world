...
SQL = SQL + "    AND ((TRIM('" + l_Seq +"')  IS NULL) OR (H.pk = TRIM('" + l_Seq +"') ))  ";
    ...
    SQL = " SELECT  a.pk,  ";
    SQL = SQL + "        TO_CHAR (a.tr_date, SF_GET_FORMAT ('ACAB0300')) TRDATE, a.voucherno, d.ac_cd, ";
    SQL = SQL + "        DECODE ('" + l_lang + "', 'ENG', d.ac_nm, 'VIE', d.ac_lnm, d.ac_knm) ac_nm, ";
    ...
    SQL = SQL + "        SF_A_GET_AGTRITEM(b.pk), TO_CHAR (a.tr_enclose, 'YYYYMMDD') ENCLOSE ";
    SQL = SQL + "     FROM tac_hgtrh a, tac_hgtrd b, tac_abacctcode c, tac_localcode d ";
    SQL = SQL + "         WHERE a.pk = b.tac_hgtrh_pk ";
    SQL = SQL + "           AND b.tac_abacctcode_pk = c.pk ";
    SQL = SQL + "           and c.TAC_LOCALCODE_PK = d.pk ";
    ...
    SQL = SQL + "                  TRIM(TO_CHAR(DECODE('VND','VND',OP*1,OP/1),DECODE('VND','VND',Sf_Get_Format('ACBG0040'),'9,999,999,999,990.99'))) CR_OP,";
    SQL = SQL + "                  TRIM(TO_CHAR(DECODE('VND','VND',DR_PS*1,DR_PS/1),DECODE('VND','VND',Sf_Get_Format('ACBG0040'),'9,999,999,999,990.99'))) DR_PS,";
    SQL = SQL + "                  TRIM(TO_CHAR(DECODE('VND','VND',CR_PS*1,CR_PS/1),DECODE('VND','VND',Sf_Get_Format('ACBG0040'),'9,999,999,999,990.99'))) CR_PS,";
    SQL = SQL + "                  TRIM(TO_CHAR(DECODE('VND','VND',BL*1,BL/1),DECODE('VND','VND',Sf_Get_Format('ACBG0040'),'9,999,999,999,990.99'))) DR_BL,";
    ...
    SQL = SQL + "                          (SELECT A.PK, c.AC_CD, DECODE(c.DRCR_TYPE,'D',1,-1) DC, B.FONT_STYPE BOLD_YN, B.PRN_COLOR COLOR, ";
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1025 style='border-collapse:
 ...
 <tr height=24 style='mso-height-source:userset;height:18.0pt'>
  ...
  colspan;width:196pt'>Nhật ký chung</td>
 ...
 <tr height=18 style='mso-height-source:userset;height:14.1pt'>
  <td colspan=4 height=18 class=xl30 style='height:14.1pt'>Đến ngày <%= l_str_today %></td>
  <td colspan=7 class=xl30 style='mso-ignore:colspan'></td>
 </tr>
 <tr height=24 style='mso-height-source:userset;height:18.0pt'>
  <td height=24 class=xl27 colspan=2 style='height:18.0pt;mso-ignore:colspan'>Công ty:</td>
  <td colspan=3 class=xl58><%= l_cmp_name %></td>
  <td class=xl41 width=53 style='width:40pt'>Kỳ:</td>
  <td colspan=2 class=xl59><%= l_date_fr2 %> ~ <%= l_date_to2 %></td>
  <td class=xl42 >Trạng thái:</td>
 ...
 <tr height=19 style='height:14.25pt'>
  ...
  <td class=xl27>Xem theo tài khoản</td>
  <td colspan=3 class=xl24 style='mso-ignore:colspan'></td>
  <td class=xl25></td>
  <td class=xl26></td>
 ...
 <tr height=42 style='mso-height-source:userset;height:31.5pt'>
  ...
  height:31.5pt;width:93pt'>Mã tài khoản</td>
  <td colspan=2 class=xl48 style='border-right:.5pt solid black;border-left:
  none'>Tên tài khoản</td>
  <td class=xl48 style='border-left:none'>Chi tiết (thao khảo)</td>
  <td colspan=2 class=xl48 style='border-right:.5pt solid black'>Số dư đầu kỳ</td>
  <td class=xl48 style='border-left:none'>Nợ</td>
  <td class=xl49>Có</td>
  <td class=xl45>Cuối kỳ</td>
  <td class=xl30></td>
 </tr>
 <%
 ...
 <tr class=xl30 height=40 style='mso-height-source:userset;height:30.0pt'>
  ...
  <td class=xl46>Số chứng từ</td>
  <td class=xl46>Ngày</td>
  <td class=xl47 width=72 style='width:54pt'>Tài khoản</td>
  <td class=xl46>Diễn giải</td>
  <td colspan=2 class=xl48 style='border-right:.5pt solid black;border-left:
  none'>Diễn giải</td>
  <td class=xl46>Nợ </td>
  <td class=xl46>Có</td>
  <td class=xl46>Chi tiết(tham khảo)</td>
  <td class=xl46 style="border-right:1;">Đính kèm</td>
 </tr>
<%
