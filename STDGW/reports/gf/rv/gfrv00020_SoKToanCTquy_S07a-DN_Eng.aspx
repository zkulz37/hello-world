...
SQL = " SELECT pk FROM tac_localcode  WHERE del_if = 0 AND leaf_yn = 'Y'  CONNECT BY pac_pk = PRIOR pk START WITH pk = " + l_account_pk;
	 ...
	 if (l_rpt_Option == "DRCR")
    {
        ...
        SQL += "TO_CHAR((TO_NUMBER ('" + p_open_bal_xls + "','9,999,999,999,999,999,990.99') + SUM ( NVL (debit_amt, 0) - NVL (credit_amt, 0)) OVER (ORDER BY tr_date,order_cond,voucherno_debit,voucherno_credit, pk,r_pk)),sf_get_format ('ACBG0040')) AS closingbalance, ";
...
SQL += "                                 START WITH pk = " + p_acc_pk_arr + "))) ";
        ...
        SQL += "     ORDER BY tr_date,order_cond,voucherno_debit,voucherno_credit, PK,r_pk";
         ...
         {
             SQL = "  SELECT  TR_DATE_FMT,voucherno_debit,voucherno_credit,REMARK2,REMARK,ACC_CODE,debit_amt,credit_amt,";
             SQL += "TO_CHAR((TO_NUMBER ('" + p_open_bal_xls + "','9,999,999,999,999,999,990.99') + SUM ( NVL (debit_amt, 0) - NVL (credit_amt, 0)) OVER (ORDER BY tr_date, pk,r_pk)),sf_get_format ('ACBG0040')) AS closingbalance, ";
             //SQL += " (0 + sum(nvl(debit_amt,0) - nvl(credit_amt,0)) OVER (ORDER BY tr_date, pk,r_pk)) AS closingbalance,";
             SQL += " pk, tr_date  ";
             SQL += " FROM (  SELECT TO_CHAR (h.tr_date, 'DD/MM/YYYY') TR_DATE_FMT, ";
             SQL += "       h.voucherno voucherno_debit, '' voucherno_credit, r1.remark2 REMARK2, ";
