...
<%
    ...
    = "select (select v.code_nm from hr.vhr_hr_code v where v.id='HR0092' and v.code=a.GRP_COST) as grp_cost " +
        ...
        "        ,b.PL_NM||' - '||d.plc_nm,count(a.pk),round(sum(nvl(a.NET_TRAMT,0))), TO_CHAR(TO_DATE(A.STD_YM,'YYYYMM'),'MM/YYYY') " +
        "        from TAC_PMSAL_EMP a,TAC_ABPL b, TAC_ABPLCENTER c,TAC_ABCENTER d " +
        "        where a.del_if=0 and b.del_if=0 and c.del_if=0 and d.del_if=0 " +
        "        and c.TAC_ABPL_PK=b.pk and c.TAC_ABCENTER_PK=d.pk " +
        ...
        "        group by a.GRP_COST,a.TYPE_COST,b.PL_NM||' - '||d.plc_nm,a.TAC_ABPL_PK,a.std_ym " +
        "        order by a.GRP_COST,a.TYPE_COST,a.TAC_ABPL_PK ";
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1173 style='border-collapse:
 ...
 <tr height=39 style='mso-height-source:userset;height:29.25pt'>
  <td colspan=5 height=39 class=xl33 style='height:29.25pt'>MONTH:<%=dt_total.Rows[0][5].ToString()%></td>
  <td class=xl24></td>
 </tr>
 <tr height=39 style='mso-height-source:userset;height:29.25pt'>
 ...
 <tr height=78 style='mso-height-source:userset;height:58.5pt'>
  ...
  <td class=xl27>Emp(s)</td>
  <td class=xl40>Transaction Amount</td>
 </tr>
 <% 
    ...
    {
        Total[0] += Double.Parse(dt_total.Rows[i][3].ToString());
        Total[1] += Double.Parse(dt_total.Rows[i][4].ToString());
 ...
 <tr class=xl38 height=38 style='mso-height-source:userset;height:28.5pt'>
  ...
  <td class=xl39 x:num><%=dt_total.Rows[i][3].ToString()%></td>
  <td class=xl41 x:num><%=dt_total.Rows[i][4].ToString()%></td>
 </tr>
 <%
    }
  %>
 <tr height=38 style='mso-height-source:userset;height:28.5pt'>
  <td colspan=4 height=38 class=xl35 style='border-right:.5pt solid black;
