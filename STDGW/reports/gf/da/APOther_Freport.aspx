...
<%
	...
	string sSQL = "";
 sSQL
= "select (select w.wh_name from inv.tin_warehouse w where rownum = 1 and w.pk in (select d.tin_warehouse_pk   " + 
        "    from tac_crdad_other d where d.del_if = 0 and d.tac_crda_other_pk = '" + pk + "') ) wh_name,   " +
        "    (select h.full_name from hr.thr_employee h where h.pk = u.thr_abemp_pk) full_nm,   " +
        "    (select p.pl_nm from v_abplcenter p where rownum = 1 and p.tac_abplcenter_pk in (select d.tac_abplcenter_pk  " +
        "    from tac_crdad_other d where d.del_if =0 and d.tac_crda_other_pk = '" + pk + "' )) pl_nm,   " +
        "    c.partner_id || ':' || c.partner_lname as cust_name,  " +
        "       a.serial_no || '  ' || a.invoice_no || '   ' || a.co_invoice_date,  " +
        "       null contract_no   " +
        "       ,to_char (a.ap_trfamt, '999,999,999,999,999,999,990')   " +
        "       ,to_char (a.ap_tramt, '999,999,999,999,999,999,990.99')   " +
        "       ,'0'    " +
        "       ,'0'   " +
        "       ,to_char (nvl(a.ap_trfamt,0), '999,999,999,999,990')   " +
        "       ,to_char (nvl(a.ap_tramt,0), '999,999,999,999,990.99')   " +
        "       ,a.voucherno   " +
        "       ,a.tr_date   " +
        "       ,0 exp_famt  " +
        "       ,0 exp_amt   " +
 ...
 sSQL1
    = " SELECT b.item_code || '    :   ' ||b.item_name, a.ap_unit, to_char (a.ap_qty, '999,999,999,999,990.99'),    " +
        " to_char (a.ap_uprice, '999,999,999,999,990'),  " +
        "   to_char ( a.ap_trfamt, '999,999,999,999,990')  ,  to_char ( a.ap_tramt, '999,999,999,999,990.99'), a.po_no " +
        "  from tac_crdad_other a, comm.tco_item b  " +
        " where a.del_if = 0 and a.tco_item_pk=b.pk and a.tac_crda_other_pk = '" + pk + "'  " +
...
string SQL = "select c.partner_id || ':' || c.partner_lname cust_name from comm.tco_company c where c.pk = '" + company_pk + "' and c.del_if = 0";
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=746 style='border-collapse:
 ...
 <tr height=21 style='mso-height-source:userset;height:15.75pt'>
  ...
  width:318pt'><%=dtComp.Rows[0][0].ToString()%></td>
 ...
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  <td class=xl90 align=left style='border-top:none;border-left:none'><%=rs1.Rows[i][6]%>&nbsp;</td>
 </tr>
  <%
 }    
	for(i=0; i < dem; i++)
