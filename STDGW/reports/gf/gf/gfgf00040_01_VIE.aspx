...
<%
	...
	else
	{
     ...
     string SQLD = "SELECT a.fa_cd, a.fa_nm, a.fa_kind_cd,a.fa_qty, " +
                 ...
                 "       DECODE('"+p_ccy+"','VND', round(FA_COST,0), round(FA_COST,2)) FA_COST, DECODE('"+p_ccy+"','VND', round(fa_depr_amt,0), round(fa_depr_amt,2)) fa_deprmonth_amt ,  DECODE('"+p_ccy+"','VND', round(NVL (b.fd_damt, 0), 0),round(NVL (b.fd_damt, 0), 2))  fa_depr_amt, " +
                 ...
                 "       DECODE ('"+p_ccy+"','VND', round(a.fa_cost - NVL (a.fa_begin_depr_amt, 0) - NVL (b.fd_begin, 0) - NVL (b.fd_damt, 0),0),round(a.fa_cost - NVL (a.fa_begin_depr_amt, 0) - NVL (b.fd_begin, 0) - NVL (b.fd_damt, 0),2)) fa_remain_amt," +
                 ...
                 "       , DECODE ('"+p_ccy+"','VND', round(NVL (a.fa_cost, 1) / DECODE (NVL (a.fa_qty, 1), 0, 1, NVL (a.fa_qty, 1)),0),round(NVL (a.fa_cost, 1) / DECODE (NVL (a.fa_qty, 1), 0, 1, NVL (a.fa_qty, 1)),2)) u_price, a.remark, a.remark2,c.org_nm,c.pk dept_pk,  " +
                 "       sf_a_get_accd (a.TAC_ABACCTCODE_PK,a.tco_company_pk) asset_acc, c.seq,g.ord " +
                 "FROM tac_gffa_mst a, " +
                 ...
                 "         , tco_org c ,tco_abcode g, tco_abcodegrp h " +
                 "      WHERE a.del_if = 0 and g.del_if = 0 and h.del_if = 0  " +
                 "        AND b.tac_gffa_mst_pk(+) = a.pk" +
                 "        and c.del_if (+)= 0 and a.tco_org_pk = c.pk(+)  and g.tco_abcodegrp_pk = h.pk and h.id = 'ACDF0010' and g.use_if = 1 and a.fa_kind_cd=g.code" +
   ...
   string SQLK = "SELECT fa_kind_cd,upper(SF_GET_COMMCODE_NM('ACDF0010',fa_kind_cd)) fa_kind_NM, sum(fa_cost) fa_cost, sum(fa_deprmonth_amt) fa_deprmonth_amt, " +
                  "       sum(fa_depr_amt) fa_depr_amt, sum(fa_depr_accum) fa_depr_accum, sum(fa_remain_amt) fa_remain_amt,ord" +
                  " FROM ( " + SQLD + " )" +
                  " GROUP BY fa_kind_cd,ord ORDER BY ord,fa_kind_cd";
    DataTable dt_kind = ESysLib.TableReadOpen(SQLK);
	
	
    string SQLK_D = "SELECT fa_kind_cd ,dept_pk,SF_GET_COMMCODE_NM('ACDF0010',fa_kind_cd) fa_kind_NM, INITCAP(org_nm) dept_nm, sum(fa_cost) fa_cost, sum(fa_deprmonth_amt) fa_deprmonth_amt, " +
    ...
    string SQLT = "SELECT sum(fa_cost) fa_cost, sum(fa_deprmonth_amt) fa_deprmonth_amt, " +
                  "       sum(fa_depr_amt) fa_depr_amt, sum(fa_depr_accum) fa_depr_accum, sum(fa_remain_amt) fa_remain_amt" +
                  " FROM ( " + SQLD + " )" ;
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1176 style='border-collapse:
 ...
 <tr height=27 style='mso-height-source:userset;height:20.25pt'>
  ...
  <td colspan=10 class=xl38><% if(p_asset_type == "T"){%>DANH SÁCH PHÂN BỔ CÔNG CỤ DỤNG CỤ<%}else{%>DANH SÁCH KHẤU HAO TÀI SẢN CỐ ĐỊNH<%}%></td>
  <td colspan=2 style='mso-ignore:colspan'></td>
 </tr>
 <tr height=17 style='height:12.75pt'>
  <td height=17 style='height:12.75pt'></td>
  <td colspan=10 class=xl24>Từ ngày :<%=from_month%><span style='mso-spacerun:yes'>           
  </span>Đên :<%=to_month%></td>
  <td colspan=2 style='mso-ignore:colspan'></td>
 </tr>
 ...
 <tr class=xl25 height=40 style='mso-height-source:userset;height:30.0pt'>
  <td height=40 class=xl26 width=32 style='height:30.0pt;width:24pt'>STT</td>
  <td class=xl27 width=79 style='width:59pt'><% if(p_asset_type == "T"){%>Mã CCDC<%}else{%>Mã TSCĐ<%}%></td>
  <td class=xl27 width=238 style='width:179pt'><% if(p_asset_type == "T"){%>Tên CCDC<%}else{%>Tên TSCĐ<%}%></td>
  <td class=xl27 width=42 style='width:32pt'>Số lượng</td>
  <td class=xl27 width=83 style='width:62pt'>Ngày <% if(p_asset_type == "T"){%>phân bổ<%}else{%>khấu hao<%}%></td>
  <td class=xl27 width=74 style='width:56pt'>Tuổi thọ(tháng)</td>
  <td class=xl27 width=102 style='width:77pt'>Nguyên giá</td>
  <td class=xl27 width=93 style='width:70pt'>Số <% if(p_asset_type == "T"){%>phân bổ<%}else{%>khấu hao<%}%>(mỗi tháng)</td>
  <td class=xl27 width=90 style='width:68pt'>Số <% if(p_asset_type == "T"){%>phân bổ<%}else{%>khấu hao<%}%>(của tháng này)</td>
  <td class=xl27 width=95 style='width:71pt'>Số <% if(p_asset_type == "T"){%>phân bổ<%}else{%>khấu hao<%}%> lũy kế</td>
  <td class=xl27 width=93 style='width:70pt'>Giá trị còn lại</td>
  <td class=xl27 width=76 style='width:57pt'>Tài khoản tài sản</td>
  <td class=xl27 width=76 style='width:57pt'>Tài khoản <% if(p_asset_type == "T"){%>phân bổ<%}else{%>khấu hao<%}%></td>
  <td class=xl27 width=79 style='width:59pt'>Tài khoản chi phí</td>
  <td class=xl27 width=79 style='width:59pt'>Đơn giá</td>
  <td class=xl27 width=79 style='width:59pt'>Diễn giải</td>
  <td class=xl27 width=79 style='width:59pt'>Diễn giải </td>
 </tr>
 <%
...
<tr class=xl28 height=17 style='mso-height-source:userset;height:12.75pt'>
     ...
     if (data.Rows.Count > 0)
     {
         ...
         for (j = 0; j < data.Rows.Count; j++)
         {
             if (data.Rows[j]["fa_kind_cd"].ToString() == kind_code && data.Rows[j]["dept_pk"].ToString() ==  kind_code_dept)
             {
				 ...
				 if (tmp_date.Length == 8){depr_date = tmp_date.Substring(6, 2) + "/" + tmp_date.Substring(4, 2) + "/" + tmp_date.Substring(0, 4);}
 ...
 <tr class=xl32 height=16 style='height:12.0pt'>
  ...
  <td class=xl46 style="border-top:none;border-left:none;text-align:right"><%=month_depr%></td>
  <td class=xl46 style="border-top:none;border-left:none;mso-number-format:'<%= p_xls_ccy_format %>'" x:num><%=data.Rows[j]["fa_cost"].ToString()%></td>
 ...
 <tr height=17 style='height:12.75pt'>
  ...
  <td colspan=3 class=xl44>Ngày ...…..Tháng...…….Năm.....</td>
  <td></td>
 </tr>
 <tr height=17 style='height:12.75pt'>
