...
<%
    ...
    string l_tim_contr_mst_pk       =Request["p_tim_contr_mst_pk"];
    ...
    " FROM TCO_COMPANY WHERE PK = '" + l_tco_company_pk + "' ";
     ...
     string sql_count="SELECT COUNT(*) FROM IMEX.TIM_CONTR_DTL D WHERE D.DEL_IF=0 AND D.TIM_CONTR_MST_PK='" + l_tim_contr_mst_pk + "'";
     ...
     if (dt_c.Rows.Count >1)
     {
         sql_master = "select m.CONTR_NO, p.PARTNER_ID,m.B_CCY, p.PARTNER_NAME, pl.plc_cd,pl.plc_nm,M.PERIOD,max(nvl(m.VAT_AMT,0)),max(nvl(m.ADJ_VAT_AMT,0)) " +
                    ...
                    "sum(nvl(d.TR_AMT,0)),sum(nvl(d.RETOUCH_AMT,0)),sum(nvl(d.TR_AMT,0))+sum(nvl(d.RETOUCH_AMT,0)) amt_sum,max(nvl(m.AP_TR_AMT,0)) " +
                    "from imex.tim_contr_mst m, imex.tim_contr_dtl d, comm.TCO_BUSPARTNER p " +
                    ",(select c.pk, a.PLC_CD, a.PLC_NM " +
                    "    from acnt.tac_abcenter a, acnt.tac_abpl b, acnt.tac_abplcenter c " +
     ...
     else 
     {
         sql_master = "select m.CONTR_NO, p.PARTNER_ID,m.B_CCY, p.PARTNER_NAME, pl.plc_cd,pl.plc_nm,M.PERIOD,max(nvl(m.VAT_AMT,0)),max(nvl(m.ADJ_VAT_AMT,0))" + 
                    ...
                    "sum(nvl(d.TR_AMT,0)),sum(nvl(d.RETOUCH_AMT,0)),sum(nvl(d.TR_AMT,0))+sum(nvl(d.RETOUCH_AMT,0)) amt,max(nvl(m.AP_TR_AMT,0)) " +
                    "from imex.tim_contr_mst m, imex.tim_contr_dtl d, comm.TCO_BUSPARTNER p " +
                    ",(select c.pk, a.PLC_CD, a.PLC_NM " +
                    "    from acnt.tac_abcenter a, acnt.tac_abpl b, acnt.tac_abplcenter c " +
                    "    where a.del_if=0 and b.DEL_IF=0 and c.DEL_IF=0 " +
    ...
    //---------------------------------------------------------------------------------
     sql_cond = "select c.code_nm, A.pay_percent/100, A.pay_amt,b.period " + 
                "from imex.tim_payment_condition A " +
                "    ,imex.tim_contr_mst B " +
                ...
                "        and a.TIM_CONTR_MST_PK=b.PK " +
                "      and tim_contr_mst_pk='" + l_tim_contr_mst_pk + "' ";
    ...
    sql_adv = "SELECT B.PK,to_char(B.TR_DATE,'dd/mm/yyyy'),E.CONTR_NO,C.DRCR_ORD, C.TR_AMT  " +
                 ...
                 "    AND E.PK='" + l_tim_contr_mst_pk + "' ";

    //Response.Write(sql_adv);
    //Response.End(); 
    ...
    //-------------------------------------------------------------------------------------------
     sql_AP = "SELECT B.PK,to_char(B.TR_DATE,'dd/mm/yyyy'),E.CONTR_NO,B.REMARK, C.TR_AMT  " + 
                ...
                ",TAC_ABACCTCODE D,IMEX.TIM_CONTR_MST E " +
                "WHERE  " +
                "    A.DEL_IF=0 AND B.DEL_IF=0 AND C.DEL_IF=0  " +
                "    AND d.DEL_IF=0 AND E.DEL_IF=0 " +
                "    AND a.TABLE_NM='TIM_CONTR_MST' " +
                "    AND D.AC_CD in ('331100') AND C.DRCR_TYPE='C' " +
                "    AND D.PK=C.TAC_ABACCTCODE_PK " +
                "    AND A.TAC_HGTRD_PK=C.PK " +
                "    AND b.PK=c.TAC_HGTRH_PK " +
                "    AND A.ITEM_TABLE_PK=E.PK " +
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=794 style='border-collapse:
 ...
 <tr height=16 style='height:12.0pt'>
  ...
  ch&#7881;: <%=l_cmp_add %><span style='mso-spacerun:yes'> </span></td>
  <td class=xl24></td>
 </tr>
 <tr height=16 style='height:12.0pt'>
  <td height=16 colspan=2 class=xl24 style='height:12.0pt;mso-ignore:colspan'></td>
