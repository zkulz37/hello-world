...
string str = "test 123";
//string[] arr_seq = p_seq.Split(',');

//for (int x = 0; x < arr_seq.Length; x++)
//{
//    Response.Write(arr_seq[x]);
//    Response.End();
	...
	= "  SELECT a.pk " + 
        ...
        "   AND a.del_if = 0 " +
        "   order by pk " ;
DataTable dt_pk = ESysLib.TableReadOpen(SQL1);
  ...
  string  SQL = "SELECT   " +
   ...
   DataTable dt_b = ESysLib.TableReadOpen(SQL);
    if (dt_b.Rows.Count > 0)
    {
        p_book_ccy = dt_b.Rows[0][0].ToString();
    }
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=720 style='border-collapse:
 ...
 <%
     ...
     {
         string SQL9 = "SELECT H.VOUCHERNO, TO_CHAR(H.TR_DATE,'DD/MM/YYYY') TRANS_DATE,  " +
         ...
         DataTable dt = ESysLib.TableReadOpen(SQL9);
         if (dt.Rows.Count > 0)
         {
         ...
         SQL = "SELECT MAX(CCY), TO_CHAR(SUM(TR_BOOKAMT),DECODE('" + p_book_ccy + "','VND','9,999,999,999,990','9,999,999,999,990.99')) BOOK_AMT, TO_CHAR(MAX(BK_RATE),'9,999,999,999,990'), TO_CHAR(MAX(TR_RATE),'9,999,999,999,990') " +
            " FROM TAC_HGTRD D  " +
            " WHERE D.DEL_IF = 0 " +
            " AND D.TAC_HGTRH_PK in (" + dt_pk.Rows[w][0].ToString() +")"+
            " AND DRCR_TYPE ='D' ";
 ...
 <tr height=32 style='mso-height-source:userset;height:24.0pt'>
  ...
  <td colspan=6 class=xl157 style='border-right:.5pt solid black'><%= p_voucher_no%>/(<%= dt_pk.Rows[w][0].ToString()%>)</td>
  <td class=xl30>&nbsp;</td>
 </tr>
 <tr height=32 style='mso-height-source:userset;height:24.0pt'>
  <td height=32 class=xl28 style='height:24.0pt'>&nbsp;</td>
...
<tr class=xl24 height=46 style='mso-height-source:userset;height:35.1pt'>
 ...
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  <td colspan=2 class=xl90 style='border-right:.5pt solid black' x:num><%= dt_detail.Rows[i][0].ToString()%></td>
 ...
 <%
     ...
     SQL = "SELECT " +
         ...
         " FROM TAC_HGTRD D " +
         " WHERE D.DEL_IF = 0 " +
         " AND D.TAC_HGTRH_PK = " + dt_pk.Rows[w][0].ToString();
 ...
 <%
     ...
     {
         p_ccy_tmp = dt.Rows[j][0].ToString();
         if (p_ccy_tmp == "VND")
         {
             p_xls_ccy_format = "_\\(* \\#\\,\\#\\#0_\\)\\;\\[Red\\]_\\(\\\\ \\\\\\(\\#\\,\\#\\#0\\\\\\)\\;_\\(* \\0022-\\0022??_\\)\\;_\\(\\@_\\)";
         }
 ...
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  <td height=20 colspan=4 class=xl31 style='height:15.0pt;mso-ignore:colspan'></td>
  <td class=xl41><%= dt.Rows[j][0].ToString()%></td>
  <td colspan=3 class=xl63 style='border-right:.5pt solid black;border-left:none; mso-number-format:"<%= p_xls_ccy_format %>"' x:num><%= dt.Rows[j][1].ToString()%></td>
  <td colspan=3 class=xl66 style='border-right:.5pt solid black;border-left:none; mso-number-format:"<%= p_xls_ccy_format %>"' x:num><%= dt.Rows[j][2].ToString()%></td>
  <td colspan=5 class=xl31 style='mso-ignore:colspan'></td>
 </tr>
 <%
     }
