...
<%
    ...
    string l_attr1 = Request["attr1"];
    string l_biz_pk = Request["biz_pk"];
...
SQL = SQL + " FROM DUAL ";
    ...
    SQL = SQL + "     and ((v.tac_abpl_pk = '" + l_pl_pk + "') or ('" + l_pl_pk + "' is null))  ";
    SQL = SQL + "     and ((v.tac_abcenter_pk = '" + l_biz_pk + "') or ('" + l_biz_pk + "' is null))  ";
    SQL = SQL + "     and ((r.tac_abacctcode_pk_dr = '" + p_acpk + "' or '" + p_acpk + "' is null) OR (r.tac_abacctcode_pk_cr = '" + p_acpk + "' or '" + p_acpk + "' is null))  ";

    SQL = SQL + "ORDER BY h.tr_date ASC, h.pk, PLC_CD, PL_CD ";
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1180 style='border-collapse:
 ...
 width:62pt'>
 <col class=xl24 width=285 style='mso-width-source:userset;mso-width-alt:
 10422;width:214pt'>
 ...
 <tr class=xl35 height=21 style='height:15.75pt'>
  ...
  x:autofilterrange="$A$8:$K$23" style='height:15.75pt'>ghi s&#7893;</td>
  ...
  <td class=xl36 id="Td1" x:autofilter="all" style='border-left:none'></td>
  <td class=xl36 id="Td2" x:autofilter="all" style='border-left:none'>Khu vực</td>
  <td class=xl36 id="Td3" x:autofilter="all" style='border-left:none'></td>
  <td class=xl36 id="Td4" x:autofilter="all" style='border-left:none'>Bộ phận</td>
 </tr>
<%
	string trs_date = "";
	string trs_date_dsp = "";
...
<%
    ...
    SQL = SQL + "    FROM tac_hgtrh h, tac_hgtrd_ref r, tac_hgtrditemc d ";
    SQL = SQL + "   WHERE h.del_if = 0 ";
    SQL = SQL + "     AND r.del_if = 0 ";
    SQL = SQL + "     AND h.pk = r.tac_hgtrh_pk ";
    SQL = SQL + "     AND r.drcr_type = 'D' ";
    SQL = SQL + "     AND H.TCO_COMPANY_PK LIKE DECODE(" + l_company + ",'0','%%'," + l_company + " ) ";
    SQL = SQL + "     AND h.tco_company_pk LIKE DECODE ('2', '0', '%%', '2') ";
    SQL = SQL + "     AND TO_CHAR (h.tr_date, 'YYYYMMDD') BETWEEN '" + l_date_from + "' AND '" + l_date_to + "' ";
    SQL = SQL + "     AND h.tr_status IN (0, " + l_Status + " ) ";
    SQL = SQL + "     AND d.tac_hgtrd_pk = r.tac_hgtrd_pk ";
    SQL = SQL + "     AND ((TRIM('" + l_voucher + "') IS NULL) OR (UPPER (h.voucherno) LIKE'%' || UPPER (TRIM('" + l_voucher + "'))|| '%'))  ";
    SQL = SQL + "     AND ((TRIM('" + l_Seq + "')  IS NULL) OR (h.pk = TRIM('" + l_Seq + "') ))  ";
    SQL = SQL + "     and ((r.tco_buspartner_pk = '" + l_partner_pk + "') or ('" + l_partner_pk + "' is null))  ";
    SQL = SQL + "     and ((d.C1 = '" + l_attr1 + "') or ('" + l_attr1 + "' is null))  ";
