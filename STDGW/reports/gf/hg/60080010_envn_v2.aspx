...
String v4 = "";
String v5 = "";
String p_voucher_name = "";
String p_voucher_lname = "";
...
string SQL = "SELECT H.VOUCHERNO, TO_CHAR(H.TR_DATE,'DD/MM/YYYY') TRANS_DATE,  " + 
        ...
        " REMARK,REMARK2,C.PARTNER_NAME,C.PK, H.TR_ENCLOSE, c.PARTNER_lNAME, h.VOUCHER_TYPE" +
...
if (dt.Rows.Count >0)
{
		...
		p_company_name = dt.Rows[0][13].ToString();
		p_company_pk = dt.Rows[0][11].ToString();
        p_enclose = dt.Rows[0][12].ToString();
        p_voucher_type = dt.Rows[0][14].ToString();
}
SQL = "SELECT UPPER(D.REMARK), upper(CODE_LNM) FROM TAC_COMMCODE_MASTER M, TAC_COMMCODE_DETAIL D WHERE M.DEL_IF =0 AND D.DEL_IF =0 AND M.ID ='ACBG0130' AND D.CODE = '" + p_voucher_type + "' ";
dt = ESysLib.TableReadOpen(SQL);

if (dt.Rows.Count > 0)
{
    p_voucher_name = dt.Rows[0][0].ToString();
    p_voucher_lname = dt.Rows[0][1].ToString();
}    
SQL = "SELECT   " +
            "    D.CODE, " +
            "    D.CHAR_1,  " +
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=720 style='border-collapse:
 ...
 <tr class=xl24 height=33 style='mso-height-source:userset;height:24.95pt'>
  ...
  <td colspan=14 class=xl167><%=p_voucher_name%>/<%=p_voucher_lname%></td>
  <td class=xl27 style='border-top:none'>&nbsp;</td>
 </tr>
 <tr height=40 style='mso-height-source:userset;height:47.25pt'>
 ...
 <tr height=24 style='mso-height-source:userset;height:22.0pt'>
 ...
 <tr height=32 style='mso-height-source:userset;height:36.0pt'>
 ...
 <tr height=17 style='mso-height-source:userset;height:25pt'>
  <td height=32 class=xl28 style='height:24.0pt'>&nbsp;</td>
  <td colspan=2 class=xl140>Department/ B&#7897; ph&#7853;n:</td>
  <td colspan=2 class=xl142 style='border-right:.5pt solid black;white-space:normal'><%= p_dept_name %></td>
...
<tr class=xl24 height=46 style='mso-height-source:userset;height:40pt'>
 ...
 <%
     SQL = "SELECT CASE    WHEN   sf_get_accdlocal_by_acpk (D.TAC_ABACCTCODE_PK, " + p_company_pk + ") <> " +
       "                                    SF_A_GET_ACCD_St1(D.TAC_ABACCTCODE_PK, " + p_company_pk + ") " +
       "            THEN " +
       "             SF_A_GET_ACCD (D.TAC_ABACCTCODE_PK, " + p_company_pk + ")  " +
       
       "          ELSE " +
       "             SF_A_GET_ACCD (D.TAC_ABACCTCODE_PK, " + p_company_pk + ") " +
       "       END   ACCD, " +
        " SF_A_GET_ACNM(D.TAC_ABACCTCODE_PK," + p_company_pk + ", 'ENG'),REMARK,REMARK2 " +
        " ,SF_A_GET_AGTRITEM1(D.PK)  || SF_A_GET_ACCD_St1 (D.TAC_ABACCTCODE_PK, " + p_company_pk + ") , " +
        " DECODE(DRCR_TYPE,'D',TO_CHAR(TR_BOOKAMT,DECODE('" + p_book_ccy + "','VND','9,999,999,999,990','9,999,999,999,990.99')),'') DEBIT_AMT, " + "DECODE(DRCR_TYPE,'D','',TO_CHAR(TR_BOOKAMT,DECODE('" + p_book_ccy + "','VND','9,999,999,999,990','9,999,999,999,990.99'))) CREDIT_AMT " +
        " FROM TAC_HGTRD D " +
        " WHERE D.DEL_IF = 0 " +
        " AND D.TAC_HGTRH_PK =  " + p_seq +
        " ORDER BY TYPE_REF , DRCR_TYPE desc,DRCR_ORD ";
     //Response.Write(SQL);
     //Response.End();	
 ...
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  <td colspan=2 class=xl90 style='border-right:.5pt solid black' x:str><%= dt_detail.Rows[i][0].ToString() %></td>
 ...
 <tr height=20 style='height:15.0pt'>
  <td height=20 colspan=16 class=xl31 style='height:15.0pt;mso-ignore:colspan'></td>
 </tr>
 
 <tr height=33 style='mso-height-source:userset;height:24.95pt'>
  <td height=33 class=xl35 style='height:24.95pt;border-left:none'>&nbsp;</td>
  <td colspan=2 class=xl111 width=139 style='border-top:.5pt solid black;border-right:.5pt solid black;
  width:104pt'>Korean Description</td>
  <td colspan=12 class=xl111 width=199 style='border-top:.5pt solid black;border-right:.5pt solid black;
  border-left:none;width:149pt'></td>
  <td class=xl30 style="border-right:none">&nbsp;</td>
 </tr>
 
 <tr height=33 style='mso-height-source:userset;height:24.95pt'>
  <td height=33 class=xl35 style='height:24.95pt;border-left:none'>&nbsp;</td>
  <td colspan=2 class=xl111 width=139 style='border-right:.5pt solid black;
  width:104pt'>General Director /<br />opinion</td>
  <td colspan=12 class=xl113 width=199 style='border-right:.5pt solid black;
  border-left:none;width:149pt'></td>
  <td class=xl30 style="border-right:none">&nbsp;</td>
 </tr>
...
<%
    ...
    decimal p_VAT = 0;
    string p_Total;
    SQL	= " SELECT sum(nvl(TR_BOOKAMT,0)) DEBIT_AMT " + 
        "    FROM TAC_HGTRD D " +
        "   WHERE D.DEL_IF = 0 AND D.TAC_HGTRH_PK = '" + p_seq + "' and D.DRCR_TYPE = 'D' " +
        "   and D.TAC_ABACCTCODE_PK in (SELECT TAC_ABACCTCODE_PK " +
    ...
    if (dt3.Rows.Count > 0)
    {
        p_Expense = decimal.Parse(dt3.Rows[0][0].ToString());
    }
    ...
    if (dt4.Rows.Count > 0)
    {
        p_VAT = decimal.Parse(dt4.Rows[0][0].ToString());
    }
 %>
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  <td height=20 class=xl24 style='height:15.0pt'></td>
  <td colspan=2 class=xl178 width=139 style='width:104pt'>Expense - Chi phí:</td>
  <td class=xl179 width=103 style='border-left:none;border-right:.5pt solid black;border-bottom:.5pt solid black' x:num><%=p_Expense %></td>
  <td colspan=12 class=xl24 style='mso-ignore:colspan'></td>
 </tr>
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  <td height=20 class=xl24 style='height:15.0pt'></td>
  <td colspan=2 class=xl178 style='border-right:.5pt solid black'>VAT - Thuế GTGT:</td>
  <td class=xl179 style='border-top:none;border-left:none' x:num><%=p_VAT %></td>
  <td colspan=12 class=xl24 style='mso-ignore:colspan'></td>
 </tr>
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  <td height=20 class=xl24 style='height:15.0pt'></td>
  <td colspan=2 class=xl178 style='border-right:.5pt solid black'>Total - Tổng:</td>
  <td class=xl179 style='border-top:none;border-left:none' x:num><%=p_Expense + p_VAT%></td>
  <td colspan=12 class=xl24 style='mso-ignore:colspan'></td>
 </tr>
