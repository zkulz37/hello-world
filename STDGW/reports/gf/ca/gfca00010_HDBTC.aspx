...
<%
    ...
    int i;
string SQL
	= "select a.invoice_date, a.recv_person, (select c.partner_name from tco_buspartner c where c.pk = a.tco_buspartner_pk ) partner_name, " + 
        "	(select nvl(c.addr1, '') || nvl(c.addr2, ' ') || nvl(c.addr3, '') addr from tco_buspartner c where c.pk = a.tco_buspartner_pk ) partner_addr, " +
        "        ( select t.item from tac_crcaditem t, tac_abitem i where t.tac_crca_pk = '" + ls_tac_crca_pk + "' and T.DEL_IF = 0  " +
        "        and t.tac_abitem_pk = i.pk and i.del_if = 0 and i.item_nm = 'PAYMENT METHOD' ) payment_method, " +
        "         (select c.tax_code from tco_buspartner c where c.pk = a.tco_buspartner_pk ) tax_code, " +
        "         ( select sum(d.net_tr_amt) from tac_crcad d where d.tac_crca_pk = '" + ls_tac_crca_pk + "' and d.del_if = 0) net_tr_amt, " +
        "        ( select t.item from tac_crcaditem t, tac_abitem i where t.tac_crca_pk = '" + ls_tac_crca_pk + "' and T.DEL_IF = 0  " +
        "        and t.tac_abitem_pk = i.pk and i.del_if = 0 and i.item_nm = 'TAX RATE' ) tax_rate, " +
        "        ( select sum(d.vat_tax_amt) from tac_crcad d where d.tac_crca_pk = '" + ls_tac_crca_pk + "' and d.del_if = 0) vat_tax_amt, a.tr_rate " +
        " from tac_crca a where a.pk = '"  + ls_tac_crca_pk + "' " +
        " and a.del_if = 0 " ;
    ...
    SQL
        = "select i.item_name, i.uom, d.qty, d.u_price, d.net_tr_amt, (select a.tr_rate from tac_crca a where a.pk = '" + ls_tac_crca_pk + "') tr_rate " +
            "from tac_crcad d, tco_item i  " +
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=809 class=xl27479
 ...
 <% 
     ...
     {
         ls_amount = decimal.Round(decimal.Parse(dtItem.Rows[i][4].ToString()));
...
<% 
 ...
 <tr height=26 style='mso-height-source:userset;height:19.5pt'>
  ...
  <td colspan=8 class=xl40479 style='mso-number-format:"_\(* \#\,\#\#0_\)\;_\(* \\\(\#\,\#\#0\\\)\;_\(* \0022-\0022??_\)\;_\(\@_\)"' x:num><span
  style='mso-spacerun:yes'>        </span><%=decimal.Round(l_net_amt * l_tr_rate, 0) %> </td>
  <td class=xl27479></td>
  <td class=xl27479></td>
  <td class=xl27479></td>
