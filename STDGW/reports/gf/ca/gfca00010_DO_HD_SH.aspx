...
<%
	...
	= "select substr(a.tr_date, 1, 4) yyyy, substr(a.tr_date, 5, 2) mm, substr(a.tr_date, 7, 2) dd,  " + 
        ...
        " a.tot_net_tr_amt  tot_net_tr_amt " +
 ...
 SQL
	= "						select tbl_sum.item_name, tbl_sum.uom, round(tbl_sum.qty, 3) qty,  " + 
        ...
        "                                                    from (      " +
        "                                                    select distinct i.item_name, 'Vien' uom, d.qty, d.u_price, d.net_tr_amt, d.item_desc, mafg.sf_get_size(s.matno) sizea,   " +
        "                                                    sf_get_colornm_ar(s1.CAP_COLOR) CAP_COLOR, sf_get_colornm_ar(s1.BODY_COLOR) BODY_COLOR, a.tr_ccy, d.ORDER_NO                              " +
        "                            from tac_crcad d, comm.tco_item i, sale.s_rqst_d s, sale.s_rqst s1, tac_crca a     " +
        "                            where d.del_if = 0      " +
        "                            and a.del_if = 0     " +
        "                            and i.del_if = 0     " +
...
%>
 <tr class=xl40 height=30 style='mso-height-source:userset;height:22.5pt'>
  ...
  <td class=xl48></td>
  <td class=xl49>Viên</td>
  <td class=xl50 x:num><span
  style='mso-spacerun:yes'> </span><%=dtDetail.Rows[j][2]%> </td>
