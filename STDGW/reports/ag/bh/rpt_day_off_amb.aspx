...
<%
  ...
  string SQL
   ...
   for (i=1;i<=i_days;i++)     
   {
          SQL= SQL +  "  ,max(NVL(DECODE(A.WORK_DT,'" + array_date[i-1] + "',DECODE(A.DAY_OFF_YN,'Y','FD',''), ''), '')) D" + i ;
   }    
   SQL = SQL + 
        "    FROM THR_WG_SCH_DETAIL a,THR_EMPLOYEE C,COMM.TCO_ORG D,THR_WORK_GROUP E " +
        "    WHERE a.DEL_IF=0 AND C.DEL_IF=0 AND D.DEL_IF=0 AND E.DEL_IF=0 " +
