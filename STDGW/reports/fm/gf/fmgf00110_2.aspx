...
<%
// Report by Item Group group by Aaccount
int iYear, iMonth;
string iFromDt, iToDt, SQL = "", ls_tin_warehouse_pk = "", ls_ac_cd = "", ls_company_pk = "", ls_item_code = "";
    ...
    DataTable lstAcct = new DataTable();	
SQL
	= "select pk, ac_cd || '-' || ac_nm from tac_abacctcode where del_if = 0 and substr(ac_cd, 1, 3) in ('152', '155') " + 
        "and tco_company_pk = '" + ls_company_pk + "' " ;	
    lstAcct = ESysLib.TableReadOpen(SQL) ;
...
<% 
    int i, j, k;
    for(k = 0 ; k < lstAcct.Rows.Count; k++)
    {
	SQL
	= "select '" + ls_std_ym + "' std_ym, g.grp_cd, g.grp_nm, g.item_uom, null price, sum(m.last_qty) begin_qty,   " + 
        "				 sum(m.last_amt) begin_amt, sum(m.input_qty) in_qty, sum(m.input_amt) in_amt, " +
        "				 sum(m.output_qty) output_qty, sum(m.output_amt) output_amt,  " +
        "				 sum(nvl(m.last_qty, 0) + nvl(m.input_qty, 0) - nvl(m.output_qty, 0)) end_qty, " +
        "				 sum(nvl(m.last_amt, 0) + nvl(m.input_amt, 0) - nvl(m.output_amt, 0)) end_amt, " +
        "				 t.ac_cd				   " +
        "             from tac_stockmm m, " +
        "			 comm.tco_itemgrp g, " +
        "			 comm.tco_item i, " +
        ...
        "			 and i.tco_itemgrp_pk = g.pk(+) " +
        "			 and m.tin_warehouse_pk = w.pk(+) " +
        "			 and m.tac_abacctcode_pk = t.pk " +
        "			 and m.std_ym = '" + ls_std_ym + "' " +
        "			 and m.tco_company_pk = '" + ls_company_pk + "' " +
        "			 and m.tac_abacctcode_pk like '" + lstAcct.Rows[k][0].ToString() + "' " +
        "			 group by '" + ls_std_ym + "', g.grp_cd, g.grp_nm, g.item_uom, t.ac_cd " +
		" having nvl(sum(m.last_qty), 0) + nvl(sum(m.input_qty), 0) + nvl(sum(m.output_qty), 0) <> 0  " +
		" order by t.ac_cd, g.grp_cd  " ;
	dt = ESysLib.TableReadOpen(SQL) ;
%> 
 <tr height=20 style='height:15.0pt'>
  <td height=20 class=xl25 style='height:15.0pt' x:str>Tài khoản:<span
  style='mso-spacerun:yes'> </span></td>
  <td colspan=12 class=xl25 style='mso-ignore:colspan'><%=lstAcct.Rows[k][1] %></td>
 </tr>
...
%>
 ...
 <tr height=20 style='height:15.0pt'>
  ...
  <td class=xl27 style='border-top:none;border-left:none'>&nbsp;</td>
  <td class=xl28 style='border-top:none;border-left:none'>&nbsp;</td>
  <td class=xl28 style='border-top:none;border-left:none' x:num>&nbsp;</td>
  <td class=xl28 style='border-top:none;border-left:none' x:num>&nbsp;</td>
  <td class=xl28 style='border-top:none;border-left:none' x:num>&nbsp;</td>
  <td class=xl28 style='border-top:none;border-left:none' x:num>&nbsp;</td>
  <td class=xl28 style='border-top:none;border-left:none' x:num>&nbsp;</td>
