...
<%
   ...
   string SQL
	= "select a.DEPT_NM,  " +                                  //1
        ...
        "ROUND(NVL(A.NT_45_L1,0)+NVL(A.NT_45_2_L1,0),10), " +                                     //30
        "ROUND(NVL(A.NT_45_L2,0)+NVL(A.NT_45_2_L2,0),10), " +                                     //31
        ...
        ",(select  to_char(to_date(m.from_dt,'yyyymmdd'),'dd-mm-yy') ||' to '|| to_char(to_date(m.to_dt,'yyyymmdd'),'dd-mm-yy')from thr_close  m where  m.del_if=0 and m.ID='02' and m.month_close='" + p_work_mon + "' and  m.times=nvl(a.times,'0'))" + //74
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=4065 style='border-collapse:
 ...
 <col class=xl29 width=77 style='mso-width-source:userset;mso-width-alt:2816;
 width:58pt;<%if(p_status!="A"&&p_sal_period=="ALL"){%> ''<% }else {%> display:none<%} %>' >
 ...
 <tr height=34 style='height:25.5pt'>
  ...
  <td class=xl55>B&#7842;NG L&#431;&#416;NG <%if (p_status == "R")
                                              {%> NGHỈ VIỆC <%} %> NHÂN VIÊN THÁNG <%=dt_Com.Rows[0][5].ToString()+" "%> <%if (p_sal_period != "ALL")
                                                                                                                           {%><%=dt_Emp.Rows[0][66].ToString()+" "%>   <%} %></td>
