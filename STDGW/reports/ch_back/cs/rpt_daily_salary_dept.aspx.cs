...
public partial class rpt_daily_salary_dept : System.Web.UI.Page
{
	...
	= " select  a.PARTNER_NAME,a.addr1,a.tax_code,b.data " + 
        ...
        "                    CONNECT BY PRIOR G.PK = G.P_PK) " +
        "                    OR '" + P_TCO_ORG_PK + "' = 'ALL')  " +
        "        GROUP BY A.WORK_DT,D.PK,D.ORG_NM " +
        "        ORDER BY A.WORK_DT,D.ORG_NM " ;
...
//Response.End();
    ...
    if (irow_emp == 0)
    {
        ...
        string TemplateFile = "rpt_daily_salary_dept.xls";
        string TempFile = "../../../system/temp/rpt_daily_salary_dept_" + Session["User_ID"].ToString() + ".xls";
