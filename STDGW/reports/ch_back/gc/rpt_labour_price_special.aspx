...
<%
	...
	= "SELECT   b.group_nm, a.item_code, a.item_nm, a.sale_order_no, " + 
        ...
        "               , d.emp_id, d.full_name,A.WORK_TIME , " +
        "               round(a.LABOUR_PRICE,1) || '%', round(a.QUANTITY,0),round(a.EMP_AMOUNT,0) " +
        "          FROM VHR_DAILY_RESULT_SPECIAL a, " +
        "               thr_group b, " +
        "               thr_employee d               " +
        "         WHERE d.del_if = 0           " +
    ...
    string SQL_cnt
	= "SELECT   b.group_nm, count( d.emp_id) " + 
        "          FROM VHR_DAILY_RESULT_SPECIAL a, " +
        "               thr_group b, " +
        "               thr_employee d               " +
        "         WHERE d.del_if = 0           " +
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1422 style='border-collapse:
 ...
 <tr height=37 style='mso-height-source:userset;height:27.75pt'>
  ...
  width:989pt'>Special Labour Price Report</td>
 </tr>
 ...
 <tr class=xl25 height=44 style='mso-height-source:userset;height:33.0pt'>
  ...
  Name</td>
  <td class=xl38 style="border-top:none;border-left:none;width:88pt">Work Time</td>
 ...
 <%
     ...
     for (int i = 0; i < dt_emp.Rows.Count; i++)
     {
         ...
         sub1 += Double.Parse(dt_emp.Rows[i][10].ToString());
         sub2 += Double.Parse(dt_emp.Rows[i][11].ToString());
  %>
 <tr class=xl34 height=28 style='mso-height-source:userset;height:21.0pt'>
  ...
  <td class=xl31 rowspan="1" style='border-left:none'><%=dt_emp.Rows[i][1].ToString()%></td>
  ...
  <td class=xl33 style="border-right:.5pt dashed #00CCFF; width: 29px; text-align:center;" ><%=dt_emp.Rows[i][5].ToString()%></td>
  ...
  <td class=xl32 style='border-left:none; text-align:center;'x:str><%=dt_emp.Rows[i][9].ToString()%></td>
  <td class=xl32 style='border-left:none; text-align:right;'x:num><%=dt_emp.Rows[i][10].ToString()%></td>
  <td class=xl33 style='border-left:none' x:num><%=dt_emp.Rows[i][11].ToString()%></td>
 </tr> 
 <!------------------------------------------------------------------------------>    
  ...
  <td class=xl32 colspan=7 style='border-left:none'><b>SUM OF: <%=dt_emp.Rows[i][0].ToString() %></b> </td>
  <td class=xl32  style='border-left:none; text-align:right' x:num><b>Work</b></td>
...
<tr class=xl34 height=28 style='mso-height-source:userset;height:21.0pt'>                      
  ...
  <td class=xl32 colspan=7 style='border-left:none'><b>SUM OF: <%=dt_emp.Rows[i][0].ToString() %></b></td>
  <td class=xl32  style='border-left:none; text-align:right' x:num><b><%=sub%></b></td>
  <td class=xl32  style='border-left:none; text-align:right' x:num><b></b></td>
  <td class=xl32  style='border-left:none; text-align:right' x:num><b><%=sub1%></b></td>  
  <td class=xl33 style='border-left:none' x:num><b><%=sub2%></b></td>  
   </tr>  
  <%      }
