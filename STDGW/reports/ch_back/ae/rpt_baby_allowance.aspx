...
<%
    string p_dept, p_group, p_search_by, p_value, p_from, p_to;
    p_dept = Request.QueryString["p_dept"].ToString();
    p_group = Request.QueryString["p_group"].ToString();
	...
	= "select  a.PK,c.group_nm " + 
        ...
        "   from thr_employee a, thr_group c, comm.tco_dept d " +
        ...
        "                group by tc.EMP_PK) " +
        "   order by d.dept_nm,c.group_nm,a.emp_id  " ;
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=921 style='border-collapse:
 ...
 <%
     for (int i = 0; i < dt_emp.Rows.Count; i++)
     {
         ...
         "    ,'dd-mm-yyyy'), to_char(to_date(c.ED_DATE,'yyyymmdd'),'dd-mm-yyyy'), c.REMARK  " +
        "   from thr_children c " +
        "   where c.DEL_IF = 0 " +
 ...
 <tr height=32 style='userset;height:24.0pt'>
  ...
  <td class=xl31 width=72 style='width:54pt'><%=dt_detail.Rows[0][2].ToString() %></td>
 ...
 <tr height=32 style='userset;height:24.0pt'>
  ...
  <td class=xl31 width=72 style='width:54pt'><%=dt_detail.Rows[j][2].ToString() %></td>
