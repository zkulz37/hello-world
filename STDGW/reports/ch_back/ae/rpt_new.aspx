...
<%
    string p_thr_Wgroup_pk,p_nation,p_search_by,p_search_temp,p_from_joindate,p_to_joindate;
    ...
    string p_ot_yn,p_to_birthdate,p_tco_org_pk,p_sal_security, p_org_type;
    string p_job,p_education,p_lstBand,p_lstGrade,p_wg;
    
    p_thr_Wgroup_pk = Request["p_1"].ToString();
    p_nation = Request["p_2"].ToString();
    p_search_by = Request["p_3"].ToString();
    p_search_temp = Request["p_4"].ToString();
    p_from_joindate = Request["p_5"].ToString();
    p_to_joindate = Request["p_6"].ToString();
    p_salary_yn = Request["p_7"].ToString();
    p_position = Request["p_8"].ToString();
    p_pay_type = Request["p_9"].ToString();
    p_contract_kind = Request["p_10"].ToString();
    p_from_contract = Request["p_11"].ToString();
    p_to_contract = Request["p_12"].ToString();
    p_sex= Request["p_13"].ToString();
    p_insurance_yn = Request["p_14"].ToString();
    p_status = Request["p_15"].ToString();
    p_from_leftdate = Request["p_16"].ToString();
    p_to_leftdate = Request["p_17"].ToString();
    p_from_birthdate = Request["p_18"].ToString();
    p_ot_yn = Request["p_19"].ToString();
    p_to_birthdate = Request["p_20"].ToString();
    p_tco_org_pk = Request["p_21"].ToString();
    p_sal_security = Request["p_22"].ToString();
    
    p_job = Request["p_23"].ToString();
    p_education = Request["p_24"].ToString();
    p_lstBand = Request["p_25"].ToString();
    p_lstGrade = Request["p_26"].ToString();
    p_wg = Request["p_27"].ToString();
    
    string SQL;
	SQL = "select to_char(sysdate,'dd/mm/yyyy'),a.emp_id ,";
//SQL += "c.ORG_nm,b.WORKGROUP_NM";
SQL += " (SELECT   MAX(T.ORG_NM) ";
SQL += "                    FROM COMM.TCO_ORG T ";
SQL += "                    WHERE T.DEL_IF = 0 ";
SQL += "                    AND T.ORG_TYPE IN('06') ";
SQL += "                    START WITH T.PK=A.TCO_ORG_PK ";
SQL += "                    CONNECT BY PRIOR T.P_PK=T.PK) AS ORG_NM, ";
SQL += "        c.ORG_NM AS TEAM ";

	SQL += ",a.full_name,a.full_fname "; 
    ...
    SQL = SQL + " ,(select code_nm from vhr_hr_code where id='HR0109' and code=a.BAND_CD)  ";
    SQL = SQL + " ,(select code_nm from vhr_hr_code where id='HR0110' and code=a.GRADE_CD) ";
    SQL = SQL + " ,nvl(a.BASIC_SAL,0)+nvl(a.ALLOW_AMT1,0)+nvl(a.ALLOW_AMT2,0)+nvl(a.ALLOW_AMT3,0)+nvl(a.ALLOW_AMT4,0)+nvl(a.ALLOW_AMT5,0)+nvl(a.ALLOW_AMT6,0)+nvl(a.ALLOW_AMT7,0)";
    SQL = SQL + ",a.salary_l2_k1,a.salary_l2_k2,a.salary_l2_k3 ";
    SQL = SQL + " ,a.ALLOW_AMT1,a.ALLOW_AMT2,a.ALLOW_AMT3,a.ALLOW_AMT4,a.ALLOW_AMT5,a.ALLOW_AMT6,a.photo_pk,a.ALLOW_AMT7 " ;
    ...
    SQL = SQL + "and decode('" + p_thr_Wgroup_pk + "','ALL','ALL',c.pk)='" + p_thr_Wgroup_pk + "'";
    ...
    SQL = SQL + "and decode('" + p_lstBand + "','ALL','ALL',A.BAND_CD)='" + p_lstBand + "'";
    SQL = SQL + "and decode('" + p_lstGrade + "','ALL','ALL',A.GRADE_CD)='" + p_lstGrade + "'";
    SQL = SQL + "order by nvl(c.seq,0), emp_id ";
//    Response.Write(SQL);
//    Response.End();
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=2314 style='border-collapse:
 ...
 width:35pt'>
 <col class=xl24 width=118 style='mso-width-source:userset;mso-width-alt:2315;
 width:89pt'>
 <col class=xl24 width=114 style='mso-width-source:userset;mso-width-alt:6169;
 width:86pt'>
 <col class=xl24 width=82 style='mso-width-source:userset;mso-width-alt:6998;
 width:62pt'>
 <col class=xl24 width=172 style='mso-width-source:userset;mso-width-alt:6290;
 width:129pt'>
 <col class=xl24 width=62 style='mso-width-source:userset;mso-width-alt:6267;
 width:47pt'>
 ...
 <col class=xl24 width=136 style='mso-width-source:userset;mso-width-alt:4973;
 width:102pt'>
 <col class=xl24 width=136 style='mso-width-source:userset;mso-width-alt:2973;
 width:102pt'>
 <tr height=38 style='mso-height-source:userset;height:28.5pt'>
  <td colspan=22 height=38 class=xl41 width=2314 style='height:28.5pt;
 ...
 <tr class=xl25 height=43 style='mso-height-source:userset;height:32.25pt'>
  ...
  <td class=xl33 style='border-left:none'>Team/Section</td>
  <td class=xl33 style='border-left:none'>Full Name</td>
  <td class=xl33 style='border-left:none'>Full Name In English</td>
  <td class=xl33 style='border-left:none'>Job Title</td>
  <td class=xl33 style='border-left:none'>Job Class</td>
  ...
  <td class=xl33 style='border-left:none'>Band</td>  
  <td class=xl33 style='border-left:none'>Grade</td> 
  <td class=xl33 style='border-left:none'>Gross Salary</td> 
  <td class=xl33 style='border-left:none'>Basic salary</td>  
  <td class=xl33 style='border-left:none'>Addition Salary</td>  
  <td class=xl33 style='border-left:none'>Salary For Crane</td>  
  <td class=xl33 style='border-left:none'>Res Allowance</td>
  <td class=xl33 style='border-left:none'>Gasoline allowance</td>
  <td class=xl33 style='border-left:none'>Telephone</td>
  <td class=xl33 style='border-left:none'>Full Attendance</td>
  <td class=xl33 style='border-left:none'>Other Allowance</td>
  <td class=xl33 style='border-left:none'>Harmfull</td>
  <td class=xl33 style='border-left:none'>Job Title Allowance</td>
 ...
 <tr class=xl25 height=43 style='mso-height-source:userset;height:32.25pt'>
  ...
  <td class=xl34 style='border-top:none;border-left:none'>Cấp bậc</td>
  ...
  <td class=xl34 style='border-top:none;border-left:none'>Ngạch</td>  
  <td class=xl34 style='border-top:none;border-left:none'>Bậc</td>  
  <td class=xl34 style='border-top:none;border-left:none'>Tổng lương </td>
  <td class=xl34 style='border-top:none;border-left:none'>Lương Cơ bản</td>  
  <td class=xl34 style='border-top:none;border-left:none'>Lương trả thêm</td> 
  <td class=xl34 style='border-top:none;border-left:none'>Lương CN vân hành cẩu</td> 
  <td class=xl34 style='border-top:none;border-left:none'>P/C trách nhiệm</td>  
  <td class=xl34 style='border-top:none;border-left:none'>P/C đi lại</td>
  <td class=xl34 style='border-top:none;border-left:none'>P/C điện thoại</td>
  <td class=xl34 style='border-top:none;border-left:none'>P/C chuyên cần</td>
  <td class=xl34 style='border-top:none;border-left:none'>P/C Khác</td>
  <td class=xl34 style='border-top:none;border-left:none'>P/C độc hại</td>
  <td class=xl34 style='border-top:none;border-left:none'>P/C C.Vụ</td>
 ...
 <tr class=xl28 height=68 style='mso-height-source:userset;height:51.0pt'>
  ...
  <td class=xl30 style='border-top:none;border-left:none<%=s_format%>'><%= dt_total.Rows[i][3].ToString() %></td>
  ...
  <td class=xl30 style='border-top:none;border-left:none<%=s_format%>' ><%=dt_total.Rows[i][6].ToString()%></td>
  <td class=xl30 style='border-top:none;border-left:none<%=s_format%>' ><span
  style='mso-spacerun:yes'> </span><%=dt_total.Rows[i][7].ToString()%></td>
  <td class=xl32 style='border-top:none;border-left:none<%=s_format%>' ><span
  ...
  <td class=xl32 style='border-top:none;border-left:none<%=s_format%>' x:num><%=dt_total.Rows[i][17].ToString()%></td>
  <td class=xl32 style='border-top:none;border-left:none<%=s_format%>' x:num><%=dt_total.Rows[i][18].ToString()%></td>
  <td class=xl32 style='border-top:none;border-left:none<%=s_format%>' x:num><%=dt_total.Rows[i][19].ToString()%></td>
  <td class=xl32 style='border-top:none;border-left:none<%=s_format%>' x:num><%=dt_total.Rows[i][20].ToString()%></td>
  <td class=xl32 style='border-top:none;border-left:none<%=s_format%>' x:num><%=dt_total.Rows[i][21].ToString()%></td>
  <td class=xl32 style='border-top:none;border-left:none<%=s_format%>' x:num><%=dt_total.Rows[i][22].ToString()%></td>
  <td class=xl32 style='border-top:none;border-left:none<%=s_format%>' x:num><%=dt_total.Rows[i][24].ToString()%></td>
  ...
  border-left:none;width:47pt<%=s_format%>'><!--[if gte vml 1]><v:shapetype id="_x0000_t75"
   ...
   <v:imagedata src="..\..\..\system\binary\ViewFile.aspx?img_pk=<%=dt_total.Rows[i][23].ToString()%>&table_name=TC_FSBINARY" o:title="03"/>
  ...
  <table cellpadding=0 cellspacing=0>
   ...
   <tr>
    <td></td>
    <td><img width=58 height=66 src="..\..\..\system\binary\ViewFile.aspx?img_pk=<%=dt_total.Rows[i][23].ToString()%>&table_name=TC_FSBINARY"
