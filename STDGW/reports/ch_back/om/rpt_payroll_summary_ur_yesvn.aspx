...
<%
   ...
   // p_to_leftdate = Request["l_to_leftdate"].ToString();
    ...
    //-------------------department summary--------------------------
   string SQL
	= "select a.dept_nm dept_nm," +                                     //0
	    "a.THR_GROUP_NM thr_group_nm,  " +                                  //1
	    "count(*) ,  " +                                  //2
        "SUM(ROUND(NVL(A.INS_SALARY,0),10)),  " +                               //3
        "SUM(ROUND(NVL(A.SALARY_LEVEL2,0),10)),  " +                               //4
        "SUM(ROUND(NVL(A.ALLOW_AMT1,0),10)),  " +                                  //5
        "SUM(ROUND(NVL(A.ALLOW_AMT2,0),10)),  " +                                  //6
        "SUM(ROUND(NVL(A.ALLOW_AMT3,0),10)),  " +                                  //7
        "SUM(ROUND(NVL(A.ALLOW_AMT4,0),10)),  " +                                  //8
        "SUM(ROUND(NVL(A.ALLOW_AMT5,0),10)),  " +                                  //9
        "SUM(ROUND(NVL(A.ALLOW_AMT6,0),10)),  " +                                  //10
        "SUM(ROUND(NVL(A.ALLOW_AMT7,0),10)),  " +                                  //11
        "SUM(ROUND(NVL(A.ALLOW_AMT8,0),10)),  " +                                  //12
        "SUM(ROUND(nvl(A.WT_L1_AMT,0) + nvl(A.WT_L2_AMT,0) + nvl(A.TOTAL_ABS_PAY_L1_AMT,0) + nvl(A.TOTAL_ABS_PAY_L2_AMT,0)+ NVL(a.HOL_DAY_L1_AMT,0)+ NVL(a.HOL_DAY_L2_AMT,0),10)),  " +  //13
        "SUM(ROUND(nvl(A.TOTAL_OT_L1_AMT,0) + nvl(A.TOTAL_OT_L2_AMT,0),10)), " +        //14
        "SUM(ROUND(NVL(A.OT_TAX_L1_AMT,0)+NVL(A.OT_TAX_L2_AMT,0),10))," + //15
        "SUM(ROUND(NVL(A.ALLOW_K1_AMT,0),10)),  " +                                //16
        "SUM(ROUND(NVL(A.ALLOW_K2_AMT,0),10)),  " +                                //17
        "SUM(ROUND(NVL(A.ALLOW_K3_AMT,0),10)),  " +                                //18
        "SUM(ROUND(NVL(A.ALLOW_K4_AMT,0),10)),  " +                                //19
        "SUM(ROUND(NVL(A.ALLOW_K5_AMT,0),10)),  " +                                //20
        "SUM(ROUND(NVL(A.ALLOW_K6_AMT,0),10)),  " +                                //21
        "SUM(ROUND(NVL(A.ALLOW_K7_AMT,0),10)),  " +                                //22
        "SUM(ROUND(NVL(A.ALLOW_K8_AMT,0),10)),  " +                                //23
        "SUM(ROUND(nvl(A.RETURN_AMT,0) ,10)), " +                //24
        "SUM(ROUND(nvl(A.INC_AMT,0),10)), " +                //25
        "SUM(ROUND(NVL(A.ALE_STOP_AMT,0),10)),  " +                                //26
        "SUM(ROUND(NVL(A.SEVERANCE_AMT,0),10)),  " +                               //27
        "SUM(ROUND(NVL(A.GROSS_AMT,0),10)),  " +                            //28
        "SUM(ROUND(NVL(A.BREAK_CONTRACT_AMT,0),10)), " +                           //29
        "SUM(ROUND(NVL(A.ADVANCE_AMT,0),10)),  " +                                 //30
        "SUM(ROUND(NVL(A.DEDUCT_PIT,0),10))," + //31
        "SUM(ROUND(NVL(A.INCOME_AMT,0),10))," + //32
        "SUM(ROUND(NVL(A.INCOME_TAX,0),10)),  " +                           //33
        "SUM(ROUND(nvl(A.SOCIAL_AMT,0) + NVL(A.UNEMP_INS,0)+nvl(A.HEALTH_AMT,0) ,10)) ,  " +           //35
        "SUM(ROUND(nvl(A.NET_AMT,0),0)),'1' seq  " +                               //39
        "from thr_month_salary a " +         
        "where a.del_if=0 and a.work_mon='" + p_work_mon + "' and a.status='A' " +
        ...
        " union all " +
        "select 'RESIGN' dept_nm," +                                     //0
        "'RESIGN' thr_group_nm,  " +                                  //1
        "count(*) ,  " +                                  //2
        "SUM(ROUND(NVL(A.INS_SALARY,0),10)),  " +                               //3
        "SUM(ROUND(NVL(A.SALARY_LEVEL2,0),10)),  " +                               //4
        "SUM(ROUND(NVL(A.ALLOW_AMT1,0),10)),  " +                                  //5
        ...
        "SUM(ROUND(NVL(A.INCOME_TAX,0),10)),  " +                           //33
        "SUM(ROUND(nvl(A.SOCIAL_AMT,0) + NVL(A.UNEMP_INS,0)+nvl(A.HEALTH_AMT,0) ,10)) ,  " +           //35
        "SUM(ROUND(nvl(A.NET_AMT,0),0)),'2' seq  " +                               //39
        "from thr_month_salary a " +
        "where a.del_if=0 and a.work_mon='" + p_work_mon + "' and a.status='R'  " +
        " and (a.tco_dept_pk in ( " +
        "                              SELECT     g.pk " +
        "                                    FROM comm.tco_org g " +
        "                                   WHERE g.del_if = 0 " +
        ...
        " and nvl(a.net_amt,0) " + p_type +
        " and decode('" + p_times + "','ALL','ALL',a.times)='" + p_times + "'" +
        "order by seq,dept_nm,thr_group_nm" ;
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=2757 style='border-collapse:
 ...
 <tr class=xl41 height=46 style='mso-height-source:userset;height:34.5pt'>
  ...
  <td  rowspan=2  class=xl43 width=132 style='border-right:.5pt solid black;
  border-left:none;width:100pt'>Insurance Salary<br>
    <font class="font8">L&#432;&#417;ng BH</font></td>
     <td rowspan=2   class=xl43 width=132 style='border-right:.5pt solid black;
  border-left:none;width:100pt'>Basic Salary<br>
    <font class="font8">L&#432;&#417;ng CB</font></td>
    <%
        if (dt_Allow.Rows[0][8].ToString()=="1")
        {
    %>
  ...
  width:56pt'>OT 100%<br>
   ...
   <%
        ...
        {
    %>
  <td rowspan=2 class=xl46 width=84 style='width:63pt'><%=dt_Allowk.Rows[0][0].ToString() %> <br /> <font class="font12">Phụ cấp tay nghề</font>
    </td>
    <%} %>
    <%
        if (dt_Allowk.Rows[0][1].ToString()!="")
        {
    %>
   <td rowspan=2 class=xl46 width=84 style='width:63pt'><%=dt_Allowk.Rows[0][1].ToString() %> <br /> <font class="font12">Phụ cấp chuyên cần</font>
    </td>
     <%} %>
    <%
        if (dt_Allowk.Rows[0][2].ToString()!="")
        {
    %>
   <td rowspan=2 class=xl46 width=84 style='width:63pt'><%=dt_Allowk.Rows[0][2].ToString() %> <br /> <font class="font12">Phụ cấp nhà ở</font>
    </td>
     <%} %>
    <%
        if (dt_Allowk.Rows[0][3].ToString()!="")
        {
    %>
   <td rowspan=2 class=xl46 width=84 style='width:63pt'><%=dt_Allowk.Rows[0][3].ToString() %><br /> <font class="font12">Phụ cấp độc hại </font>
    </td>
     <%} %>
    <%
        if (dt_Allowk.Rows[0][4].ToString()!="")
  ...
  <td rowspan=2 class=xl48 width=100 style='width:75pt'>Insurance 8.5%<br>
    <font class="font8">Bảo hiểm</font></td>
  <td rowspan=2 class=xl48 width=89 style='width:67pt'>Net Salary<br>
    <font class="font8">Th&#7921;c lãnh</font></td>
 </tr>
 
 <tr class=xl41 height=46 style='mso-height-source:userset;height:34.5pt'>
  <td height=46  class=xl27 width=124 style='height:34.5pt;border-left:none;width:93pt'>Salary <br>
 ...
 <tr class=xl35 height=46 style='mso-height-source:userset;height:34.5pt'>
    ...
    <td height=46  class=xl33 style='height:34.5pt;border-top:none'><%=dt_Emp.Rows[i][0].ToString()%></td>
