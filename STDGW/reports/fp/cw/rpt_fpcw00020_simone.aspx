...
<%        
    ...
    string l_array_wi_pk = Request["array_wi_pk"];
    
    string sql = "SELECT   c.pattern_group_name group_name, ";
           sql = sql + "  DECODE (NVL (a.mat_pk, 0), ";
           sql = sql + "            0, st_mat.item_code, ";
           sql = sql + "            mat.item_code ";
           sql = sql + "           ) mat_code, ";
           sql = sql + "    DECODE (NVL (a.mat_pk, 0), ";
           sql = sql + "            0, st_mat.item_name, ";
           sql = sql + "            mat.item_name ";
           sql = sql + "           ) mat_name, ";
           sql = sql + "    spec_color.spec_nm color, b.net_qty, b.loss_rate, ( b.gross_qty * b.WI_QTY ) gross_qty, ";
           sql = sql + "    b.uom uom1, e.component_name component, d.qty comp_qty, ";
           sql = sql + "    d.t_length, d.t_width, d.uom uom2 , d.tes_lgpicture_pk PICTURE_PK, b.WI_QTY ";
           sql = sql + " FROM prod.wi_cut_ins_spec a, ";
           sql = sql + "    prod.wi_cut_insd_spec b, ";
           sql = sql + "    prod.pt_pattern_group c, ";
           sql = sql + "    comm.tco_stitem st_mat, ";
           sql = sql + "    comm.tco_item mat, ";
           sql = sql + "    comm.tco_spec spec_color, ";
           sql = sql + "    prod.pt_pattern_grpcom d, ";
           sql = sql + "    prod.pt_component e ";
           sql = sql + " WHERE a.del_if = 0 ";
           sql = sql + " AND b.del_if = 0 ";
           sql = sql + " AND a.pk = b.wi_cut_ins_spec_pk ";
           sql = sql + " AND c.del_if(+) = 0 ";
           sql = sql + " AND a.pt_pattern_group_pk = c.pk(+) ";
           sql = sql + " AND st_mat.del_if(+) = 0 ";
           sql = sql + " AND mat.del_if(+) = 0 ";
           sql = sql + " AND a.st_mat_pk = st_mat.pk(+) ";
           sql = sql + " AND a.mat_pk = mat.pk(+) ";
           sql = sql + " AND spec_color.del_if(+) = 0 ";
           sql = sql + " AND b.spec01_pk = spec_color.pk(+) ";
           sql = sql + " AND d.del_if(+) = 0 ";
           sql = sql + " AND a.pt_pattern_group_pk = d.pt_pattern_group_pk(+) ";
           sql = sql + " AND e.del_if(+) = 0 ";
           sql = sql + " AND d.pt_component_pk = e.pk(+) ";
           sql = sql + " AND a.pk in (" + l_array_wi_pk + ") ";    
           sql = sql + " ORDER BY c.pattern_group_name, e.component_name " ;
    ...
    string p_po_no = "", p_cust_name = "" ,  p_group = "" , p_size = "" , p_style_no = "", p_style_name = "" , p_t_a = "" , p_leather = "" ;
     ...
     sql = " SELECT max(a.ref_po_no) po_no, max(c.partner_name) cust_name, ";
     sql = sql + "        max(style.att02) t_group, max(style.att03) t_size, ";
     sql = sql + "        max(style.item_code) style_no, max(style.item_name) style_name, ";
     sql = sql + "        max(style.att04) t_a, max(style.att01) leather ";
     sql = sql + "   FROM prod.wi_cut_ins_spec a, ";
     sql = sql + "        comm.tco_stitem style, ";
     sql = sql + "        comm.tco_buspartner c ";
     sql = sql + "  WHERE a.del_if = 0 ";
     sql = sql + "    AND style.del_if(+) = 0 ";
     sql = sql + "    AND a.style_pk = style.pk(+) ";
     sql = sql + "    AND c.del_if(+) = 0 ";
     sql = sql + "    AND style.tco_buspartner_pk = c.pk(+) ";
     sql = sql + "   AND a.pk in (" + l_array_wi_pk + ") ";
    
    dt_master = ESysLib.TableReadOpen(sql);
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1234 style='border-collapse:
 ...
 <tr height=30 style='mso-height-source:userset;height:22.5pt'>
  <td colspan=14 height=30 class=xl34 width=974 style='height:22.5pt;
 ...
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  <td colspan=6 class=xl28 style='border-left:none'><%=p_group%></td>
 ...
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  <td colspan=6 class=xl28 style='border-left:none'><%=p_style_name%></td>
 ...
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  <td colspan=13 class=xl35 style='border-right:.5pt solid black;border-left:
  none'><%=p_leather%></td>
  <td colspan=4 class=xl24 style='mso-ignore:colspan'></td>
 </tr>
 <tr height=15 style='mso-height-source:userset;height:11.25pt'>
  <td height=15 colspan=18 class=xl24 style='height:11.25pt;mso-ignore:colspan'></td>
 </tr>
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  <td class=xl29 style='border-left:none' >W/I Qty</td>
 ...
 <tr height=80 style='mso-height-source:userset;height:60.0pt'>
  ...
  <td class=xl31 style='border-top:none;border-left:none'><%=dt.Rows[i]["wi_qty"].ToString()%></td>
