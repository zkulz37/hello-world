...
<%
    ...
    string p_from, p_to, p_month, p_nation;
//uong&p_wg=ALL&p_search=1&p_input=&p_tco_org_pk=ALL&p_from=20100104&p_to=20100131&p_opt=1&p_wt_ot=&p_month=01/2010&p_nation=01
   ...
   string SQL
     = "select to_char(sysdate,'dd/mm/yyyy'),D.ORG_NM,E.WORKGROUP_NM,F.EMP_ID,F.FULL_NAME,TO_CHAR(TO_DATE(F.JOIN_DT,'YYYYMMDD'),'DD/MM/YYYY') " +
         ...
         ",DECODE(HR_HR_F_GET_DATE_WEEK(ex.work_dt,'" + p_from + "','" + p_to + "'),1,SUM(DECODE(ex.OT_TYPE,'OT',NVL(ex.OT_TIME,0),0)),0) AS OT_1   " +
         ",DECODE(HR_F_GET_DATE_WEEK(ex.work_dt,'" + p_from + "','" + p_to + "'),1,SUM(DECODE(ex.OT_TYPE,'NT',NVL(ex.OT_TIME,0),0)),0) AS NT_1   " +
         ...
         ",DECODE(HR_F_GET_DATE_WEEK(ex.work_dt,'" + p_from + "','" + p_to + "'),2,SUM(DECODE(ex.OT_TYPE,'NT',NVL(ex.OT_TIME,0),0)),0) AS NT_2   " +
         ...
         ",DECODE(HR_F_GET_DATE_WEEK(ex.work_dt,'" + p_from + "','" + p_to + "'),3,SUM(DECODE(ex.OT_TYPE,'NT',NVL(ex.OT_TIME,0),0)),0) AS NT_3   " +
         ...
         ",DECODE(HR_F_GET_DATE_WEEK(ex.work_dt,'" + p_from + "','" + p_to + "'),4,SUM(DECODE(ex.OT_TYPE,'NT',NVL(ex.OT_TIME,0),0)),0) AS NT_4   " +
         ...
         ",DECODE(HR_F_GET_DATE_WEEK(ex.work_dt,'" + p_from + "','" + p_to + "'),5,SUM(DECODE(ex.OT_TYPE,'NT',NVL(ex.OT_TIME,0),0)),0) AS NT_5   " +
         ...
         ",SUM(DECODE(ex.OT_TYPE,'NT',NVL(ex.OT_TIME,0),0)) as total_nt  " +
         ...
         " having ( DECODE('" + p_opt + "' ,'1',MAX(HR.TOTAL_WT),'2',MAX(HR.TOTAL_OT) " +
         "        ,'3',MAX(HR.TOTAL_HT),'4',MAX(HR.TOTAL_WT) + MAX(HR.TOTAL_OT) " +
         "        ,'5',MAX(HR.TOTAL_OT) + MAX(HR.TOTAL_HT) )>=NVL('" + p_wt_ot + "' ,0) OR '" + p_wt_ot + "' IS NULL  " +
         "        )  " +
