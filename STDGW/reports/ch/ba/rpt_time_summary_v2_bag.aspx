...
<%
    ...
    if (p_opt=="5")
    {
        ...
        "            ,MAX(D.MOD_BY) " +
        "            ,MAX(DECODE(E.OT_TYPE,'NT2',NVL(E.OT_TIME,0),0)) AS NT2,max(g.note_att) note_att " +
        "            ,max(decode(e.ot_type,'OT',nvl(e.approve_yn,'N'),'HT',nvl(e.approve_yn,'N'),'NT2',nvl(e.approve_yn,'N'),nvl(e.approve_yn,'N'))) app_yn " +
        "            FROM THR_EMPLOYEE A,THR_work_group B, THR_WORK_SHIFT C,THR_TIME_MACHINE D,THR_EXTRA_TIME E,tco_org F, THR_WG_SCH_DETAIL G " +
        "            WHERE A.DEL_IF=0 AND B.DEL_IF=0 AND C.DEL_IF=0 AND D.DEL_IF=0 AND E.DEL_IF(+)=0 AND F.DEL_IF=0 AND G.DEL_IF=0  " +
        "            AND A.THR_wg_PK=B.PK AND A.PK=D.THR_EMP_PK AND C.PK=D.THR_WS_PK AND A.TCO_org_PK=F.PK " +
        "            AND D.THR_EMP_PK=E.THR_EMP_PK(+) AND D.WORK_DT=E.WORK_DT(+) AND D.THR_EMP_PK=G.THR_EMP_PK AND D.WORK_DT=G.WORK_DT" +
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1539 style='border-collapse:
 ...
 <tr height=38 style='mso-height-source:userset;height:28.5pt'>
  <td colspan=15 height=38 class=xl42 width=1207 style='height:28.5pt;
 ...
 <tr height=24 style='mso-height-source:userset;height:18.0pt'>
  ...
  <td class=xl24></td>
  <td colspan=4 class=xl43><%=p_user %></td>
 ...
 <tr height=21 style='height:15.75pt'>
  ...
  <td colspan=4 class=xl26 x:num><%= dt_total.Rows[0][0].ToString() %></td>
 ...
 <tr class=xl27 height=26 style='mso-height-source:userset;height:19.5pt'>
  ...
  <td class=xl28 style='border-left:none'>Note</td>
  <td class=xl28 style='border-left:none'>Approve OT</td>
 </tr>
 <tr class=xl27 height=26 style='mso-height-source:userset;height:19.5pt'>
  ...
  s&#7917;a</td>
  <td class=xl32 style='border-top:none;border-left:none'>Ghi chú</td>
  <td class=xl32 style='border-top:none;border-left:none'>Xác nhận tăng ca</td>
 </tr>
 <% 
     string s_format;
 ...
 <tr class=xl29 height=26 style='mso-height-source:userset;height:19.5pt'>
  ...
  <td class=xl33 style='border-top:none;border-left:none<%=s_format%>' ><% = dt_total.Rows[i][18].ToString()%></td>
  <td class=xl33 style='border-top:none;border-left:none<%=s_format%>' ><% = dt_total.Rows[i][19].ToString()%></td>
 </tr>
 <%
    }
  %>
