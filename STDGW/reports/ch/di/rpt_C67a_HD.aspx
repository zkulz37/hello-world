...
<%
   ...
   = "select count(*),nvl(sum(decode(b.sex,'F',1,0)),0),nvl(sum(luong_cb),0)  " +
    ...
    = "select v.CODE_NM, ti.FULL_NAME, ti.SOCIAL_NO, ti.CONDITION, round(NVL(ti.AVERAGE_SALARY,0),0) " +
        "    ,to_char(to_date(ti.ST_SOCIAL_DT,'yyyymm'),'mm-yyyy'),round(nvl(ti.DAYS,0),2), round(nvl(ti.PROGRESSIVE,0),0)  " +
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1254 style='border-collapse:
 ...
 <tr class=xl93 height=17 style='height:12.75pt'>
  ...
  <td colspan=2 class=xl94 width=204 style='width:154pt'>Mẫu số: C 67a- HD</td>
 ...
 <tr class=xl96 height=20 style='height:15.0pt'>
  ...
  ĐỘNG<span style='mso-spacerun:yes'>  </span>ĐỀ NGHỊ HƯỞNG CHẾ ĐỘ THAI SẢN</td>
  <td class=xl96></td>
 </tr>
 ...
 <tr class=xl115 height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  <td class=xl114 align=center style='border-top:none;border-left:none; text-align:center' x:str><%=dt_emp.Rows[i][9].ToString() %></td>
  <td class=xl114 align=center style='border-top:none;border-left:none; text-align:center' x:str><%=dt_emp.Rows[i][10].ToString() %></td>
 ...
 <%     }//if %>
  ...
  <td class=xl114 style='border-top:none;border-left:none; text-align:center' x:num><%=dt_emp.Rows[i][7].ToString() %></td>
  <td class=xl114 style='border-top:none;border-left:none' x:num><%=dt_emp.Rows[i][8].ToString() %></td>
  <td class=xl114 style='border-top:none;border-left:none; text-align:center' x:str><%=dt_emp.Rows[i][9].ToString() %></td>
  <td class=xl114 style='border-top:none;border-left:none; text-align:center' x:str><%=dt_emp.Rows[i][10].ToString() %></td>
  <td class=xl114 style='border-top:none;border-left:none' x:str><%=dt_emp.Rows[i][11].ToString() %></td>  
 </tr>
 <% }//for %>
