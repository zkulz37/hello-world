...
public partial class rpt_policy_behaviour_vst : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
	 ...
	 string p_thr_eva_master_pk,P_SEARCH_BY,P_SEARCH_TEMP,P_TCO_ORG_PK,p_work_group;	
    ...
    = "SELECT  " + 
        ...
        ",NVL((select NVL(EV.POINT,0) from thr_eva_emp_other ev where ev.del_if =0 and EV.GROUP_TYPE ='HR0171' and EV.POINT_TYPE ='01'AND EV.THR_EVA_EMP_PK = A.PK),0) as TC01 " +
        ",NVL((select NVL(EV.POINT,0) from thr_eva_emp_other ev where ev.del_if =0  and EV.GROUP_TYPE ='HR0171' and EV.POINT_TYPE ='02'AND EV.THR_EVA_EMP_PK = A.PK),0) as TC02 " +
        ",NVL((select NVL(EV.POINT,0) from thr_eva_emp_other ev where ev.del_if =0  and EV.GROUP_TYPE ='HR0171' and EV.POINT_TYPE ='03'AND EV.THR_EVA_EMP_PK = A.PK),0) as TC03 " +
        ...
        ",nvl(A.OTHER_SCORE2,0) OTHER_SCORE2 " +
        "FROM thr_eva_emp a,thr_employee e,TCO_ORG o,THR_WORK_GROUP w " +
        "where  e.del_if =0 and o.del_if =0  AND A.DEL_IF =0 AND w.DEL_IF=0 AND e.THR_WG_PK=w.PK  AND DECODE('" + p_work_group + "','ALL','ALL',w.PK)= '" + p_work_group + "' " +
		...
		string TemplateFile = "rpt_policy_behaviour_vst.xls";
        string TempFile = "../../../system/temp/rpt_policy_behaviour_vst_" + Session["User_ID"].ToString() + ".xls";
