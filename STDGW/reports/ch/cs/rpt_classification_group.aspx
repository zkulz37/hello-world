...
<%
    ...
    {
        sql_temp = sql_temp + ",HR_CLASSIFICATION_MONEY_DETAIL(round(a.net_amt,3)," + i.ToString() + ") l" + i + " ";
    }
    //-------------------------main sql-------------------------------
  

		string SQL
	= "select detail.dept_nm, count(*), round(sum(NVL(detail.net_amt,0)),3) "+
		", sum(l1), sum(l2), sum(l3), sum(l4), sum(l5), sum(l6), sum(l7), sum(l8), sum(l9), sum(l10) "+
		"from (select a.THR_GROUP_NM,  " +                                  //1
        "A.EMP_ID, a.dept_nm, " +                                             //2
        "A.FULL_NAME,  " +                                          //3
        "round(NVL(A.net_amt,0),3) net_amt "           ;              //54
   SQL = SQL + sql_temp  +
        ...
        " ) detail " +
		"group by detail.dept_nm order by dept_nm " ;
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1790 style='border-collapse:
 ...
 <tr height=29 style='mso-height-source:userset;height:21.75pt'>
  ...
  <td class=xl28 width=71 style='width:53pt'>CLASSIFICATION OF MONEY SUMMARY IN
 ...
 <tr height=25 style='mso-height-source:userset;height:18.75pt'>
  ...
  <td class=xl24></td>
  <td class=xl24></td>
  <td class=xl29>TỔNG HỢP PHÂN LO&#7840;I TI&#7872;N THÁNG <%=dt_Com.Rows[0][5].ToString()%></td>
