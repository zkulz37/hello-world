...
<%
   ...
   /* p_menu_id="10030007"; //menu salary report form (ko lay tu form truyen vao nhe)
    ...
    string p_dept, p_group, p_status, p_type, p_pay_type, p_emp_id, p_work_mon, p_sal_period, p_emp_type, p_from_leftdate, p_to_leftdate;
   ...
   string SQL
	= "select c.PARTNER_LNAME,  " +                                  //0
        ...
        "ROUND(NVL(A.NT_45_L1,0)+NVL(A.NT_45_2_L1,0),3), " +                                     //37
        "ROUND(NVL(A.NT_45_L2,0)+NVL(A.NT_45_2_L2,0),3), " +                                     //38
        "ROUND(NVL(A.NT_45_L1_AMT,0)+NVL(A.NT_45_L2_AMT,0)+NVL(A.NT_45_2_L1_AMT,0)+NVL(A.NT_45_2_L2_AMT,0),3), " +         //39
        ...
        "from thr_month_salary a,tco_org b,tco_company c " +         
    ...
    <x:PageBreaks>
     <x:RowBreaks>
      <% 
        ...
        {
            if (p_status=="A")
                irow+=51 - (8-icol_Allow) - (8-icol_Allowk)-2 ;
            else
                irow+=51 - (8-icol_Allow) - (8-icol_Allowk) ;
            %>
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=503 style='border-collapse:
 ...
 <tr class=xl25 height=23 style='mso-height-source:userset;height:17.25pt'>
  ...
  height:17.25pt;width:379pt' align=left valign=top><!--[if gte vml 1]><v:shapetype
   ...
   <v:imagedata src="..\..\..\system\binary\ViewFile.aspx?img_pk=<%=dt_Emp.Rows[i][1].ToString()%>&table_name=<%=Session["APP_DBUSER"].ToString()%>.tco_bpphoto" o:title="vinagenuwin"/>
  ...
  src="..\..\..\system\binary\ViewFile.aspx?img_pk=<%=dt_Emp.Rows[i][1].ToString()%>&table_name=<%=Session["APP_DBUSER"].ToString()%>.tco_bpphoto" v:shapes="_x0000_s1025"></span><![endif]><span
 ...
 <tr class=xl25 height=23 style='mso-height-source:userset;height:17.25pt'>
  ...
  <td class=xl34>:</td>
  <td colspan=2 class=xl91 style='border-top:none;border-left:none' x:num><%=dt_Emp.Rows[i][74].ToString() %></td>
