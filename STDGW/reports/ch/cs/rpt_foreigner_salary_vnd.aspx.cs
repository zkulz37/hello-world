...
using NativeExcel;
public partial class reports_ch_cs_rpt_foreigner_salary_vnd : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string TemplateFile = "rpt_foreigner_salary_vnd.xls";
        string TempFile = "../../../system/temp/rpt_foreigner_salary_vnd_" + Session["User_ID"].ToString() + ".xls";
        ...
        = "  SELECT '','',A.EMP_ID, " +
        "         A.FULL_NAME, " +
        "         ROUND (NVL (A.INCOME_AMT_2, 0) - NVL (A.HOUSE_FOR_PIT, 0), 3) basic_sal, " +
        "         ROUND (NVL (A.HOUSE_FOR_PIT, 0), 3) rental_house, " +
        "         ROUND (NVL (A.INCOME_AMT_2, 0), 3) income_amt_2, " +
        "         A.FAMILY_DEPEND, " +
        "         ROUND (NVL (A.DEDUCT_PIT, 0), 3), " +
        "         ROUND (NVL (A.HEALTH_COM_NET, 0), 3), " +
        "         ROUND (NVL (A.INCOME_BEFORE_TAX, 0), 3), " +
        "         ROUND (NVL (A.INCOME_TAX_NET, 0), 3), " +
        "         ROUND (NVL (A.INCOME_AMT_2, 0) - NVL (A.INCOME_TAX_NET, 0), 3), " +
        "         ROUND (NVL (A.ADVANCE_NO_PIT, 0), 3), " +
        "         ROUND (NVL (A.NET_VND, 0), 3), " +
        "         A.ACCOUNT, " +
        "         A.BANK_NM||'-'||A.BANK_BRANCH " +
        ...
        SQL
        = "SELECT COUNT (*) AS l_day_in_month,to_char(to_date('" + p_work_mon + "','yyyymm'),'mm/yyyy'),to_char(to_date('" + p_work_mon + "','yyyymm'),'yyyy.mm') " +
         "  FROM THR_CALENDAR A " +
         " WHERE     A.DEL_IF = 0 " +
        ...
        exSheet.Cells["A1"].Value = "SALARY SHEET IN " + dt_info.Rows[0][1].ToString();
        
        //Show data
        for (i = 0; i < irow_emp; i++)
        {
            for (k = 0; k < total_col; k++)
                total[k] = total[k] + double.Parse(dt_emp.Rows[i][k + 4].ToString());
        ...
        for (k = 0; k < total_col; k++)
        {
            if (k == 3)
                continue;
            exSheet.Cells[pos + irow_emp, k + 4].Value = total[k];
        }
