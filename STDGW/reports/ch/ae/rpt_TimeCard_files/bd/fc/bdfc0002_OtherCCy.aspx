...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1515 style='border-collapse:
 ...
 <tr class=xl25 height=22 style='mso-height-source:userset;height:16.5pt'>
      ...
      <td class=xl45 width=89 style='border-top:none;border-left:none;width:67pt'
       x:str><%= dt_account.Rows[0][3].ToString() %></td>
      <td class=xl45 width=89 style='border-top:none;border-left:none;width:67pt' x:str><%= p_book_ccy %></td>
      <td class=xl45 width=89 style='border-left:none;width:67pt'  x:str><%= dt_account.Rows[0][3].ToString() %></td>
      <td class=xl45 width=89 style='border-left:none;width:67pt'  x:str><%= p_book_ccy %></td>
      <td class=xl45 width=89 style='border-top:none;border-left:none;width:67pt'
       x:str><%= dt_account.Rows[0][3].ToString() %></td>
      <td class=xl45 width=89 style='border-top:none;border-left:none;width:67pt'
       x:str><%= p_book_ccy %></td>
...
{
	if(dt_account.Rows[i][0].ToString().Substring(0, 3) == "111") //cash foreign
	{
		SQL_1 = " SELECT TO_CHAR(sf_get_cashddbal(" + p_company_pk + ",'" + p_from_date + "'," + dt_account.Rows[i][2].ToString() + ",'','TRANS'),'"+p_trans_format+"') ,";
		SQL_1 = SQL_1 + " TO_CHAR(sf_get_cashddbal(" + p_company_pk + ",'" + p_from_date + "'," + dt_account.Rows[i][2].ToString() + ",'','BOOKS'),'"+p_book_format+"') ";
...
%>
 ...
 <tr class=xl25 height=47 style='mso-height-source:userset;height:35.25pt'>
  ...
  colspan="3" ><b>CASH IN HAND (<%= dt_account.Rows[0][3].ToString() %>) </b></td>
...
<%
	SQL_1 = " SELECT  ";
    SQL_1 = SQL_1 + "  DECODE(DRCR_TYPE,'C',D.TR_AMT,0) REC_USD ";
    SQL_1 = SQL_1 + " ,DECODE(DRCR_TYPE,'C',D.TR_BOOKAMT,0) REC_VND ";
    SQL_1 = SQL_1 + " ,DECODE(DRCR_TYPE,'C',0,D.TR_AMT) SPE_USD";
    SQL_1 = SQL_1 + " ,DECODE(DRCR_TYPE,'C',0,D.TR_BOOKAMT)  SPE_VND ";
    SQL_1 = SQL_1 + " ,d.pk, to_char(to_date(h.tr_date,'YYYYMMDD'),'dd-Mon-yy'), d.remark, d.TR_EXRATE, d.PL_TYPE ";
    SQL_1 = SQL_1 + " ,TO_CHAR(DECODE(DRCR_TYPE,'C',D.TR_AMT,0),'"+p_trans_format+"') REC_USD ";
    SQL_1 = SQL_1 + " ,TO_CHAR(DECODE(DRCR_TYPE,'C',D.TR_BOOKAMT,0),'"+p_book_format+"') REC_VND ";
    SQL_1 = SQL_1 + " ,TO_CHAR(DECODE(DRCR_TYPE,'C',0,D.TR_AMT),'"+p_trans_format+"') SPE_USD";
    SQL_1 = SQL_1 + " ,TO_CHAR(DECODE(DRCR_TYPE,'C',0,D.TR_BOOKAMT),'"+p_book_format+"')  SPE_VND ";
    SQL_1 = SQL_1 + " , TO_CHAR((TO_NUMBER('"+p_open_trans+"','"+p_trans_format+"')+ SUM(DECODE(DRCR_TYPE,'C',D.TR_AMT,0) - DECODE(DRCR_TYPE,'C',0,D.TR_AMT))  over (order by h.tr_date,d.pk)),'"+p_trans_format+"') Bal_USD ";
    SQL_1 = SQL_1 + " , TO_CHAR((TO_NUMBER('"+p_open_books+"','"+p_book_format+"')+ SUM(DECODE(DRCR_TYPE,'C',D.TR_BOOKAMT,0) - DECODE(DRCR_TYPE,'C',0,D.TR_BOOKAMT))  over (order by h.tr_date,d.pk)),'"+p_book_format+"') Bal_VND "; 
	...
	SQL_1 = " SELECT  ";
SQL_1 = SQL_1 + " TO_CHAR(SUM(DECODE(DRCR_TYPE,'C',D.TR_AMT,0)),'"+p_trans_format+"') REC_USD ";
SQL_1 = SQL_1 + " ,TO_CHAR(SUM(DECODE(DRCR_TYPE,'C',D.TR_BOOKAMT,0)),'"+p_book_format+"') REC_VND ";
SQL_1 = SQL_1 + " ,TO_CHAR(SUM(DECODE(DRCR_TYPE,'C',0,D.TR_AMT)),'"+p_trans_format+"') SPE_USD";
SQL_1 = SQL_1 + " ,TO_CHAR(SUM(DECODE(DRCR_TYPE,'C',0,D.TR_BOOKAMT)),'"+p_book_format+"')  SPE_VND ";
SQL_1 = SQL_1 + " FROM TAC_BGCASH H, TAC_BGCASHD D ";
SQL_1 = SQL_1 + " WHERE H.DEL_IF = 0 ";
SQL_1 = SQL_1 + " and h.tr_date between '" + p_from_date + "' AND "+"'"+p_to_date+"' ";
SQL_1 = SQL_1 + " AND D.DEL_IF = 0 ";
...
SQL_1 = SQL_1 + " order by tr_date,d.pk ";
 ...
 <tr class=xl25 height=40 style='height:30.0pt'>
  ...
  <td class=xl101 style='border-top:none;border-left:none' x:num><%= dt_cash.Rows[j][7].ToString()%></td>
...
<tr class=xl25 height=47 style='mso-height-source:userset;height:35.25pt'>
  <td height=47 class=xl55  style='height:35.25pt;border-top:none;text-align:left;'
  colspan="3" ><b>CASH IN BANK (<%= dt_account.Rows[0][3].ToString() %>)</b></td>
  <td class=xl44 width=348 style='border-top:none;border-left:none;width:261pt'>A
  balance carried forward from the last account</td>
  <td class=xl92 width=89 style='border-top:none;border-left:none;width:67pt'>&nbsp;</td>
  <td class=xl89 width=89 style='border-top:none;border-left:none;width:67pt'>&nbsp;</td>
 ...
 <%
	SQL_1 = " SELECT account_no, bank_id, pk " + 
        ...
        " AND ccy = '"+dt_account.Rows[0][3].ToString()+"' " ;
	DataTable tbl_bank = ESysLib.TableReadOpen(SQL_1); 
	
		for(k=0;k<tbl_bank.Rows.Count;k++)
		{
			SQL_1 = " SELECT TO_CHAR(sf_get_cashddbal(" + p_company_pk + ",'" + p_from_date + "'," + dt_account.Rows[i][2].ToString() + ","+tbl_bank.Rows[k][2].ToString()+",'TRANS'),'"+p_trans_format+"') ,";
			SQL_1 = SQL_1 + " TO_CHAR(sf_get_cashddbal(" + p_company_pk + ",'" + p_from_date + "'," + dt_account.Rows[i][2].ToString() + ","+tbl_bank.Rows[k][2].ToString()+",'BOOKS'),'"+p_book_format+"') ";
			SQL_1 = SQL_1 + " FROM DUAL ";
			dt_openbal = ESysLib.TableReadOpen(SQL_1);
			p_open_trans = dt_openbal.Rows[0][0].ToString();
			p_open_books = dt_openbal.Rows[0][1].ToString();
...
SQL_1 = SQL_1 + " order by tr_date,d.pk ";
 ...
 <tr class=xl25 height=40 style='height:30.0pt'>
  ...
  <td class=xl100 style='border-top:none;border-left:none;white-space:nowrap;mso-text-control:shrinktofit;' style="mso-number-format:'<%= p_xls_ccy_format_trans %>';" x:num><%= dt_cash.Rows[j][13].ToString()%></td>
  <td class=xl99 style='border-top:none;border-left:none;white-space:nowrap;mso-text-control:shrinktofit;' style="mso-number-format:'<%= p_xls_ccy_format_book %>';" x:num><%= dt_cash.Rows[j][14].ToString()%></td>
  <td class=xl101 style='border-top:none;border-left:none;white-space:nowrap;mso-text-control:shrinktofit;' x:num><%= dt_cash.Rows[j][7].ToString()%></td>
  <td class=xl61 style='border-top:none;border-left:none'>&nbsp;</td>
  <td colspan=6 class=xl25 style='mso-ignore:colspan'></td>
 </tr>
