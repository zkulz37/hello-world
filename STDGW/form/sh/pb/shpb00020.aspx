...
{
    for(var i=1;i<grdMoldPin.rows;i++)
    {
        if(grdMoldPin.GetGridData(i,4)=="")
        {
            alert("Please input pin no(Cap) row "+ i);
            return false;
        }
        else if(grdMoldPin.GetGridData(i,5)=="")
        {
            alert("Please input pin no(Body) row "+ i);
            return false;
        } 
    }
   return true;
