	...
	switch (pos)
	{
		case 'VIEW_DETAIL':
			if ( grdDetail.row > 0 )
			{
				...
				{
					var path   = System.RootURL + '/form/ep/bp/epbp00381.aspx?p_tlg_po_pr_d_pk=' + grdDetail.GetGridData( grdDetail.row, G_PR_D_PK ) + "&p_req_dt=" + grdDetail.GetGridData( grdDetail.row, G_REQ_DATE )  + "&p_ref_no=" + grdDetail.GetGridData( grdDetail.row, G_PR_NO ) + "&p_dept_name=" + grdDetail.GetGridData( grdDetail.row, G_DEPT_NAME );
        			var object = System.OpenModal( path ,950 , 400 ,  'resizable:yes;status:yes');			
				}
				else if ( grdDetail.col == G_IN_QTY ) 
				{
					var path   = System.RootURL + '/form/ep/bp/epbp00382.aspx?p_tlg_po_pr_d_pk=' + grdDetail.GetGridData( grdDetail.row, G_PR_D_PK ) + "&p_req_dt=" + grdDetail.GetGridData( grdDetail.row, G_REQ_DATE )  + "&p_ref_no=" + grdDetail.GetGridData( grdDetail.row, G_PR_NO ) + "&p_dept_name=" + grdDetail.GetGridData( grdDetail.row, G_DEPT_NAME );
        			var object = System.OpenModal( path ,950 , 400 ,  'resizable:yes;status:yes');
				}																		
			}	
		break;
		...
		case 'Report':			 			
			var path = System.RootURL + '/form/ep/bp/epbp00383.aspx';
 ...
 function OnReport(para)
 {
    switch(para)
    {
		...
		case 'FOSE01':
            ...
            var url =System.RootURL + '/reports/ep/bp/rpt_epbp00380_FOSE01.aspx?p_from_date=' + dtFrom.value + '&p_to_date=' + dtTo.value + '&p_wh_type=' + lstWHType.value + '&p_wh_pk=' + lstWarehouse.value + '&p_group_pk=' + lstGroup.value + '&p_item=' + txtItem.text + '&p_partner_pk=' + lstPartner.value + '&p_partner_name=' + txtPartner.text;
...
<body>
    ...
    <gw:data id="pro_epbp00380" onreceive="OnDataReceive(this)">
        <xml>
            <dso  type="list" procedure="<%=l_user%>lg_pro_epbp00380" >
                <input>
    ...
    <gw:data id="pro_epbp00380_1" onreceive="OnDataReceive(this)">
        <xml>
            <dso  type="list" procedure="<%=l_user%>lg_pro_epbp00380_1" >
                <input>
                    <input bind="dtFrom" />
                    <input bind="dtTo" /> 
                </input>
    ...
    <gw:data id="data_epbp00380" onreceive="OnDataReceive(this)"> 
        <xml>
           <dso  type="grid"   function="<%=l_user%>lg_SEL_epbp00380" >
