...
function BodyInit()
{
	var data = "<%=ESysLib.SetListDataFUNC("SELECT ACNT.SF_A_GET_COMMONCODE('ACAB0110') FROM DUAL" )%>";    
...
function OnPopUp(pos)
{
    ...
    {
        case 'project':
		    var path = System.RootURL + "/form/kp/bp/kpbp00060_1.aspx?Project_Pk=" + txtProjectPK.text;
		...
		if(txtProjectPK.text !="")
		{
			if(gridMST.rows-1 > 0 && gridMST.row !='-1')
				{ 
					var path = System.RootURL + "/form/kp/de/kpde00020_adv_regist_popup.aspx?Project_Pk=" + txtProjectPK.text+"&master_pk="+gridMST.GetGridData(gridMST.row, 0)+"&currency="+gridMST.GetGridData(gridMST.row, 4);
					var object = System.OpenModal( path ,750 , 500 , 'resizable:yes;status:yes');
				}
				else
				{
...
<gw:data id="dso_kpde00010_expenditure" onreceive=""> 
	<xml> 
		<dso type="grid" parameter="" function="ec111.sp_sel_kpde00020">
			<input bind="gridMST">
