...
function BindingDataList()
{
    ...
    lstReport.SetDataText(ls_report);
     ...
     var  data = "<%=ESysLib.SetDataSQL("select pk, AC_CD,  TRIM(REPLACE(REPLACE(TRIM(AC_NM), CHR(13), ''), CHR(10), '') )    FROM tac_abacctcode  WHERE ac_cd = ac_get_common_bycode('EACAB053','02') and del_if=0 and USE_YN='Y'")%>";
    var arr  = data.split("|");
...
function GetAccount()
{
   var fpath = System.RootURL + "/form/60/08/60080100_popup_1.aspx?col_nm=Account Name&col_code=Account Code&company=" + lstCompany.value + '&comm_code=&comm_nm=&'+"&val1=" + '' + "&val2=" + '' + "&val3=" + '' + "&dsqlid=ac_SEL_60080100_popup_ACC";
...
function onGetEmployee()
{
    ...
    var fpath = System.RootURL + "/form/60/08/60080100_popup_2.aspx?company_pk=" + lstCompany.value;
...
function OnShowDetail()
{
    ...
    if (ctrl.row >1)
    {
        ...
        var fpath = System.RootURL + '/form/60/08/60080100_popup_3.aspx?comp_pk=' + lstCompany.value + '&from_date=' + txtFROM_DATE.value + '&to_date=' + txtTO_DATE.value + "&acc_pk=" + txtAC_PK.text + '&acc_cd=' + txtAC_CD.text +'&acc_nm=' + txtAC_NM.text + "&emp_pk=" + emp_pk + "&emp_id=" + emp_id + "&emp_nm=" + emp_nm + "&status=" + rdoSTATUS.value + "&ccy=" + ccy ;
        //alert(fpath);
        var wWidth = 1000;
        var wHeight = 500;
