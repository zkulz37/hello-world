...
function BindingData()
{
    ...
    var language = "<%=ESysLib.SetListDataFUNC("SELECT ac_GET_COMMONCODE('COAB0070') FROM DUAL")%>";
    var report = "<%=ESysLib.SetListDataFUNC("SELECT ac_GET_COMMONCODE('EACBK036') FROM DUAL")%>";
...
function OnPopUp(pname)
{
    if (pname=='ACCT')
   {
        var fpath = System.RootURL + "/form/60/08/60080050_popup_Acc.aspx?comm_nm=" + "" + "&comm_code=" + "" + "&comm_nm2=" + "" + "&val1=" + '' + "&val2=" + '' + "&val3=" + lstCompany.value+'' + "&dsqlid=ac_SEL_60080190_ACCD_PL";
...
function RowClick1()
{
    ...
    {       
        ...
        //} 
        this.name = "gfka00050";
        var fpath = System.RootURL + '/form/60/08/60080050_popup.aspx?PK_ROW=' + pk + '&ROW=' + rownum + '&ROWS=' + rows + '&APPRCANCEL=' + status + "&ARRAY_PK=" + ls_arr;
        var wWidth = 950;
        var wHeight = 600;
...
function RowClick()
{
    ...
    if (rownum > 0) 
    {
        ...
        //{
            //ls_arr = ls_arr + ' ' + grdVoucher.GetGridData(i, 0);
        //}
        var fpath = System.RootURL + '/form/60/08/60080050_popup.aspx?PK_ROW=' + pk + '&ROW=' + ctrl.Row + '&ROWS=' + rows + '&APPRCANCEL=' + status + "&ARRAY_PK=" + ls_arr;
        var wWidth = 950;
        var wHeight = 600;
        var x = Math.round((screen.availWidth - wWidth)/2);
        var y = Math.round((screen.availHeight - wHeight)/2);
...
{         
	...
	{
		var url =System.RootURL + '/reports/60/08/rpt_60080020_SL_PL.aspx?p_compk=' +lstCompany.value + '&p_from=' + txtFROM_DATE.value + '&p_to=' + txtTO_DATE.value+ '&p_acc_pk='+txtACPK.text+ '&p_plcenter='+txtPLC_PK.text+ '&p_plunit='+txtPLPK.text+ '&p_status='+rdoSTATUS.value+ '&p_ccy='+lstCcy.value;
	}	
	else if(lstREPORT.value=='01')
	{
		var url =System.RootURL + '/reports/60/08/rpt_60080200_expense_monthly.aspx?p_compk=' +lstCompany.value + '&p_from=' + txtFROM_DATE.value + '&p_to=' + txtTO_DATE.value+ '&p_acc_pk='+txtACPK.text+ '&p_plcenter='+txtPLC_PK.text+ '&p_plunit='+txtPLPK.text+ '&p_status='+rdoSTATUS.value+ '&p_ccy='+lstCcy.value;
	}	
