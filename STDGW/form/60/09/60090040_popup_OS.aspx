...
function BindingDataList()
{
    ...
    ls_Book_ccy     = "<%=ESysLib.SetListDataFUNC("SELECT F_COMMONCODE('ACBG0040','04','') FROM DUAL")%>";
    ...
    <%=ESysLib.SetGridColumnComboFormat("grdDtl", 8,"SELECT trim(A.CODE), A.CODE_NM FROM TCO_ABCODE A, TCO_ABCODEGRP B WHERE A.DEL_IF = 0 AND B.DEL_IF = 0  AND A.TCO_ABCODEGRP_PK = B.PK AND B.ID = 'ACAR0190' AND A.USE_IF = 1")%>;
    <%=ESysLib.SetGridColumnComboFormat("grdDtl", 4,"SELECT trim(A.CODE), A.CODE_NM || ' - (' || A.NUM_VALUE1 || ')' FROM TCO_ABCODE A, TCO_ABCODEGRP B WHERE A.DEL_IF = 0 AND B.DEL_IF = 0 AND A.TCO_ABCODEGRP_PK = B.PK AND B.ID = 'ACCR0160' AND A.USE_IF = 1 AND A.GRP_CODE = 'AP' ORDER BY A.NUM_VALUE1")%>;
...
function OnGetPO()
{
    var fpath   = System.RootURL + "/form/gf/da/gfda00010_APTrType.aspx?company=" + lstCompany.GetData() + "&aptr_type=" + txtAP_Type.GetData();
...
function Popup_Expense()
{
	...
	var fpath  = System.RootURL + "/form/60/09/60090040_expense_popup.aspx?tac_crda_pk=" + grdMst.GetGridData(grdMst.selrow, 0) + "&rate=" + grdMst.GetGridData(grdMst.selrow, 10) + "&status=" + grdMst.GetGridData(grdMst.selrow, 29) + "&txt_book=" + txtBookRate.GetData() + "&ccy=" + grdMst.GetGridData(grdMst.selrow, 9);
...
function onSearchTAcc()
{
    ...
    if ( ls_seq != '')
    {
		...
		var fpath   = System.RootURL + '/form/60/09/60090040_popup_TAcc.aspx?PK_ROW=' + pk + '&APPRCANCEL=' + status;
		var wWidth  = 950;
		var wHeight = 570;
