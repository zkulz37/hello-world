...
{		 
    ...
    ls_data      = "<%=ESysLib.SetListDataFUNC("SELECT AC_GET_COMMONCODE('ACBG0130') FROM DUAL")%>";
	lstVoucherType.SetDataText(ls_data);
	lstVoucherType.value = "AR";
    ls_data      = "<%=ESysLib.SetListDataSQL("select TRIM(CODE) code, A.code_nm FROM TAC_COMMCODE_DETAIL A, TAC_COMMCODE_MASTER B WHERE A.DEL_IF=0 and b.del_if = 0 and a.TAC_COMMCODE_MASTER_PK = b.pk AND TRIM(b.ID) = 'EACAB014' and A.use_yn = 'Y'")%>";        
	...
	var l_whnm = "<%=ESysLib.SetDataSQL("select wh_name from TLG_IN_WAREHOUSE where wh_id like 'PR-DS-COM' and del_if = 0")%>";    	
	txtWH_NM.text = l_whnm ;
	var l_whpk = "<%=ESysLib.SetDataSQL("select pk from TLG_IN_WAREHOUSE where wh_id like 'PR-DS-COM' and del_if = 0")%>";    	
	...
	ls_data = "<%=ESysLib.SetListDataFUNC("SELECT AC_GET_COMMONCODE('ACBG0010') FROM DUAL ")%>" + "|ALL|Select All";	
	...
	<%=ESysLib.SetGridColumnComboFormat("grdDtl", 7, "SELECT A.UOM_CODE, A.UOM_NM FROM TLG_IT_UOM A WHERE A.DEL_IF = 0 ORDER BY A.UOM_NM ")%>;
    ...
    // Serial No
    var ls_serial_no = "<%=ESysLib.SetDataSQL("select serial_no from tco_company where del_if = 0 and rownum = 1 order by pk")%>";
    txtSerialNo_1.SetDataText(ls_serial_no);
    
    
    var l_print_page = "<%=ESysLib.SetDataSQL("select d.code from tac_commcode_detail d, tac_commcode_master a where a.pk =  d.tac_commcode_master_pk and a.del_if = 0 and d.del_if = 0 and a.id = 'ACBG0182'")%>";
    txtPrint.SetDataText(l_print_page);
    // DATA|0|Hóa đơn bán hàng|1|Hóa đơn BTC|2|Hóa đơn điện tử
    ls_data = "<%=ESysLib.SetListDataFUNC("SELECT AC_GET_COMMONCODE('GFQC0026') FROM DUAL ")%>" ;
    lstReportType.SetDataText( ls_data );
    lstReportType.value = '2'; // PHoa don tu in
    txtOrderRate.SetDataText("1");
	
	var ls_BizPlace      = "<%=ESysLib.SetListDataSQL("select pk, loc_nm from tco_busplace where del_if = 0 and tco_company_pk="+ Session("COMPANY_PK") +" order by 1")%>";
...
function onSearchCust(iCase)
{
	switch(iCase)
	{
		...
		case 'Account Code A/R':
			var fpath = System.RootURL + "/form/60/08/60080050_popup_Acc.aspx?comm_nm=" + "" + "&comm_code=" + "131" + "&comm_nm2=" + "" + "&val1=" + 'Y' + "&val2=" + 'Y' + "&val3=" + lstCompany.GetData() + "&dsqlid=ac_sel_60080050_acct";
		...
		break ;
		case 'Trans Type AR':
			var path = System.RootURL + '/form/60/06/60060010_transaction.aspx?compk='+ lstCompany.value;
			var object = System.OpenModal( path ,800 , 600 ,  'resizable:yes;status:yes');
			if( object != null )
			{
				var tmp = object[0];
    ...
    if ((grdInfor.GetGridData( ctrl.row, 4) == 'M') || (grdInfor.GetGridData(ctrl.row, 4) == 'C'))
	{
        ...
        else
        {
            ...
            else if ( grdInfor.GetGridData(ctrl.row, 2)== "Bank Account No." || grdInfor.GetGridData( ctrl.row, 2 )== "DEPOSIT ACCOUNT NO" || grdInfor.GetGridData( ctrl.row, 2 )== "BANK ACCOUNT NUMBER" || grdInfor.GetGridData( ctrl.row, 2 )== "BANK ACCOUNT NO.")
	        {
	            ...
	            var fpath   = System.RootURL + "/form/60/09/60090010_CtrItem2.aspx?comm_code="+ "" + "&comm_nm=" + "" + "&val1=" + ls_ccy + "&company="+ lstCompany.GetData() + '&dsqlid=ac_sel_bank_popup&col_code=' + 'Bank Code'  + '&col_nm=' + 'Bank Name';
                var object  = System.OpenModal(  fpath , 600 , 500 , 'resizable:yes;status:yes'); 
                if (object != null)
                {
                    if (object[0] != 0)
...
//CHK|_MST_PK|_PK|_TCO_ITEM_PK|ITEM CODE|ITEM NAME|UNIT|QTY|PRICE|EXT PRICE|NET AMT|_COMPANY		
		...
		break;
		case "TO":
			var fpath   = System.RootURL + "/form/60/11/60110010_GetSO.aspx?cus_pk="+txtConsignorPK.text+"&cus_id="+url_encode(txtConsignorCD.text)+"&cus_nm="+url_encode(txtConsignorNM.text);
			var object  = System.OpenModal(fpath , 800 , 600 , 'resizable:yes;status:yes'); 
			var i, j;
			if(object != null)
			{
...
{
    var fpath = System.RootURL + "/form/60/08/60080050_popup_Acc.aspx?comm_nm=" + "" + "&comm_code=" + "155" + "&comm_nm2=" + "" + "&val1=" + 'Y' + "&val2=" + 'Y' + "&val3=" + lstCompany.GetData() + "&dsqlid=ac_sel_60080050_acct";
    var o = System.OpenModal( fpath , 550 , 550 , 'resizable:yes;status:yes;toolbar=no;location:no;directories:no;menubar:no;scrollbars:no;'); 
    if (o != null)
    {
        if(o[0] != 0)
...
function Popup_Item()
{
    ...
    {
        fpath = System.RootURL + "/form/60/11/60110010_popup_ShowItem.aspx?type=PROD&IsSelectItem=1";
    }
    else
    {        
       var fpath   = System.RootURL + "/form/60/11/60110010_popup_ShowItem.aspx?type=PROD";
    }           
...
function OnDblDtlVAT()
{
	...
	if(l_col == g_vat_accd)
	{
    var fpath = System.RootURL + "/form/60/08/60080050_popup_Acc?comm_nm=" + "" + "&comm_code=" + "333110" + "&comm_nm2=" + "" + "&val1=" + 'Y' + "&val2=" + 'Y' + "&val3=" + lstCompany.GetData() + "&ac_sel_60080050_acct";
    var o = System.OpenModal( fpath , 550 , 550 , 'resizable:yes;status:yes;toolbar=no;location:no;directories:no;menubar:no;scrollbars:no;'); 
		if (o != null)
		{
			if(o[0] != 0)
...
function OnDblDtlPL()
{
	...
	if(l_col == g_pl_cd)
	{
        var fpath   = System.RootURL + "/form/60/09/60090010_CtrItem2.aspx?comm_code="+ "" + "&comm_nm=" + '' + "&company=" + lstCompany.GetData() + '&dsqlid=ac_sel_60110010_PL_POPUP&col_code=' + "PL CODE" + '&col_nm=' + "PL NAME";
        var object  = System.OpenModal(fpath , 600 , 500 , 'resizable:yes;status:yes'); 
        if ( object != null )  
        {
            if (object[0] != 0)
...
function OnDblDtlCost()
{
	 ...
	 {
		var fpath = System.RootURL + "/form/60/11/60110010_PL_Popup.aspx?comm_nm=" + "" + "&comm_code=" + "632" + "&comm_nm2=" + "" + "&val1=" + 'Y' + "&val2=" + 'Y' + "&val3=" + lstCompany.GetData() + "&dsqlid=ac_sel_60110010_acct_popup";
		var o = System.OpenModal( fpath , 550 , 550 , 'resizable:yes;status:yes;toolbar=no;location:no;directories:no;menubar:no;scrollbars:no;'); 
		if (o != null)
		{
			if(o[0] != 0)
...
function OnCancel()
{
	...
	var fpath = System.RootURL + '/form/60/11/60110010_popup_causecancel.aspx';
	var wWidth = 500;
	var wHeight = 400;
...
function OnPrint()
{
    ...
    {
	    var url = System.RootURL + "/reports/60/11/INVOICE_GTGT_AMB.aspx?company_pk=" + lstCompany.value + "&tac_hgtrh_pk=" + txtseq.text + "&tac_crca_pk=" + txtCRCA_PK.text ;
	    System.OpenTargetPage(url);	
    ...
    else if(lstReportType.value == '2') // hoa don tu in
    {
		...
		if(lstCompany.value==7480)//only print invoice ho chi minh company
		{
			if(lstBiz_Place.value=="135")//hoa don hcm  tco_busplace_pk la 135:,con 138 la chi nhanh')
			{
				...
				{
					var url = System.RootURL + "/reports/60/11/rpt_gfca00010_GTGT_paris_baguette_hcm.aspx?p_tac_crca_pk=" + txtCRCA_PK.GetData()+"&p_user_id="+g_user_id;
					System.OpenTargetPage(url);	
					grdMst.Call("SELECT");
				}
			}
			else if(lstBiz_Place.value=="138") //hoa don hcm  tco_busplace_pk la 135:,con 138 la chi nhanh')
			{
				if(confirm("Bạn có chắc chắn muốn in hoá đơn này ?"))
				{
					var url = System.RootURL + "/reports/60/11/rpt_gfca00010_GTGT_paris_baguette_hcm_CN_DN.aspx?p_tac_crca_pk=" + txtCRCA_PK.GetData()+"&p_user_id="+g_user_id;
					System.OpenTargetPage(url);	
					grdMst.Call("SELECT");
				}
			}
		...
		else if(lstCompany.value==9939)//only print invoice hanoi company (pk=9939)
		{
			if(lstBiz_Place.value=="139")//hoad don ha noi
			{
				if(confirm("Bạn có chắc chắn muốn in hoá đơn này ?"))
				{
					var url = System.RootURL + "/reports/60/11/rpt_60110010_baguette_hn.aspx?p_tac_crca_pk=" + txtCRCA_PK.GetData()+"&p_user_id="+g_user_id;
					System.OpenTargetPage(url);	
					grdMst.Call("SELECT");
				}
			}
			else if(lstBiz_Place.value=="153")//hoa don chi nhanh da nang
			{
				if(confirm("Bạn có chắc chắn muốn in hoá đơn này ?"))
				{
					var url = System.RootURL + "/reports/60/11/rpt_60110010_baguette_hn_CN_DaNang.aspx?p_tac_crca_pk=" + txtCRCA_PK.GetData()+"&p_user_id="+g_user_id;
					System.OpenTargetPage(url);	
					grdMst.Call("SELECT");
				}
			}
	    ...
	    //var url = System.RootURL + "/reports/gf/ca/gfca00010_Vinagenuwin.aspx?tac_crca_pk=" + txtCRCA_PK.GetData() + "&company_pk=" + lstCompany.value + "&tac_hgtrh_pk=" + txtseq.GetData() ;
	    //System.OpenTargetPage(url);	                        
    }    
    ...
    else if(lstReportType.value == '1') // Hoa don dat in
    {
		...
		if(lstCompany.value==9939)//only print invoice hanoi company (pk=9939)
		{
			if(confirm("Bạn có chắc chắn muốn in hoá đơn này ?"))
			{
				var url = System.RootURL + "/reports/60/11/rpt_gfca00010_GTGT_paris_baguette_hanoi.aspx?p_tac_crca_pk=" + txtCRCA_PK.GetData()+"&p_user_id="+g_user_id;
				System.OpenTargetPage(url);	
				grdMst.Call("SELECT");
			}
		}		
...
function OnPopUp()
{
	var fpath = System.RootURL + "/form/60/11/60110010_popup_contract.aspx";
...
function OnShowACNTCode()
{
 //D:\WebProject\ESYS_NEW\form\ag\ga\agga00090_popup.aspx     ?p_tac_crca_pk=" + txtCRCA_PK.GetData()+"&p_user_id="+g_user_id;
 	var fpath = System.RootURL + "/form/60/11/60110010_popup_ACNTCode.aspx?ID="+'ACBG0181'+"&company_pk="+lstCompany.value;// ID=ACBG0181+"&p_user_id="+g_user_id;; company_pk------------------------------------------------------------------------
	var o = System.OpenModal( fpath , 850 , 550 , 'resizable:yes;status:yes;toolbar=no;location:no;directories:no;menubar:no;scrollbars:no;'); 
	if(o != null)
	{	    
	    if(o[1]=="1")
