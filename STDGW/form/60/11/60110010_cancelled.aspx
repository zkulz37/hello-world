...
function BindingDataList()
{
	...
	var l_whnm = "<%=ESysLib.SetDataSQL("select wh_name from TLG_IN_WAREHOUSE where wh_id like 'VCSC CM' and del_if = 0")%>";    	
	txtWH_NM.text = l_whnm ;
	var l_whpk = "<%=ESysLib.SetDataSQL("select pk from TLG_IN_WAREHOUSE where wh_id like 'VCSC CM' and del_if = 0")%>";    	
...
function onSearchCust(iCase)
{
	switch(iCase)
	{
		...
		break ;
		case 'Trans Type AR':
			var path = System.RootURL + '/form/60/11/60090010_Transaction.aspx?compk='+ lstCompany.value;
			var object = System.OpenModal( path ,800 , 600 ,  'resizable:yes;status:yes');
			if( object != null )
			{
				var tmp = object[0];
