...
function Binding()
{
    data = "<%=ESysLib.SetListDataSQL("SELECT  CODE, NAME FROM tco_commcode WHERE PARENT_CODE = 'POINT' and del_if = 0 and code in ('POINT-01','POINT-02','POINT-03','POINT-04','POINT-08')")%>|ALL|Select All";
	lstLocation.SetDataText(data);
	lstLocation.value = 'ALL';
	
	data = "<%=ESysLib.SetListDataSQL("SELECT pk, last_name FROM tht_room_allocate WHERE pk < 0 AND del_if = 0")%>||Select All";
...
function OnReport()
{
	...
	{
		var url = '/reports/60/27/rpt_60270110_sale_trend_month.aspx?p_month='+ dtMonth.value+'&p_location='+lstLocation.value+'&p_item='+txtItem.text+'&p_customer_type='+lstGolfType.value;
	...
	if(radTab.GetData()=='1')//grid date
	{
		var url = '/reports/60/27/rpt_60270110_sale_trend_date_from_to.aspx?p_from_date='+ dtFrom.value+'&p_to_date='+dtTo.value+'&p_location='+lstLocation.value+'&p_item='+txtItem.text+'&p_customer_type='+lstGolfType.value;
			System.OpenTargetPage( System.RootURL+url , "newform" );  
