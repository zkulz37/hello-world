...
function BindingDataList()
{
    ...
    var ls_data4    = "<%=ESysLib.SetDataSQL("SELECT CODE FROM TAC_COMMCODE_MASTER A, TAC_COMMCODE_DETAIL B WHERE A.PK = B.TAC_COMMCODE_MASTER_PK AND A.ID = 'ACBG0130' AND B.CHAR_3 = 'BC' AND A.DEL_IF = 0 AND B.DEL_IF = 0")%>";
    var ls_data5    = "<%=ESysLib.SetDataSQL("SELECT NUM_2 FROM TAC_COMMCODE_MASTER A, TAC_COMMCODE_DETAIL B WHERE A.PK = B.TAC_COMMCODE_MASTER_PK AND A.ID = 'ACBG0130' AND B.CHAR_3 = 'BC' AND A.DEL_IF = 0 AND B.DEL_IF = 0")%>";
    var ls_data6    = "<%=ESysLib.SetDataSQL("SELECT NVL(NUM_3, '0') FROM TAC_COMMCODE_MASTER A, TAC_COMMCODE_DETAIL B WHERE A.PK = B.TAC_COMMCODE_MASTER_PK AND A.ID = 'ACBG0130' AND B.CHAR_3 = 'BC' AND A.DEL_IF = 0 AND B.DEL_IF = 0")%>";
...
//					    }
					    ...
					    {
					        ls_ref = grdDebit.GetGridData(grdDebit.selrow, G_D_TYPE_REF);
					    }
					    else
					    {
					        ls_ref = 1;
...
function onSetData_CR(p_ref)
{
	...
	{
        if ((grdCredit.GetGridData(i, G_C_TYPE_REF) == p_ref) && (grdCredit.selrow != click_cr))
        {
...
function RowClick_DR()
{
	...
	else
	{
	    ...
	    if ((grdDebit.rows > 1) && (grdDebit.selrow < grdDebit.rows) && (grdDebit.selrow != click_dr))
	    {
            ...
            txtacpk.SetDataText(grdDebit.GetGridData(grdDebit.selrow, G_D_ACC_PK));
            txtbprvpvd.SetDataText(grdDebit.GetGridData(grdDebit.selrow, G_D_PK));
            var dr_ratevat  = grdDebit.GetGridData(grdDebit.selrow, G_D_VAT_PER);
            if (dr_ratevat != 'NO')
            {
                if (dr_ratevat != 1)
                {
                    txt_rate.SetDataText(grdDebit.GetGridData(grdDebit.selrow, G_D_VAT_PER));
                }
                else
                {
...
function RowClick_CR()
{
	...
	else
	{
	    ...
	    if ((grdCredit.rows > 1) && (grdCredit.selrow < grdCredit.rows) && (grdCredit.selrow != click_cr))
	    {
            ...
            txtacpk.SetDataText(grdCredit.GetGridData(grdCredit.selrow, G_C_ACC_PK));
            txtbprvpvd.SetDataText(grdCredit.GetGridData(grdCredit.selrow, G_C_PK));
            var cr_ratevat  = grdCredit.GetGridData(grdCredit.selrow, G_C_VAT_PER);
            if (cr_ratevat != 'NO')
            {
                if (cr_ratevat != 1)
                {
                    txt_rate.SetDataText(grdCredit.GetGridData(grdCredit.selrow, G_C_VAT_PER));
                }
                else
                {
                    txt_rate.SetDataText('');
        ...
        if (aValue.length > 0)
        {
            for(i=0; i<aValue.length; i++)
            {
                ...
                if (tmp[1] != 0)
                {
                    ...
                    {
                        grdDebit.SetGridText( grdDebit.selrow, G_D_DRCR, 'D' );
					    grdDebit.SetGridText( grdDebit.selrow,  G_D_TYPE_REF, '1' );
					    grdDebit.SetGridText( grdDebit.selrow, G_D_ACC_PK, tmp[1]);
					    grdDebit.SetGridText( grdDebit.selrow, G_D_ACC_CD, tmp[4]);
					    grdDebit.SetGridText( grdDebit.selrow, G_D_ACC_NM, tmp[5]);
                        grdDebit.SetGridText( grdDebit.selrow, G_D_CCY, tmp[6]);  
                        grdDebit.SetGridText( grdDebit.selrow, G_D_EXRATE, book_rate);
					    click_dr = 0;
                        click_cr = 0;
					    RowClick_DR();
                    }
                    ...
                    {
                        grdCredit.SetGridText( grdCredit.selrow, G_C_DRCR, 'C' );
					    grdCredit.SetGridText( grdCredit.selrow, G_C_ACC_PK, tmp[1]);
					    grdCredit.SetGridText( grdCredit.selrow, G_C_ACC_CD, tmp[4]);
					    grdCredit.SetGridText( grdCredit.selrow, G_C_ACC_NM, tmp[5]);
                        grdCredit.SetGridText( grdCredit.selrow, G_C_CCY, tmp[6]); 
                        grdCredit.SetGridText( grdCredit.selrow, G_C_EXRATE, book_rate);       //Rate
					    var ls_ref;
					    if (grdDebit.selrow != -1)
					    {
					        ls_ref = grdDebit.GetGridData(grdDebit.selrow, G_D_TYPE_REF);
			...
			else
			{
				...
				else
				{
					...
					/*else if(idGridItem.GetGridData( ctrl.row, 8 )=="BENEFICIARY BANK")
					{
					    ...
					    var fpath   = System.RootURL + "/form/60/05/60050010_popup_2005.aspx?comm_code="+ "" + "&comm_nm=" + "" + "&val1=" + ls_ccy + "&company="+ lstCompany.GetData() + '&dsqlid=sp_sel_BFBENEFICIARY_popup&col_code=' + ' Beneficiary Bank '  + '&col_nm=' + 'Account No';
                        var object  =   System.OpenModal(  fpath , 600 , 500 , 'resizable:yes;status:yes'); 
                        if (object != null)
                        {
                            if (object[0] != 0)
...
function OnReportRV()
{
    if (lstPayment.GetData() == "1")//Payment voucher - Phieu Chi
	{
		...
		url =System.RootURL + '/reports/gf/br/gfbr00030_PaymentVoucher_General_pdf.aspx' + rqt; 
		if(confirm("Are you sure you want to report ?" ))
		{
			System.OpenTargetPage( url , 'newform' );
			return;
	...
	else if (lstPayment.GetData()=="2")//Receipt Voucher- Phieu Thu
	{
		...
		if(lstVoucherType.value == 'BC')
			url = System.RootURL + '/reports/gf/br/gfbr00030_ReceiptVoucher_General_Bank_pdf.aspx'+rqt; 
		else		
									...
									<td width='100%' colspan='5' align='right'>
										<table width='100%' cellpadding='1' cellspacing='1'>
												<tr>
														...
														<td width=""><gw:button id="ibtnNew" img="new" alt="New" onclick="onNew()" /></td>
														<td width=""><gw:button id="ibtnSave" img="save" alt="Save" onclick="onSave()" /></td>
														<td width=""><gw:button id="ibtnDelete" img="delete" alt="Delete" onclick="onDelete()" /></td>
														<td width=""><gw:button id="ibtnReport" img="excel" alt="Print Acct" onclick="OnReportAcct()" /></td>
														<td width=""><gw:button id="btnReportRV" img="excel" alt="Print RV" onclick="OnReportRV()" /></td>
												</tr>
										</table>
									</td>
								</tr>
                                        ...
                                        <td>
                                                <table width='100%' cellpadding='0' cellspacing='0'>
                                                        <tr>
                                                                ...
                                                                <td width="70%" colspan='3'><gw:textbox id="txtLocalDescription" csstype="mandatory" /></td>
