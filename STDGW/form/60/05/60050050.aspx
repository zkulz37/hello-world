...
function BodyInit()
{
    ...
    */
	...
	var ls_data9 = "<%=ESysLib.SetListDataSQL("SELECT D.CODE, D.CODE_NM  FROM TAC_COMMCODE_MASTER M, TAC_COMMCODE_DETAIL D WHERE M.DEL_IF =0 AND D.DEL_IF =0 AND M.PK = TAC_COMMCODE_MASTER_PK AND M.ID='GFQC0029' and d.use_yn = 'Y' ORDER BY D.CODE_NM")%>";
...
//			}
			var fpath   = System.RootURL + '/form/60/06/60060020_popup.aspx?PK_ROW=' + pk + '&ROW=' + ctrl.row + '&ROWS=' + rows + "&ARRAY_PK=" + ls_arr;
			var wWidth  = 950;
			var wHeight = 510;
...
function Popup_Cust()
{
    var fpath   = System.RootURL + '/form/60/05/60050010_popup_4.aspx?com_pk='+lstCompany.value;
	...
	switch(lstReport.value)
	{
		case '01':	
			if(dtTrs_fr.value == dtTrs_to.value)
			{
				...
				var url =System.RootURL + '/reports/60/05/60050050_Payment_List_For_Bank_Transfering.aspx'+rqt;   
			...
			if((txtStatusPrint.text!="0") && (txtStatusPrint.text!="2"))
		   {
			...
			if (lstVoucherTypePrint.GetText()=="1")//Receipt Voucher- Phieu Thu
			{
				...
				if(lstVoucherType.value == 'BC' )		
					url =System.RootURL + '/reports/60/05/60050050_ReceiptVoucher_List_General_Bank.aspx'+rqt;
				else
					url =System.RootURL + '/reports/60/05/60050050_ReceiptVoucher_List_General.aspx'+rqt;
				System.OpenTargetPage( url , 'newform' );
			}
					...
					case "EACAB005"://ACCOUNTING
						if(g_report_opt == '2' ) //A5
						{
							if(lstVoucherType.value == 'BN' )						
								url =System.RootURL + '/reports/60/05/60050050_PaymentVoucher_List_General_Bank.aspx'+rqt;
							else
								url =System.RootURL + '/reports/60/05/60050050_PaymentVoucher_List_General.aspx'+rqt;
						}	
						else if(g_report_opt == '1') //A4
						{
							if(lstVoucherType.value == 'BN' )
								url =System.RootURL + '/reports/60/05/60050050_PaymentVoucher_List_General_A4_Bank.aspx'+rqt;
							else
								url =System.RootURL + '/reports/60/05/60050050_PaymentVoucher_List_General_A4.aspx'+rqt;													
						}	
						break;
					case "EACAB017"://OTHER
						url =System.RootURL + '/reports/60/05/60050050_PaymentVoucher_List_Other.aspx'+rqt; 
						break;
					case "EACAB016"://FACOTRY
						url =System.RootURL + '/reports/60/05/60050050_PaymentVoucher_List_Fact.aspx'+rqt; 
						break;  
					  case "20":
		//		        if(lstVoucherType.value == 'PV' )
		//		        {
							  url =System.RootURL + '/reports/60/05/60050030_Payment_Order.aspx?seq='+ arr_pk ;		
		//		        }
		//		        else
		//		        {
		...
		case '03':
			...
			{
				var url =System.RootURL + '/reports/60/05/60050050_ReceiptPaymentInq_VIE.aspx'+rqt;   
			}     
			else
			{
				var url =System.RootURL + '/reports/60/05/60050050_ReceiptPaymentInq.aspx'+rqt;  
		...
		case '04':	
			if(dtTrs_fr.value == dtTrs_to.value)
			{
				...
				var url =System.RootURL + '/reports/60/05/rpt_60050050_Payment_List_For_Bank_Transfering_2.aspx'+rqt;   
				System.OpenTargetPage( url , 'newform' );
			}
			else
			{
		...
		<tr>
            <td width="100%">
                ...
                <tr>
                    ...
                    <td colspan=""><gw:list id="lstDepositNo" styles='width:100%'  /></td>
