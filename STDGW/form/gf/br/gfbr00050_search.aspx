...
function BindingDataList()
{
    ...
    var ls_data1    = "<%=ESysLib.SetListDataSQL("SELECT A.PK, A.org_NM FROM comm.TCO_org A WHERE del_if = 0 ")%>";
...
<!------------------------------------------------------------------------------------------------------->
 ...
 <gw:data id="dso_sel_ctl_mst_pay_pvp" onreceive="OnDataReceive(this)"> 
    ...
    <gw:data id="dso_udp_ctl_mst_pay_pvp" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso type="control" function="ACNT.SP_SEL_PAYMENT_VOUCHER">
            <inout> 
                <inout bind="txtseq1"/>
                <inout bind="lstCompany"/>
                <inout bind="chkauto_YN"/>
    ...
    <gw:data id="dso_sel_count_ctl_item" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso type="process" procedure="ACNT.SP_PRO_GRD_DTL_CNT_PAYMENT" > 
                <input> 
    ...
    <gw:data id="dso_sel_control_new_item" onreceive="ItemStyle()">
        <xml>
            <dso type="grid" function="acnt.SP_SEL_GRD_CTL_ITEM_PAYMENT" >
    ...
    <gw:data id="dso_sel_grd_dtl_payment_dr" onreceive="OnDataReceive(this)">
        <xml>
            <dso type="grid" function="acnt.SP_SEL_GRD_DTL_PAYMENT">
                <input bind="grdDebit" >
                    <input bind="txtseq1"/>
                    <input bind="txt_drcr_type"/>
                    <input bind="txtLang"/>
    ...
    <gw:data id="dso_upd_ctl_item" onreceive="ItemStyle()">
        <xml>
            <dso type="grid" function="acnt.SP_SEL_GRD_CTL_ITEM_PAYMT_CALL" >
                <input bind="idGridItem">
                    <input bind="lstCompany"/>
                    <input bind="txtbprvpvd"/>
                    <input bind="txtacpk"/>
