...
function BodyInit()
{
    ...
    <%=ESysLib.SetGridColumnComboFormat("grdDtl", 3,"SELECT A.CODE, A.CODE_NM FROM COMM.TCO_ABCODE A, COMM.TCO_ABCODEGRP B WHERE A.DEL_IF = 0 AND B.DEL_IF = 0  AND A.TCO_ABCODEGRP_PK = B.PK AND B.ID = 'ACCR0110' AND A.USE_IF = 1 ORDER BY A.CODE")%>;
    var ls_bookcyy  = "<%=ESysLib.SetListDataFUNC("SELECT comm.F_COMMONCODE('ACBG0040','04','') FROM DUAL")%>";
...
<!----------------------------------------------------------------------------------------------------------------->
    <gw:data id="dso_upd_grd_dtl_allocate" onreceive="onRecv()">
        <xml>
            <dso type="grid" parameter="0,1,2,3,4,5" function="acnt.SP_SEL_GRID_AP_DO_ALLOCATE" procedure="ACNT.SP_UPD_GRID_AP_DO_ALLOCATE">
                <input bind="grdDtl">
                    <input bind="txtpk"/>
                </input>
                <output bind="grdDtl"/>
