...
function OnDblClick()
{
    ...
    {
        var path = System.RootURL + "/form/gf/da/gfda00020_DiffQty_Details.aspx?com_pk=" + lstCompany.value + "&date_from=" + dbFrom.value + "&date_to=" + dbTo.value + "&wh_pk=" + lstWH.value + "&item_pk=" + idGrid.GetGridData(event.row, g_tco_item_pk) + "&ac_cd=" + txtAccountCode.GetData() + "&vendor_pk=" + idGrid.GetGridData(event.row, g_vendor_pk);
...
<!------------------------------------------------------------------------------------------->
  <gw:data id="dso_get_data" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso id="1" type="grid"  function="ACNT.sp_sel_other_inventory" > 
                <input bind="idGrid" >                 
