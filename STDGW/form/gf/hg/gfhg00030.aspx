...
function BodyInit()
{
    ...
    <%=ESysLib.SetGridColumnComboFormat("grid", 2,"SELECT A.CODE, A.CODE_NM FROM acnt.TAC_COMMCODE_DETAIL A, acnt.TAC_COMMCODE_MASTER B WHERE A.DEL_IF = 0 AND B.DEL_IF = 0  AND A.TAC_COMMCODE_MASTER_PK = B.PK AND B.ID = 'EACAB030'  ORDER BY A.CODE")%>;
...
function OnPopupSlipEntry()
{
	...
	if (rownum > 0)		 
	{
		if (event.col >= 1)
		{
			...
			var fpath   = System.RootURL + '/form/gf/hg/gfhg00020_popup.aspx?PK_ROW=' + pk + '&ROW=' + ctrl.row + '&ROWS=' + rows + '&APPRCANCEL=0' + "&ARRAY_PK=" + ls_arr;
			var wWidth  = 950;
			var wHeight = 600;
...
{
    var fpath   = System.RootURL + "/form/gf/co/gfco002005.aspx?comm_code="+ "" + "&comm_nm=" + '' + "&company=" + lstCompany.GetData() + '&dsqlid=ACNT.SP_SEL_USER_ID_NM&col_code=' + '' +"User ID" + '&col_nm=' + '' + "User name";
