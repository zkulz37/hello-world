...
{ 
    ...
    if (AdminYN == 'Y')
    {
        var ls_data     = "<%=ESysLib.SetListDataSQL("SELECT TRIM(A.CODE), A.CODE_NM FROM COMM.TCO_ABCODE A, COMM.TCO_ABCODEGRP B WHERE A.DEL_IF = 0 AND B.DEL_IF = 0  AND A.TCO_ABCODEGRP_PK = B.PK AND B.ID = 'ACAB0070' AND A.USE_IF = 1 ORDER BY ORD")%>";
    }
    else if (AdminYN == 'N')
    { 
        var ls_data     = "<%=ESysLib.SetListDataSQL("SELECT TRIM(A.CODE), A.CODE_NM FROM COMM.TCO_ABCODE A, COMM.TCO_ABCODEGRP B WHERE A.DEL_IF = 0 AND B.DEL_IF = 0  AND A.TCO_ABCODEGRP_PK = B.PK AND B.ID = 'ACAB0070' AND A.USE_IF = 1 AND A.SYS_YN = 'N' ORDER BY ORD")%>";
    }
...
<body>
    <gw:data id="dso_sel_trans">
        <xml>
            <dso type="grid" parameter="0,1,2,3,4" function="acnt.SP_SEL_TRANSGROUP">
                <input bind="idGrid" >
    ...
    <gw:data id="dso_sel_trans_debit" onreceive="OnDataReceive(this)">
        <xml>
            <dso type="grid" parameter="0,1,2" function="acnt.SP_SEL_ACCOUNT_DRCR">
                <input bind="idGridDr" >
                    <input bind="txt_code"/>
                    <input bind="txt_name"/>
                    <input bind="txt_DRCR"/>
