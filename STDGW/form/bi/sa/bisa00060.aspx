...
function OnPopUp(pos)
{
    switch(pos)
    {
         case 'DetailConfirm':
            if ( grdConfirm.row > 0 )
            {
                var path = System.RootURL + '/form/bi/sa/bisa00061.aspx?type=APPROVE&slip_pk=' + grdConfirm.GetGridData( grdConfirm.row, G1_OS_PK);
         ...
         case 'DetailApprove':
            if ( grdApprove.row > 0 )
            {
                var path = System.RootURL + '/form/bi/sa/bisa00061.aspx?type=CANCEL&slip_pk=' + grdApprove.GetGridData( grdApprove.row, G2_OS_PK);
                var object = System.OpenModal( path, 1000, 600, 'resizable:yes;status:yes', this); 
...
{ 
 /*   if(grdApprove.row > 0)
    ...
    else
    {
     ...
     if(grdApprove.row > 0)
    {
        var path = System.RootURL + '/form/bi/sa/bisa00063.aspx?master_pk='+grdApprove.GetGridData( grdApprove.row, G2_OS_PK);
...
<body>
    ...
    <gw:data id="dso_wh_list_approve" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso type="list" procedure="<%=l_user%>lg_sel_bisa00060_2" > 
                <input>
    ...
    <gw:data id="dso_wh_list_cancel" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso type="list" procedure="<%=l_user%>lg_sel_bisa00060_3" > 
                <input>
                    <input bind="txtUser_PK" /> 
                </input> 
                <output>
    ...
    <gw:data id="data_bisa00060" onreceive="OnDataReceive(this)">
        <xml> 
            <dso id="1" type="grid"  function="<%=l_user%>lg_SEL_bisa00060"  >
    ...
    <gw:data id="data_bisa00060_1" onreceive="OnDataReceive(this)">
        <xml>
            <dso id="2" type="grid"  function="<%=l_user%>lg_SEL_bisa00060_1"  >
    ...
    <gw:data id="pro_bisa00060" onreceive="OnDataReceive(this)">
        <xml>
            <dso type="process" procedure="<%=l_user%>lg_pro_bisa00060" >
                <input>
    ...
    <gw:data id="pro_bisa00060_1" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso type="process" procedure="<%=l_user%>lg_pro_bisa00060_1" > 
                <input>
                    <input bind="txtSlipPK" /> 
                </input> 
                <output>
