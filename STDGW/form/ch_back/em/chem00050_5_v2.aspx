...
<gw:data id="datUser_info" onreceive="OnDataReceive(this)"  > 
        <xml>
            <dso  type="list" procedure="hr_sp_sel_chem00050_51"  > 
                <input>
...
<gw:data id="datDeptData" onreceive="OnDataReceive(this)"  > 
        <xml> 
            <dso  type="list" procedure="hr_sp_sel_chem00050_52" > 
                <input>
...
<gw:data id="datGroupData" onreceive="OnDataReceive(this)"  > 
        <xml> 
            <dso  type="list" procedure="hr_sp_sel_chem00050_53" > 
                <input>
                    <input bind="lstDept" /> 
                </input> 
                <output>
...
<gw:data id="datEmployeePopup"> 
    <xml> 
        <dso  type="grid" function="hr_sp_sel_chem00050_54"> 
            <input> 
                <input bind="lstDept" /> 
                <input bind="lstGroup" /> 
                <input bind="chkTemp" /> 
...
<table style='margin-left:10px' align = top class="itable" cellspacing=0 cellpadding=0 border=1 style="width:100%;height:100%;" >
		        ...
		        <tr>
			        <td align="Left">
				        <table   border="0" CELLSPACING="0" CELLPADDING="0" style="height:inherit" >
					        <tr>   
						        <td width="10%" align="right">Organization</td>
						        <td width="22%" colspan=2><gw:list id="lstDept" styles="color:black;width:100%" value="ALL" onchange="OnChangeDept()"> 
	                                           <data><%=ESysLib.SetListDataSQL("SELECT PK,org_NM FROM TCO_org WHERE DEL_IF = 0 order by org_ID, org_NM")%>|ALL|Select All</data>
	                                           </gw:list > 
						        </td>
						        <!--td width="7%" align="right"> &nbsp;</td-->
						        <td width="15%"><gw:list id="lstGroup" styles="color:black;width:100%;display:none" value="ALL" onchange=""> 
	                                            <data><%=ESysLib.SetListDataSQL("SELECT PK,workgroup_NM FROM Thr_work_Group WHERE DEL_IF = 0 order by seq")%>|ALL|Select All</data>
	                                            </gw:list > 
						        </td>
	...
	<tr align = top> 
	    <td align = top  width="100%" style="height:93%" >
  		    <gw:grid   
			    id="grdEmployee"  
			    header="_PK|Organization|_Group|Emp ID|Card ID|Full Name|OLD CODE|Personal ID|Join Date|_social_no|_Tel|_Name_VN|_Dept_VN|_Pos_nm|_Pos_Kname|_Pos_Type|_Dept_PK|_Group_PK|_Approver_PK|_Approver_ID|_Approver_Name|_Education|_Married_YN|_PersonID|_Address|_manager_kind|manager nm|Company"   
