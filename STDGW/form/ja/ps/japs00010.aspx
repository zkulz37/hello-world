...
function BodyInit()
{
    ...
    txtTotalProshop.SetReadOnly(true);
	txtDistcProshop.SetReadOnly(true);
	txtTotalClubHouse.SetReadOnly(true);
	txtDistcClubHouse.SetReadOnly(true);
...
function Binding()
{
    ...
    data = "<%=ESysLib.SetGridColumnDataSQL("SELECT CODE, NAME FROM comm.tco_commcode WHERE PARENT_CODE = 'POINT' and code in ('POINT-02','POINT-07')")%>";
...
function Control_OnClick(obj){
    switch(obj){
        case "Report1":
            var url = System.RootURL + "/system/ReportEngine.aspx?export_pdf=Y&file=ja/ps/japs00010.rpt&procedure=CRM.sp_sel_japs00010_rptmt|CRM.sp_sel_japs00010_rptdt&parameter=" + dtDate.GetData() + "|" + dtDate.GetData();
        ...
        case "dsoListInvoices":
            ...
            if(grdInvoices.rows > 1){
                for ( var i=1; i < grdInvoices.rows; i++){
                    ...
                    if(grdInvoices.GetGridData( i, COL_LOC) == 'POINT-02'){
                    ...
                    if(grdInvoices.GetGridData( i, COL_LOC) == 'POINT-07'){
                        sum_Proshop  	+= Number(grdInvoices.GetGridData( i, COL_AMT));
						dis_Proshop  	+= Number(grdInvoices.GetGridData( i, COL_DIS));
					}
            ...
            txtTotalProshop.SetDataText("" + sum_Proshop);
			txtDistcProshop.SetDataText("" + dis_Proshop);
            txtTotalClubHouse.SetDataText("" + sum_ClubHouse);
			txtDistcClubHouse.SetDataText("" + dis_ClubHouse);
...
<body bgcolor='#F5F8FF'>
    <gw:data id="dsoListInvoices" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso type="grid" function="crm.sp_sel_japs00010" procedure="">  
    ...
    <gw:data id="dsoListSell" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso type="grid" function="crm.sp_sel_japs00011" procedure="">  
                <input bind="grdSell">  
                    <input bind="dtDate" />
                    <input bind="lstLocation" />
                </input> 
                             ...
                             <fieldset>
						        <table style="width: 100%; height: 100%">
							        <tr>
								        ...
								        <td style="width:3%">
								            <gw:imgbtn id="ibtnReport" img="printer" alt="ProShop Daily Report" onclick="Control_OnClick('Report1');" styles='width:100%'/>
								        </td>
                                ...
                                <tr>
                                    <td valign="top" style="background-color:#33CCFF">
                                        <table style="height: 100%; width: 100%;border:0px;" cellspacing="1" cellpadding="1">
                                            ...
                                            <tr style="height:17px">
                                                <td style="padding-left:5px;background-color:White">Total Proshop Club House</td>
                                                <td style="background-color:White"><gw:textbox id="txtTotalClubHouse" type="number" format="###,###,###,###.##" styles="width:100%;border:0px;"  /></td>
												<td style="background-color:White"><gw:textbox id="txtDistcClubHouse" type="number" format="###,###,###,###.##" styles="width:100%;border:0px;"  /></td>
                                            </tr>
                                            <tr style="height:17px">
                                                <td style="padding-left:5px;background-color:White">Total Proshop Golf Academy</td>
                                                <td style="background-color:White"><gw:textbox id="txtTotalProshop" type="number" format="###,###,###,###.##" styles="width:100%;border:0px;"  /></td>
												<td style="background-color:White"><gw:textbox id="txtDistcProshop" type="number" format="###,###,###,###.##" styles="width:100%;border:0px;"  /></td>
                                            </tr>
                                        </table>
                                    </td>
                                </tr>
