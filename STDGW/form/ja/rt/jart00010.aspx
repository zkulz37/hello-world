...
<script>
var arrArea = "<%=ESysLib.SetGridColumnDataSQL("SELECT NAME ||'     '|| code||'-'||parent_code  cl1, NAME ||'     '|| code||'-'||parent_code cl2 FROM comm.tco_commcode WHERE parent_code in (SELECT code FROM comm.tco_commcode WHERE parent_code = 'POINT') ")%>";
function BodyInit()
{
    Binding();
	OnSearch();
...
{
	var data; 
	data = "<%=ESysLib.SetListDataSQL("SELECT replace(cl1,' ','') cl1, cl2 FROM (SELECT ' ' cl1, 'ALL' cl2 FROM DUAL union SELECT code, NAME FROM comm.tco_commcode WHERE parent_code = 'POINT' ) a")%>";
...
<body bgcolor='#F5F8FF'>
    <gw:data id="dsoGroupEntry"> 
        <xml> 
            <dso type="grid" parameter="0,1,2,3,4,5,6,7,8" function="crm.sp_sel_jart00010" procedure="crm.sp_upd_jart00010">  
