...
function BuildGrid()
{
	...
	grd_con.TextMatrix(0, 0)  = 'Other Operated Departments Statement of Income & Expenses';
	grd_con.TextMatrix(0, 1)  = 'Account Code';
	grd_con.TextMatrix(0, 2)  = 'Dept Code';
	grd_con.TextMatrix(0, 3)  = 'Convenience Store';
	grd_con.TextMatrix(0, 4)  = 'Others';
	grd_con.TextMatrix(0, 5) = '1';
	grd_con.TextMatrix(0, 6) = '2';
	grd_con.TextMatrix(0, 7) = '3';
}
//======================================================================
function OnClick(obj)
{
    ...
    {
        case "ibtnSearch":
			dso_sp_sel_jama00050.Call('SELET');
        break;
        case "ibtnUpdate":
            alert("Update");
        break;
        ...
        break;
        case "ibtnReport":
            var fpath = System.RootURL+ '/reports/ja/ma/rpt_jama00050.aspx?p_Month=' + idMonth.GetData();
            window.open(fpath);
        break;
    }
}
...
{	
	...
	{
	    if(grdData.GetGridData(i, 6)== "B")
        {
	        ctrl.Cell(14, i, 0, i, 4) = false;
	        grdData.SetCellBold(i, 0, i, 4, true);
        }
        else if (grdData.GetGridData(i, 6)== "I")
        {
	        grdData.SetCellBold(i, 0, i, 4, false);
		...
		grdData.GetGridControl().TextMatrix(1, 3)  = 'Convenience Store';
		grdData.GetGridControl().TextMatrix(1, 4)  = 'Others';

		grdData.SetCellBgColor(i, 0, i, 4, Number(grdData.GetGridData(i, 7)));
	}
	...
	{			
		...
		grdData.SetGridText(2, j, 'Outlets');
	}
	grdData.GetGridControl().Cell(14, 2, 0, 2, 4) = false;
	grdData.SetCellBold(2, 0, 2, 4, true);
...
<body>
<!--------------------------------------------------------------------------------------->
<gw:data id="dso_sp_sel_jama00050" onreceive="OnDataReceive()">
    <xml>                                                               
        <dso id="1" type="grid" user="prod" function="acnt.sp_sel_jama00050"> 
            <input> 
                <input bind="idMonth" />
            </input> 
            <output bind="grdData" /> 
                ...
                <tr width="100%">
				 ...
				 <tr style="width:100%; height:95%">
                	<td colspan="4" style="padding-top:4px;background: #BDE9FF">
                    	<gw:grid id="grdData"
                        	 header		="0|1|2|3|4|_5|_6|_7"
                             format		="0|0|0|1|1|0|0|0"
                             aligns		="0|1|1|3|3|0|0|0"
                             defaults	="|||||||"
                             editcol	="0|0|0|0|0|0|0|0"
							 sorting	="F"
                             widths		="3200|1200|1200|3000|3000|0|0|0"
