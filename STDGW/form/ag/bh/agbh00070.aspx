...
{	 
	...
	var url = "/form/ag/bh/agbh00070_popup.aspx?MONTH=" + v_month + "&YEAR=" + v_year+"&v_company_pk="+lstCompany.value;
	var obj;
...
<gw:data id="datCanlendar" onreceive="OnDataReceive(this)"  > 
        <xml> 
            <dso  type="grid"  function="HR_sp_sel_calendar" > 
...
<gw:data id="datMaxDate" onreceive="OnDataReceive(this)"  > 
        <xml> 
            <dso  type="process" procedure="HR_sp_sel_max_date" > 
                <input>
                    <input bind="dtDate" /> 
					<input bind="lstCompany" />
                </input> 
...
<gw:data id="datCreateDate" onreceive="OnDataReceive(this)"  > 
        <xml> 
            <dso  type="process" procedure="HR_pro_create_date" > 
                <input>
                    <input bind="dtDate" />
                    <input bind="dtUntilDate" /> 
					<input bind="lstCompany" />
...
<table width="100%" border=0>
	<tr>
		<td width=7% align=right>Company</td>
		<td class="topright" width="15%">
                    <gw:list  id="lstCompany"  styles='width:100%' onchange="OnSearch()" >
                        <data>
                            <%= ESysLib.SetListDataSQL("select a.pk,a.PARTNER_NAME from tco_company a  where a.del_if = 0 and nvl(a.active_yn,'N')='Y' order by a.pk ")%>
                        </data>
                    </gw:list>
                </td>
		<td width=10% align=right>Max date</td>
		<td width=10% align=right><gw:datebox id="dtDate" type="date" lang="<%=session("lang")%>" nullaccept onchange=""/></td>
		<td width="15%" align=right>Create Date Until </td>
		<td width="10%" align=left><gw:datebox id="dtUntilDate" type="date" lang="<%=session("lang")%>" onchange="OnChangeDate()"/>				
		</td>
		<td width="5%"><gw:imgBtn img="process" id="ibtnProcess"   alt="Create Date"  onclick="OnCreate()"/></td>
