...
{
// cac nut Approve, Reject , Request more Information
...
// total amount
    ...
    // trangthai cua response
    tarResponse.SetEnable(false);    
    // trangthai cua cac nut
    btnSave.SetEnable(false);    
    // dept mac dinh la phong cua user log in vao
    lstDept.value = dept_pk ;    
...
function OnDataReceive(iObj)
{
    ...
    switch(iObj.id)
    {
        ...
        {
            tarResponse.SetEnable(true);                
            var ls_data     = "<%=ESysLib.SetListDataSQL("SELECT A.code, A.code_nm FROM THR_CODE_detail A, THR_CODE_MASTER B WHERE A.DEL_IF=0 AND B.DEL_IF=0 AND B.PK = A.thr_code_master_pk AND B.ID = 'HR0080' and A.code in ('20', '30', '60', '70', '80', '110') ")%>";
            lstStatus.SetDataText(ls_data);                                 
            lstStatus.value = '20'; // submit
        }            
        else // disable form
        {
            dtSubmitFr.SetEnable(false);
            dtSubmitTo.SetEnable(false);
            lstDept.SetEnable(false);           
            lstSearchBy.SetEnable(false);   
            txtSearchValue.SetEnable(false);
...
function OnApprove()
{
    ...
    for(i = 1; i < idGrid.rows ; i++)
    {
        ...
        {
            if( idGrid.GetGridData(i, iStatus) != '20' && idGrid.GetGridData(i, iStatus) != '110' )    
            {
                alert('Please select the Submited or Revised status at row ' + i + ' to approve...');
                return ;
            }
        }                
    }                
    // update DB
    lstStatus.value = '30'; //Approved(Manager)
    for(i = 1; i < idGrid.rows ; i++)
    {
        if(idGrid.GetGridData(i, iChk) == '-1')
        {
            idGrid.SetGridText(i, iStatus, '30' ); //Approved Manager
            n += 1;           
        }
        else
...
{    
    ...
    // response textarea
    tarResponse.SetDataText( idGrid.GetGridData(lrow, imanager_Response) );    
    if( idGrid.GetGridData(lrow, iStatus) == '20' || idGrid.GetGridData(lrow, iStatus) == '110' )
    {
...
function OnSave()
{
    ...
    for( i = 1; i < idGrid.rows ; i++)
    {
        ...
        if(lCheck == '-1')
        {
            idGrid.SetGridText(i, imanager_Response, tarResponse.GetData() );
            n += 1;    
        }
        else
            idGrid.SetRowStatus(i, 0);                
...
function OnDenied()
{
    ...
    for(i = 1; i < idGrid.rows ; i++)
    {
        if(idGrid.GetGridData(i, iChk) == '-1')
        {
            if( idGrid.GetGridData(i, iStatus) != '20' && idGrid.GetGridData(i, iStatus) != '110' )    
            {
                alert('Please select the Submited or Revised status at row ' + i + ' to reject...');
                return ;
            }
            else if(Trim(tarResponse.GetData()) == '')
            {
                alert('Please input Response information...');
                return ;
            }
            idGrid.SetGridText(i,imanager_Response, tarResponse.GetData() );
        }                
    ...
    // update DB
    lstStatus.value = '70'; //Reject(Manager)
    for(i = 1; i < idGrid.rows ; i++)
    {
        if(idGrid.GetGridData(i, iChk) == '-1')
        {
            idGrid.SetGridText(i, iStatus, '70' ); // Reject (Manager)
            n += 1;               
        }
        else
            idGrid.SetRowStatus(i, 0);
   ...
   var n = 0;
    ...
    for(i = 1; i < idGrid.rows ; i++)
    {
        if(idGrid.GetGridData(i, iChk) == '-1')
        {
            if( idGrid.GetGridData(i, iStatus) != '20' && idGrid.GetGridData(i, iStatus) != '110' )    
            {
                alert('Please select the Submited or Revised status at row ' + i + ' to request more info...');
                return ;
            }
            else if(Trim(tarResponse.text) == '')
            {
                alert('Please input Response information...');
                return ;
            }            
    ...
    // update DB
    lstStatus.value = '60'; //Request more info(Manager)
    for(i = 1; i < idGrid.rows ; i++)
    {
        if(idGrid.GetGridData(i, iChk) == '-1')
        {
            idGrid.SetGridText(i, iStatus, '60' );                
            n += 1;
        }
        else
            idGrid.SetRowStatus(i, 0);
...
<gw:data id="thr_expense_mst" onreceive="OnDataReceive(this)"> 
    <xml> 
        <dso type="grid" function="hr.sp_sel_cc_expense_approve" parameter="1,10,22" procedure="hr.sp_upd_cc_expense_approve" > 
            <input bind="idGrid" > 
...
<table width="100%" border="1" cellpadding="0" cellspacing="0" style="height:100%" >
        ...
        <td style="border-bottom:none; border-left:none; border-right:none; ">
            <table border="0" cellpadding="0" cellspacing="0" width="100%" style="height:100%" >
                ...
                <tr>
                    <td ></td>
                    <td align="left"></td>
                    <td align="right"><gw:imgBtn id="btnSearch" img="search" alt="Search" styels="width:100%"  onclick="OnSearch()" /></td>
                    <td align="right"><gw:icon id="btnApprove" text="Approved" styels="width:100%" onclick="OnApprove()" /></td>
                    <td align="right"><gw:icon id="btnDenied" text="Reject" styels="width:100%" onclick="OnDenied()" /></td>
                    <td align="right"><gw:icon id="btnReqInfo" text="Request More Info" styels="width:100%" onclick="OnRequestInfo()" /></td>                    
                    <td align="right"><gw:icon id="btnSave" text="Save Response" styels="width:100%"  onclick="OnSave()" /></td>                     
                </tr>
            </table>
        </td>
    </tr>
    <tr style="height:58%">
        <td>
            <gw:grid id="idGrid" 
                header="Select|_thr_expense_req_pk|Req No|_requester_pk|EMP ID|EMP Name|_tco_dept_pk|Department|_Group|Submitted Date|Status|Occurred Date|Type|Amount|Leave Days|Leave From|Leave To|Join Date|Remaining|Total Budget|Payment Method|Approved Date|_Response|_ga_response|Description"
                ...
                widths="700|0|1200|0|1200|2500|0|1500|1500|1500|1500|1500|2000|1500|1200|1200|1200|1200|2000|1700|2000|1500|0|0|2000"
                sorting="F" 
                acceptNullDate="T"
   ...
   <tr style="height:20%">
        <td style="border-bottom:none; border-left:none; border-right:none; border-top:none " >
            <table border="0" cellpadding="0" cellspacing="0" width="100%" style="height:100%" >
                ...
                <tr>
                    <td align="left"><gw:textarea id="tarResponse" rows="5" styles="width:100%; " /></td>                    
                </tr>
            </table>
        </td>
    </tr>
...
<gw:textbox id="txtTacABPLCenter_PK" style="display:none" />
