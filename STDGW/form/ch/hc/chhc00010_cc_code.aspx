...
function BodyInit()
{
    ...
    <%=ESysLib.SetGridColumnComboFormatString2( "grdCharacters" , 5 , "SYSDATE|SYSDATE|BYHAND|BY HAND|CHAR|CHAR|CARDINAL|CARDIAL|EMPID|EMP ID|APPROVERID|APPROVER ID") %>;  
    txtIDCode.text=idcode;
    var maxid;
    if (txtIDCode.text=="Emp")
        maxid="<%=ESysLib.SetDataSQL("select 1 from hr.thr_employee where del_if=0 and rownum=1")%>";
    else
        maxid="<%=ESysLib.SetDataSQL("select 1 from thr_expense_req where del_if=0 and rownum=1")%>";
    if (maxid!="1") 
