...
{ 	
    ...
    if (v_language!="ENG")
     ...
     <%=ESysLib.SetGridColumnComboFormat( "grdEmployee" , 3 , "select a.code,a.code_nm from vhr_hr_code a where a.id='HR0071' ORDER BY A.seq") %>; 
...
<gw:data id="datDeptData" onreceive="OnDataReceive(this)"  > 
        <xml> 
            <dso  type="list" procedure="HR_PRO_DEPT_DATA_ALL" > 
                <input>
...
<gw:data id="datGroupData" onreceive="OnDataReceive(this)"  > 
        <xml> 
            <dso  type="list" procedure="hr_pro_group_data_all" > 
                <input>
                    <input bind="lstDept" /> 
                </input> 
                <output>
...
<gw:data id="datEmployeePopup"> 
    <xml> 
        <dso  type="grid" function="HR_SEL_BIZ_EMP_POPUP"> 
            <input> 
                <input bind="lstDept" /> 
                <input bind="lstGroup" /> 
                <input bind="chkTemp" /> 
				        ...
				        <table   border="0" CELLSPACING="0" CELLPADDING="0" style="height:inherit" >
					        <tr>   
						        ...
						        <td width="15%"><gw:list id="lstDept" styles="color:black;width:100%" value="ALL" onchange=""> 
	                                           <data><%=ESysLib.SetListDataSQL("SELECT PK,ORG_NM FROM TCO_ORG WHERE DEL_IF = 0 AND ORG_TYPE='06' order by ORG_ID")%>|ALL|Select All</data>
	                                           </gw:list > 
						        </td>
