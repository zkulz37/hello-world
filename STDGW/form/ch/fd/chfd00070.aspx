...
function OnSelect()
{
    ...
    if (irow>=1)
    {
        ...
        if (iapp_level!="01" ) // dang o cap do GA Staff
            Display_Res_2(true);
        else
            Display_Res_2(false);
        if ((iapp_level=="01" && txtStatus.text=="03" ) || (iapp_level=="03" && txtStatus.text=="06" ) )//chi hien thi cac chuc nang neu (status ="Approve" && Approve Level ="Manager") hoac (status="Revise" && Approve level="GA Staff")
        {
...
<gw:data id="datProReview" onreceive="OnDataReceive(this)"  > 
        <xml> 
            <dso  type="process" procedure="hr.sp_pro_biz_review" > 
                <input>                    
...
<gw:data id="datApproval_Biz_Trip" onreceive="OnDataReceive(this)"> 
    <xml> 
        <dso  type="grid" function="hr.sp_sel_approval_biz_trip_s" > 
            <input> 
                <input bind="lstDept" /> 
                <input bind="lstGroup" /> 
                <input bind="chkTemp" /> 
		        ...
		        <tr>
			        <td align="Left">
				        <table   border="0" CELLSPACING="0" CELLPADDING="0" style="height:inherit" >
					        <tr>   
						        <td width="11%" align="right"><font color="black"><b >Department &nbsp;</b></td>
						        <td width="15%" colspan=2 ><gw:list id="lstDept" styles="color:black;width:100%" value="ALL" onchange=""> 
	                                           <data><%=ESysLib.SetListDataSQL("SELECT PK,ORG_NM FROM comm.TCO_ORG WHERE DEL_IF = 0 order by ORG_ID")%>|ALL|Select All</data>
	                                           </gw:list > 
						        </td>
						        <td width="10%" align="right"><font color="black"><b >WGroup &nbsp;</b></td>
        ...
        <td  id="tblMaster"  width=100% >
            ...
            <table id="idGrid" align = top  width="100%" style="height:70%" CELLSPACING="0" CELLPADDING="0">
                <tr align = top style="height:100%"> 
	                <td colspan=3 align = top  width="100%"  >
  		                <gw:grid   
			                id="grdEmployee"  
                                header='Application No|Applicant ID|Applicant Name|Department|Group|Status|From|To|Biz Type|Biz Object|Total Expenses|Summitted DT|App Type|_PK|_Response1|_Response2|_Approve_Level|_Status_Code|_App_Type_Code|_Biz_Trip_PK|Approval Level|_Request_Flag|_pl_pk|_account_cost'
            ...
            <table id="idRes" align = top  width="100%" style="height:30%" CELLSPACING="0" CELLPADDING="0">
                ...
                <tr style="height:10%">
                    ...
                    <td align = center width="10%" ><gw:icon id="icnReviewed" img="in" text="Reviewed" onclick="OnApprove(1)" /></td>
                    <td align = right  width="10%" ><gw:icon id="icnRequest" img="in" text="Request Infor" onclick="OnApprove(2)" /></td>
                </tr>
            </table>
	    </td>
