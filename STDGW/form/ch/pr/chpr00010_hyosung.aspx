    ...
    <table width='100%'>
        ...
        <td id="tr4" colspan=6 style="white-space:nowrap;display:none">  
            <gw:list  id="lstSchool" value=" " maxlen = "10" styles='width:100%' onchange="OnSetSchool()" > 
                            <data>
                                <%=ESysLib.SetListDataSQL("select code,CODE_NM from vhr_hr_code where id='HR0146' order by code_nm")%>||
                            </data>
             </gw:list>
        </td>
...
<fieldset> <legend><font color="#e9361d" size="1" >Apply</font></legend>
    <table width='100%'>
        ...
        <td colspan=6><gw:list  id="lstOrg_Code" value=" " maxlen = "100" styles='width:100%' >
                            <data>
                                <%=ESysLib.SetListDataSQL("SELECT PK, ORG_NM FROM COMM.TCO_ORG A WHERE a.DEL_IF=0 and nvl(a.end_date,to_char(sysdate+1,'yyyymmdd'))>to_char(sysdate,'yyyymmdd') ORDER BY NVL(SEQ,99999)")%>||
                            </data>
                        </gw:list>
        </td>
