...
function OnDataReceive(po_oData)
{
    ...
    lbRecord.text = grdPackWI.rows-1 + " record(s)";
}
//===========================================================
function OnSelect()
{
    if ( chkClose.value == 'N' )
    {
        ...
        if(grdPackWI.row>0)   
            code_data[0] = grdPackWI.GetGridData(grdPackWI.row, 0) ;	
...
function OnProcess(pos)
{
    switch (pos)
    {
        case 'Close' :
            if ( grdPackWI.row > 0 )
            {
                if ( confirm ('Do you want to close W/I : ' + grdPackWI.GetGridData( grdPackWI.row, G_WI_No) ))
                {
                    txtWIPK.text = grdPackWI.GetGridData( grdPackWI.row, G_WI_PK);
        ...
        case 'UnClose' :
            if ( grdPackWI.row > 0 )
            {
                if ( confirm ('Do you want to unclose W/I : ' + grdPackWI.GetGridData( grdPackWI.row, G_WI_No) ))
                {            
                    txtWIPK.text = grdPackWI.GetGridData( grdPackWI.row, G_WI_PK);
                    
                    pro_dsep00072_1.Call();
                }    
            }
            else
            {
...
<body>
    <!------------------------------------------------------------------>
    <gw:data id="data_dsep00072" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso id="1" type="grid"   parameter="0,1,2" function="<%=l_user%>lg_sel_dsep00072" > 
                <input> 
                ...
                <output bind="grdPackWI" /> 
            </dso> 
        </xml> 
    </gw:data>
    <!------------------------------------------------------------------>
    <gw:data id="pro_dsep00072" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso type="process" procedure="<%=l_user%>lg_pro_dsep00072" > 
                <input>
                    <input bind="txtWIPK" /> 
                </input> 
                <output>
    ...
    <!---------------------------------------------------------------->
    <gw:data id="pro_dsep00072_1" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso type="process" procedure="<%=l_user%>lg_pro_dsep00072_1" > 
                <input>
                    <input bind="txtWIPK" /> 
                </input> 
                <output>
    ...
    <table style="width: 100%; height: 100%">
        ...
        <tr id='top' style="height: 99%">
            <td colspan="8">
                <gw:grid id="grdPackWI" header='_pk|W/I Date|W/I No|Status|P/O No|Style|Employee|Description'
