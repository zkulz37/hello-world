...
function OnPopUp(pos)
{
    switch(pos)
    {
         case 'POConfirm':
            if ( grdConfirm.row > 0 )
            {
                var path = System.RootURL + '/form/ds/cd/dscd00061.aspx?type=APPROVE&RETURN_PK=' + grdConfirm.GetGridData( grdConfirm.row, G1_RETURN_PK);
         ...
         case 'POApprove':
            if ( grdApprove.row > 0 )
            {
                    var path = System.RootURL + '/form/ds/cd/dscd00061.aspx?type=CANCEL&RETURN_PK=' + grdApprove.GetGridData( grdApprove.row, G2_RETURN_PK);
...
<body>
    ...
    <gw:data id="data_dscd00060" onreceive="OnDataReceive(this)">
        <xml> 
            <dso id="1" type="grid"  function="<%=l_user%>lg_sel_dscd00060"  >
    ...
    <gw:data id="data_dscd00060_1" onreceive="OnDataReceive(this)">
        <xml> 
            <dso id="2" type="grid"  function="<%=l_user%>lg_sel_dscd00060_1"  >
