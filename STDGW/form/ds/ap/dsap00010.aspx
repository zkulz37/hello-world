...
function FormatGrid()
{
    ...
    grdOutDetail.GetGridControl().FrozenCols = G1_INCOME_M_PK;
...
function OnSearch(id)
{
    ...
    {
        case 'grdOutDetail':
            data_dsap00010.Call('SELECT');
        break;
        
        case 'grdReqDetail':
            data_dsap00010_1.Call('SELECT')
        break;        
...
function OnPopUp(pos)
{
    switch(pos)
    {
		case 'NewReq':   
               ...
               var path = System.RootURL + '/form/ds/ap/dsap00011.aspx';
        ...
        case 'MakeReq':        
         ...
         case 'ModifyOrdPlan':
            ...
            {  
                var path = System.RootURL + '/form/ds/ap/dsap00012.aspx?master_pk='+ grdReqDetail.GetGridData( grdReqDetail.row, G2_PAYMENT_M_PK);
	            var object = System.OpenModal( path, 1000, 600, 'resizable:yes;status:yes', this); 
...
function OnDataReceive(obj)
{
    switch(obj.id)
    {
        ...
        case 'pro_dsap00010_1' :
            alert(txtReturnValue.text);
            
            OnSearch('grdReqDetail') ;
        break;  
        
        case 'pro_dsap00010_2' :
            alert(txtReturnValue.text);
...
function OnProcess(pos)
{
    switch (pos)
    {
        case 'Close' :
            if ( grdReqDetail.row > 0 )
            {
                if ( confirm ('Do you want to close request no : ' + grdReqDetail.GetGridData( grdReqDetail.row, G2_SLIP_NO) ))
                {
                    txtCollectReqPK.text = grdReqDetail.GetGridData( grdReqDetail.row, G2_PAYMENT_M_PK);
                    
                    pro_dsap00010_1.Call();
                }    
            }
            else
            {
                alert('Pls select one request slip.');
            }            
        break;
        ...
        case 'UnClose' :
            if ( grdReqDetail.row > 0 )
            {
                if ( confirm ('Do you want to unclose request no : ' + grdReqDetail.GetGridData( grdReqDetail.row, G2_SLIP_NO) ))
                {            
                    txtCollectReqPK.text = grdReqDetail.GetGridData( grdReqDetail.row, G2_PAYMENT_M_PK);
                    
                    pro_dsap00010_2.Call();
                }    
            }
            else
            {
     ...
     //------------------------------------------                    
            for(i = 1; i < grdOutDetail.rows; i++)
            {
                if (grdOutDetail.GetGridData(i, G1_CHK) == -1)
                {
                    ...
                    array_ref_pk = array_ref_pk + t_link + grdOutDetail.GetGridData(i, G1_INCOME_M_PK);
...
<body>
    <!--------------------------------------------------------------------->
    <gw:data id="data_dsap00010" onreceive="OnDataReceive(this)">
        <xml> 
            <dso id="1" type="grid"  function="<%=l_user%>lg_sel_dsap00010"  >
    ...
    <!--------------------------------------------------------------------->
    <gw:data id="data_dsap00010_1" onreceive="OnDataReceive(this)">
        <xml> 
            <dso id="2" type="grid"  function="<%=l_user%>lg_SEL_dsap00010_1"  >
    ...
    <!---------------------------------------------------------------->
    <gw:data id="pro_dsap00010_1" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso type="process" procedure="<%=l_user%>lg_pro_dsap00010_1" > 
                <input>
    ...
    <!--------------------------------------make plan------------------------------->
    <gw:data id="pro_dsap00010" onreceive="OnDataReceive(this)"> 
        <xml> 
            ...
            <td>
                <table style="width: 100%; height: 100%" border="0">
                    <tr>
                        ...
                        <td style="width: 20%">
                            <gw:textbox id="txtSupplier" maxlen="100" styles='width:100%' onenterkey="OnSearch('grdOutDetail')" />
                        </td>
