...
function BodyInit()
{
     var data = "<%=ESysLib.SetGridColumnDataSQL("SELECT PK, MENU_NAME FROM  CRM.THT_MENU WHERE DEL_IF=0")%>";
	 ...
	 data = "<%=ESysLib.SetListDataSQL("SELECT PK, MENU_NAME FROM  CRM.THT_MENU WHERE DEL_IF=0" )%>"; 
	 lstMenu.SetDataText(data);
	 data = "<%=ESysLib.SetListDataSQL("SELECT CODE, NAME FROM  COMM.TCO_COMMCODE WHERE DEL_IF=0 AND PARENT_CODE='POINT'" )%>"; 
...
<body bgcolor='#F5F8FF'>
    <gw:data id="dsoGrdLocationEntry"> 
        <xml> 
            <dso type="grid" parameter="0,1,2,3" function="crm.sp_sel_htrt00080" procedure="crm.sp_upd_htrt00080">  
